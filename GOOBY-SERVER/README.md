...
Privater Multiplayer-/Meta-Server für **GOOBY 5.1** (Godot-Client, siehe
...
```bash
npm ci --omit=dev               # installiert reproduzierbar express + ws
...
**Lokaler Test mit dem Godot-Client:** Entwicklungs-Builds haben absichtlich
keinen gepackten Server-Host. Unter *Einstellungen → Mehrspieler* deshalb
`127.0.0.1`, Port `8080`, TLS aus (und gegebenenfalls das Join-Secret) eintragen.
Release-IPAs erhalten ihren öffentlichen WSS-Host über die CI-Konfiguration;
localhost wird nie als Produktadresse eingebettet.
...
|---|---|---|
| `PORT` | `8080` | Der EINE Port für HTTP + WebSocket; lokale Godot-Clients tragen ihn in den Mehrspieler-Einstellungen ein |
