...
_(leer — hier neue Punkte reinschreiben; zuletzt gezogen: 01.08. ~05:10 UTC — keine neuen Einträge; F-103-Teams gelandet, Acceptance läuft: B serverseitig PASS, A braucht Runde 2 (neue Runtime-Evidenz), C-Sichttest auf frischem Client gleich)_

## Aktuell in Arbeit

| # | Punkt | Status |
|---|-------|--------|
| F-103 | **Polish-Welle 3** (3 parallele Fable-5-Max-Thinking-Teams, alle gelandet + gepusht): **A** FX-Respawn-Hygiene Runde 1 — Root-Cause „FXHelper-Cache-Vergiftung durch createInternalRuntime in den Storm-Tunern" gefixt (Bytecode verifiziert clean); Live-Acceptance fand aber einen ZWEITEN Vektor: Respawn-Stampede auf render-ausgehungertem Client akkumuliert ~12 Stale-Kopien/s/Emitter, jeder Fenster-Re-Attach flusht die volle Population (3 Bursts à ~7180 Warns im Teleport-Churn-Test) ⇒ **Runde 2 läuft** (Team A' mit komplettem forensischem Beweispaket: +7-Delta pro Zyklus, Wisp-Anomalie diskriminiert die Lanes, 83s-Akkumulationsfenster). **B** Credits-Restrisiken GELANDET + serverseitig ABGENOMMEN ✅: Verify-Full-Run — Shatter exakt 290, Formation exakt 300, Blackhole exakt 180, +60 Kamera-Lune-Drains exakt 240 (Lune ändert Counts nicht), HOLD-Drain + end_event-Leak-Gate alle Tags 0, kein Crash; Effigy/Blackhole-Slicing choreo-neutral (Mathe-Beweis im Report, EPIC-Peak −83%). **C** 3 neue Cue-Familien (7 Assets: Altar-Kauf-Blüte kategorie-getönt, Vein-Clear-Jackpot warm/kühl, Night-Omen pale/umbral personal-lane) GELANDET; Sichttest wartete auf frischen Client (alter Boot hatte die Rows nicht gescannt — Boot-Artefakt, frischer Boot subscribed sie nachweislich; dazu processResources-Learning: Team-Gates kompilieren nur Klassen, .fx-Assets brauchen explizit processResources) | 🟡 |
