...
test('§G5.4 targets table: 28 rows verbatim, capScore = divisor × rowMax, exclusions absent', () => {
  assert.deepEqual(Object.keys(TARGETS).sort(), GAME_IDS.slice().sort(), 'exactly the 28 §G5.1 games');
  for (const id of GAME_IDS) {
...
test('§G5.1 wave-3 rollout coverage: all 28 games export applyDifficulty', (t) => {
