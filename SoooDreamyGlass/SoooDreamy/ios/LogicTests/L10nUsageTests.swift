...
final class L10nUsageTests: XCTestCase {
    ...
    private static let textLiteralPattern = #"Text\(\s*"([^"\\]*)"\s*\)"#
    ...
    private var mergedTableKeys: Set<String> {
        Set(CoreStrings.table.keys)
            ...
            .union(OnboardingL10n.table.keys)
            .union(RitualsL10n.table.keys)
            .union(PlatformL10n.table.keys)
            .union(IntelligenceL10n.table.keys)
            .union(SettingsL10n.table.keys)
            .union(PostfachL10n.table.keys)
    }
    ...
    func testUserFacingTextLiteralsStayOnTheReviewedAllowlist() throws {
        let regex = try NSRegularExpression(pattern: Self.textLiteralPattern)
        let reviewedBrandLiterals: Set<String> = [
            "SoooDreamy",
        ...
        for dir in ["SoooDreamy", "Widgets"] {
            for file in try swiftFiles(under: dir) {
                let source = strippingCommentOnlyLines(
                    try String(contentsOf: file, encoding: .utf8)
                )
                let range = NSRange(source.startIndex..., in: source)
                regex.enumerateMatches(in: source, range: range) { match, _, _ in
                    guard let match,
                          let literalRange = Range(match.range(at: 1), in: source) else { return }
                    let literal = String(source[literalRange])
                    let containsLetters = literal.range(
                        of: #"[A-Za-zÀ-ž]"#,
                        options: .regularExpression
                    ) != nil
                    if containsLetters && !reviewedBrandLiterals.contains(literal) {
                        violations.append("\(file.lastPathComponent): Text(\"\(literal)\")")
                    }
                }
            }
        }
        XCTAssertTrue(
            violations.isEmpty,
            "plain user-facing Text literals must use L10n.t or the reviewed brand allowlist: "
