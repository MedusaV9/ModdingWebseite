...
struct StreakCalendarView: View {
    ...
    private var weekdaySymbols: [String] {
        let symbols = AppFormatters.veryShortWeekdaySymbols(language: L10n.lang)
    ...
    private var monthTitle: String {
        AppFormatters.monthYear(displayedMonth, language: L10n.lang)
    }
    ...
    private func dayView(_ cell: DayCell) -> some View {
        if let day = cell.day, let key = cell.key {
            ...
            ZStack {
                // Nacht-first §5: shared days are DEEP wax drops on the
                // night card (wachsTief + light embossing); my-only days a
                // night dot with the member's raw color ring (a fill).
                Circle()
                    .fill(both ? AnyShapeStyle(coupleTint.wachsTief)
                          : AnyShapeStyle(Papier.nachtInnenFill))
                if mine && !both {
                    Circle().strokeBorder(coupleTint.primary, lineWidth: 1.5)
                }
                if isToday {
                    Circle().strokeBorder(Nacht.sekundaer, lineWidth: 1.5)
                        .padding(both || mine ? -3 : 0)
                }
                Text("\(day)")
                    .font(.system(.footnote, design: .rounded).weight(both ? .heavy : .semibold))
                    // Light embossing ink on the deep wax — pinned against
                    // both wax stops; night inks otherwise.
                    .foregroundStyle(both ? AnyShapeStyle(coupleTint.aufWachs)
                                     : mine ? AnyShapeStyle(Papier.aufNacht)
                                     : AnyShapeStyle(Nacht.tertiaer))
                    .monospacedDigit()
            }
    ...
    private var legend: some View {
        HStack(spacing: LayoutMetrics.s(14)) {
            legendItem(fill: AnyShapeStyle(coupleTint.wachsTief),
                       label: L10n.t("home.streakCalendar.legendBoth"))
            legendItem(fill: AnyShapeStyle(coupleTint.primary.opacity(0.35)),
