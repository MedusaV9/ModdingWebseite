...
// MARK: - Pinned messages persistence
...
struct ChatPinnedBanner: View {
        ...
        if let entry {
            ...
            .background(
                RoundedRectangle(cornerRadius: Radius.control, style: .continuous)
                    .fill(Licht.lampengold.opacity(0.10))
                    .overlay(
                        RoundedRectangle(cornerRadius: Radius.control, style: .continuous)
                            .strokeBorder(Licht.lampengold.opacity(0.35), lineWidth: Theme.hairlineWidth)
                    )
            )
    ...
    /// SF Symbol marking the pinned message's kind — icons are symbols,
    /// not emoji glyphs glued onto the preview string.
    private func kindIcon(_ entry: PinnedMessageEntry) -> String? {
        switch entry.kind {
