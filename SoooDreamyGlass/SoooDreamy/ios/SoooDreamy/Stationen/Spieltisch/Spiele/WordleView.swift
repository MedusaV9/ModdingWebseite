...
struct WordleView: View {
    ...
    var body: some View {
        ZStack {
            ...
            ScrollView {
                ...
                // Letter-game column (roadmap 22): board + keys stay one
                // hand-width — a 900-point keyboard helps nobody.
                .contentColumn(.keys)
            }
            if celebrate {
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button {
                    tutorial = GameTutorialCatalog.intro(for: "wordle")
                } label: {
    ...
    private var hardModeChip: some View {
        ...
        return Button(action: toggleHardMode) {
            Text(L10n.t("games.wordle.hard.toggle"))
                ...
                .foregroundStyle(active ? Papier.zimmerOben : Nacht.sekundaer)
                .padding(.vertical, 6)
                .padding(.horizontal, LayoutMetrics.s(11))
                .background(
                    // Lamp-gold when armed, the quiet night inner fill
                    // otherwise — no more freehand white wash.
                    Capsule().fill(active ? AnyShapeStyle(Licht.lampengold)
    ...
    /// VoiceOver reads the whole board as ONE element: the label carries
    /// the guess budget, the value speaks every submitted row letter by
    /// letter with its verdict plus the current entry.
    private var grid: some View {
        ...
        // The Tafel: six stamp rows on one letter-paper sheet.
        .paperCard(padding: .compact)
        .accessibilityElement(children: .ignore)
        .accessibilityLabel(L10n.t("games.wordle.a11y.board",
                                   ["n": "\(guesses.count)",
                                    "total": "\(WordleDaily.maxGuesses)"]))
        .accessibilityValue(boardA11yValue)
    }

    private var boardA11yValue: String {
        var lines = (0..<guesses.count).map { rowA11ySummary($0) }
        if !finished {
            lines.append(currentGuess.isEmpty
                         ? L10n.t("games.wordle.a11y.activeEmpty")
                         : L10n.t("games.wordle.a11y.activeRow",
                                  ["letters": spelledOut(currentGuess)]))
        }
        return lines.joined(separator: ". ")
    }

    /// "Reihe 3: B richtig, L an falscher Stelle, U nicht im Wort, …"
    private func rowA11ySummary(_ row: Int) -> String {
        let marks = WordleDaily.score(guess: guesses[row], solution: solution)
        let content = zip(Array(guesses[row]), marks)
    ...
    private func submittedRow(_ row: Int) -> some View {
        ...
        .animation(reduceMotion ? nil : Theme.Motion.playful, value: bounceRow)
    }
    ...
    /// The letter tray: a karton Zettel under the Tafel, keys speak the
    /// same stamp language as the tiles (fresh brief chips, faded absents).
    ...
    private func letterKey(_ letter: String) -> some View {
        ...
        .buttonStyle(.plain)
        // The best-known verdict of the letter travels with the key.
        .accessibilityValue(mark.map(markA11yName) ?? "")
    }
    ...
    private var backspaceKey: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(L10n.t("games.wordle.a11y.backspace"))
    }

    /// Key verdicts in the stamp inks (documented Tinte/Wachs mapping —
    /// the night mint/gold pair fails ≥4.5:1 on paper): correct keys wear
    /// the full dark-ink stamp, present ones the wax-red pad, absents sink
    /// into the tray as faded ink, unused keys stay fresh brief chips.
    ...
    private func submitGuess() {
        ...
        if guess == solution {
            ...
            GamesA11y.announce(rowA11ySummary(row))
        }
    }
    ...
    private func finishGame(won: Bool, row: Int) {
        ...
        submitResultIfNeeded()
        GamesA11y.announce(won
            ? L10n.t("games.wordle.a11y.won",
                     ["word": solution, "n": "\(guesses.count)",
                      "total": "\(WordleDaily.maxGuesses)"])
            : L10n.t("games.wordle.a11y.lost", ["word": solution]))
        Task {
            // Let the row finish its flip reveal first (instant under
            // Reduce Motion, so the end card may follow right away).
            try? await Task.sleep(nanoseconds: reduceMotion ? 150_000_000 : 1_400_000_000)
            ...
            if won {
                ...
                bounceRow = reduceMotion ? nil : row
    ...
    /// The focused night verdict: dark panel, the verdict symbol in the
    /// lamp's resting glow — the bright Tafel above stays the artifact.
    private var endCard: some View {
        VStack(spacing: LayoutMetrics.s(12)) {
            Image(systemName: didWin ? "heart.fill" : "heart.slash.fill")
                .font(.system(.largeTitle, design: .rounded).weight(.semibold))
                .symbolRenderingMode(.hierarchical)
                .foregroundStyle(didWin ? coupleTint.blend : Nacht.sekundaer)
            ...
            if boardWasHard {
                PillTag(text: L10n.t("games.wordle.hard.pill"), tint: Licht.glut)
            }
            if !didWin {
                solutionReveal
    ...
    private var solutionReveal: some View {
        VStack(spacing: 4) {
            Text(L10n.t("games.wordle.solutionLabel"))
                .font(.system(.caption, design: .rounded))
                .foregroundStyle(Nacht.sekundaer)
            // The ember, not the wax — wax stays material on the night
            // panel (3.2:1); glut is the pinned warm accent (5.6:1).
            Text(solution)
    ...
    private static func isBadDateKey(_ error: Error) -> Bool {
        if case APIError.http(_, let code, _, _) = error, code == "bad_datekey" {
            return true
        }
        return false
    }
    ...
    private var recordLink: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(L10n.t("games.wordle.record.button"))
    }
    ...
    private func duelColumn(name: String, result: WordleResult) -> some View {
        VStack(spacing: 6) {
            ...
            Text(result.win ? "\(result.rows)/6" : "X/6")
                ...
                .foregroundStyle(result.win ? Tinte.dunkel : Tinte.tertiaer)
        }
        .frame(maxWidth: .infinity)
    }
        ...
        HStack(spacing: 3) {
            ForEach(0..<3, id: \.self) { index in
                Circle()
                    ...
                    .opacity(reduceMotion || animating ? 1 : 0.25)
...
private struct WordleTileView: View {
    ...
    var body: some View {
        ...
        // Reduce Motion: the verdict appears in place, no 3D flip.
        .animation(reduceMotion ? nil : .easeInOut(duration: 0.5).delay(delay),
                   value: revealed)
    }

    /// Pencil frame on the paper Tafel — the entry a guess is written into.
    private var unrevealedFace: some View {
        RoundedRectangle(cornerRadius: 10, style: .continuous)
            .fill(Papier.innenFill)
            .overlay(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .strokeBorder(letter.isEmpty ? Papier.kante : Tinte.sekundaer,
                                  lineWidth: 1.5)
            )
            .overlay(
                Text(letter)
    ...
    /// The verdict stamps ON PAPER: the night pair (mint/gold) fails
    /// ≥4.5:1 on brief, so the documented Tinte/Wachs mapping applies —
    /// correct = full dark-ink stamp (13.6:1 pinned), present = wax-red
    /// stamp pad (5.2:1 pinned), absent = faded ink on the sheet
    /// (Tinte.tertiaer, 5.7:1 pinned). The cue icons above keep the
    /// verdicts color-independent.
    ...
    private func borderColor(_ mark: WordleMark) -> Color {
        ...
        case .correct: return Tinte.dunkel
        case .present: return Wachs.rot
        case .absent: return Papier.kante
        }
    }
