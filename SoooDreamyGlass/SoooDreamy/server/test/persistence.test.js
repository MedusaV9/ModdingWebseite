...
test('data survives close() + reopen with the same DATA_DIR (flush on close)', async (t) => {
  ...
  // store.json is a small v2 manifest; couple data lives in its own segment.
  const raw = JSON.parse(await readFile(path.join(dataDir, 'store.json'), 'utf8'));
  assert.equal(raw.version, 2);
  assert.equal(raw.format, 'segmented-v1');
  assert.ok(raw.couples[coupleId]);
  // v6.1: segments are checksum envelopes ({format, sha256, couple}).
  const segment = JSON.parse(
    await readFile(path.join(dataDir, 'segments', raw.couples[coupleId]), 'utf8'),
  );
  assert.equal(segment.format, 'segment-v2');
  assert.equal(segment.couple.id, coupleId);
  assert.equal(segment.couple.name, 'Mia & Ben');
  ...
  const rawVoice = await aApi.get(voice.audioUrl);
