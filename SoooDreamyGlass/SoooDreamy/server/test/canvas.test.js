...
test('strokes: create, broadcast, list ascending, clear with broadcast', async (t) => {
  ...
  // Clear attribution: the partner's client says WHO wiped the board. The
  // generation counter (+1 per clear, sync contract f) rides along so
  // clients can drop in-flight strokes of the wiped board.
  assert.deepEqual(cleared.payload, { by: b.memberId, generation: 2 });
  assert.equal((await a.api.get('/api/canvas')).body.strokes.length, 0);
});

test('canvas_live: in-progress strokes relay to the partner, unsanitized junk is clamped', async (t) => {
  const { baseUrl } = await makeApp(t);
  const { a, b } = await setupCouple(baseUrl);
  ...
  aSock.send({
    type: 'canvas_live',
    payload: { phase: 'draw', color: '#FF5C8A', width: 6, tool: 'glow', points: [[0.1, 0.2], [0.2, 0.3]] },
  });
  const live = await bSock.waitFor('canvas_live');
  assert.equal(live.payload.memberId, a.memberId);
  assert.equal(live.payload.phase, 'draw');
  assert.equal(live.payload.color, '#FF5C8A');
  assert.equal(live.payload.width, 6);
  assert.equal(live.payload.tool, 'glow');
  assert.deepEqual(live.payload.points, [[0.1, 0.2], [0.2, 0.3]]);

  // Junk payload → clamped defaults, never a crash, still relayed.
  aSock.send({ type: 'canvas_live', payload: { phase: 42, width: 'huge', color: 7, tool: null } });
  const junk = await bSock.waitFor('canvas_live');
  assert.equal(junk.payload.phase, 'draw');
  assert.equal(junk.payload.width, 5);
  assert.equal(junk.payload.color, '#FF5C8A');
  assert.equal(junk.payload.tool, 'pen');
  assert.deepEqual(junk.payload.points, []);

  // The relay is partner-only: the sender never gets an echo. Verify by
  // sending a follow-up "end" frame — b receives it as the NEXT frame.
  aSock.send({ type: 'canvas_live', payload: { phase: 'end' } });
  const end = await bSock.waitFor('canvas_live');
  assert.equal(end.payload.phase, 'end');
});
