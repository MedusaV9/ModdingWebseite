...
const LEGACY_TOKEN_A = 'legacy_token_a_1234567890';
const LEGACY_TOKEN_B = 'legacy_token_b_1234567890';
...
function v1StoreJson() {
  ...
  return {
    ...
    tokens: {
      [LEGACY_TOKEN_A]: { coupleId: 'c_old', memberId: 'm_a' },
      [LEGACY_TOKEN_B]: { coupleId: 'c_old', memberId: 'm_b' },
    },
  };
}
...
test('a v1.0 store.json loads cleanly and all v1.1/v1.2 endpoints work on it', async (t) => {
  ...
  const a = client(baseUrl, LEGACY_TOKEN_A);
  const b = client(baseUrl, LEGACY_TOKEN_B);
