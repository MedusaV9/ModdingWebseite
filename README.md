...
Freunde-Besuchen. Dieses Repo (bzw. dieser Branch) enthält **GOOBY 5.1**: den
...
> [!IMPORTANT]
> **Dieser Branch ist eine eigenständige Arbeitslinie** im Repo
> `MedusaV9/MinecraftBubbleShieldMod` (aktuell: `cursor/gooby-godot-loop-d1d8`;
> Vorgänger: `loop-2c10` → `loop-next`). Der `main`-Branch enthält ein
> **anderes Projekt** (eine Minecraft-Mod „Bubble Shield", Java/Gradle) —
> nicht mischen, nicht nach `main` mergen. Das GOOBY-Projekt ist in Runde W16
> aus dem Repo `MedusaV9/CustomServerPrivate` (Branch
> `cursor/gooby-godot-rewrite-d1d8`) hierher umgezogen; der volle Verlauf
> (Runden W1–W15) wurde übernommen.
