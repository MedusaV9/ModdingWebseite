...
A single-page marketing/community website for **BAPBAP Modding** (a fan-made community modding project for the Steam roguelike party game *BAPBAP*). Built with **Vite + React + TypeScript + Tailwind CSS v4** as a static SPA. Primary community CTA is the Discord `discord.gg/BAPBAPMods`.
...
- **Content lives in data files**: all site content (mod catalog, game modes, launcher features/changelog, external links) is hardcoded in `src/data/` (`mods.ts`, `modes.ts`, `launcher.ts`, `links.ts`). Edit these to update the catalog — sections in `src/sections/` render from them.
