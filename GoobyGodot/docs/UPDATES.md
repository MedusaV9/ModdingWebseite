...
`application/config/version` (`project.godot`, aktuell 5.1.0). Release-Tag bzw.
Dispatch-Input, Projektversion, `CFBundleShortVersionString` und
`CFBundleVersion` müssen in CI exakt übereinstimmen.
...
Einmalig unter GitHub → Settings → Secrets and variables → Actions →
**Variables** die öffentliche Adresse setzen:

```text
GOOBY_RELEASE_WSS_URL=wss://<öffentlicher-dns-name>/ws
```

Erlaubt sind ausschließlich öffentliche `wss://`-Hosts mit `/ws`; localhost,
private IPs, reservierte Namen, URL-Zugangsdaten und unsicheres `ws://` machen
den Release-Lauf rot. Normale Branch-Builds ignorieren die Variable und behalten
den ehrlichen Offline-Default.
...
1. validiert den öffentlichen WSS-Endpunkt und bettet ihn in das Config-Pack ein,
2. baut wie bei jedem Push die verifizierte unsignierte .ipa (`ios-ipa`),
3. benennt sie versioniert um (`GOOBY-godot-unsigned-v5.1.0.ipa`),
4. erstellt/aktualisiert das GitHub-Release `ipa-v5.1.0` mit deutschem
   Release-Notes-Gerüst („Was ist neu?“ nach dem Lauf ausfüllen!),
5. bumpt `latest_native` im `updates`-Manifest via
...
Vor dem Tag/Dispatch muss `application/config/version` in `project.godot` exakt
der Release-Version entsprechen. Der Lauf bricht sonst vor dem Export ab; auch
abweichende Bundle-Versionen in der gebauten IPA werden nicht veröffentlicht.
...
1. `GOOBY-GODOT/content/config/data/config.json` editieren:
   `"net": {"host": "<öffentlicher-dns-name>", "port": 443, "tls": true}` +
