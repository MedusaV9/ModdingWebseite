...
## Diese Dateien gehören dem separat in CI ausgeführten W1c-Runner. Sie werden
## bei der Discovery ausgeschlossen statt als scheinbar grüne SKIPs zu enden.
const DEDICATED_RUNNER_PREFIXES: Array[String] = ["res://tests/unit/test_ui_"]
## Renderer-/Screenshot-Fahrer mit eigenem `extends SceneTree` und direktem
## CLI-Aufruf. Sie sind keine TestCase-Suiten und werden in ihren dedizierten
## visuellen Läufen ausgeführt.
const DIRECT_DRIVER_FILES: Array[String] = [
	"test_mpa_screens.gd",
	"test_mpb_screens.gd",
	"test_mpc_screens.gd",
	"test_mpf_screens.gd",
...
func _run_all() -> void:
	...
	# Pending queue_free-Aufträge abarbeiten und die Coroutine vollständig
	# abwickeln, bevor SceneTree beendet wird. Ein direktes quit() hielt die
	# letzten Script-/Case-Referenzen bis in ResourceLoader.cleanup lebendig
	# und produzierte falsche "resources still in use"-Leakmeldungen.
	await process_frame
	call_deferred("_finish_run", 1 if failed > 0 else 0)


func _finish_run(exit_code: int) -> void:
	quit(exit_code)
...
func _run_script(script_path: String) -> Dictionary:
	...
	if not (case is TestCase):
		print("  FAIL %s — unerwartete Test-Basisklasse (nicht TestCase)." % script_path)
	...
	if methods.is_empty():
		print("  FAIL %s — keine test_*-Methode entdeckt." % script_path)
		case = null
		script = null
		return {"total": 1, "failed": 1}
	for method in methods:
		...
		if not finished:
			...
			for failure in failures:
				print("  FAIL %s::%s — %s" % [script_path.get_file(), method, failure])
	# TestCase und geladenes Skript VOR dem nächsten Suite-Durchlauf
	# freigeben. Besonders große Testdateien halten über const-preloads sonst
	# hunderte Mesh-/Material-Ressourcen bis zum Engine-Shutdown fest.
	case.tree = null
	case = null
	methods.clear()
	script = null
...
func _discover_test_scripts(root: String) -> Array[String]:
	...
	while entry != "":
		...
		if dir.current_is_dir():
			...
			if not DIRECT_DRIVER_FILES.has(entry) and not _belongs_to_dedicated_runner(path):
				found.append(path)
		entry = dir.get_next()
	dir.list_dir_end()
	return found
...
func _belongs_to_dedicated_runner(path: String) -> bool:
	for prefix in DEDICATED_RUNNER_PREFIXES:
