...
##   tools/ci/run_godot_isolated.sh \
##     xvfb-run -a -s "-screen 0 2868x1320x24" godot --path GOOBY-GODOT \
##     --rendering-method gl_compatibility --rendering-driver opengl3 \
##     --audio-driver Dummy --resolution 2868x1320 \
##     --script res://tests/tools/playtest_harness.gd 2>&1 | tee lauf.log
##   WICHTIG: xvfb-Screen = Fenstergröße (sonst Phantom-Safe-Insets, Befund
##   B6 W18 — xvfb-runs Default 1280x1024 ist kleiner als das Leitformat).
##
## ── Umgebungsvariablen ───────────────────────────────────────────────────────
...
##   PLAYTEST_DEVICE_METRICS  "0": Geräte-Metriken AUS (nur für Vergleiche
##                     mit alten Läufen). DEFAULT AN (W20/D4): jeder Lauf
##                     simuliert das Leitgerät — UiScale.screen_scale_override
##                     = 3.0 (Retina) + Dynamic-Island-Insets (quer 59/0/59/21,
##                     hoch 0/59/0/34 pt, Werte wie fb3_ui_audit.SIZES).
##                     OHNE die Metriken rendert die Harness mit Canvas-f 1,0
##                     statt ~1,64 — alle Screenshots/Proben zeigten die UI
##                     ~40 % kleiner und randnäher als auf dem echten iPhone
##                     (Befund D4, UI-Rework W20).
...
## Geräte-Metriken (W20/D4): Leitgerät iPhone 17 Pro Max — Retina-Faktor
## + Safe-Insets in PUNKTEN [l, t, r, b] (Werte wie fb3_ui_audit.SIZES).
const GERAET_SCREEN_SCALE := 3.0
const GERAET_INSETS_QUER_PT: Array[float] = [59.0, 0.0, 59.0, 21.0]
const GERAET_INSETS_HOCH_PT: Array[float] = [0.0, 59.0, 0.0, 34.0]
...
func _lauf() -> void:
	...
	if not _konfiguriere():
		quit(3)
		return
	# Canvas-Größe steht erst nach einem Frame fest (Stretch-Neuberechnung
	# nach dem Resize) — die Insets-Simulation braucht sie in Canvas-px.
	await process_frame
	_setze_geraete_metriken()
...
## W20/D4: Geräte-Metriken simulieren (Default AN, PLAYTEST_DEVICE_METRICS=0
## schaltet ab) — Retina-screen_scale 3.0 + Dynamic-Island-Insets des
## Leitgeräts, Rechnung wie fb3_ui_audit._audit_size (Punkte → Canvas-px).
## MUSS vor dem Mount der Main-Szene laufen: Screens lesen die Overrides
## beim ersten Layout-Pass.
func _setze_geraete_metriken() -> void:
	_geraete_metriken = OS.get_environment("PLAYTEST_DEVICE_METRICS") != "0"
	if not _geraete_metriken:
		UiScale.screen_scale_override = 0.0
		UiScale.insets_override = Rect2()
		_log("Geräte-Metriken AUS (PLAYTEST_DEVICE_METRICS=0) — Vergleichsmodus")
		return
	UiScale.screen_scale_override = GERAET_SCREEN_SCALE
	var fenster := Vector2(root.size)
	var canvas := _canvas_groesse()
	var pt_kurz := minf(fenster.x, fenster.y) / GERAET_SCREEN_SCALE
	var px_per_pt := minf(canvas.x, canvas.y) / maxf(pt_kurz, 1.0)
	var insets_pt := GERAET_INSETS_QUER_PT if fenster.x >= fenster.y else GERAET_INSETS_HOCH_PT
	...
	_log(
		(
			"Geräte-Metriken AN: screen_scale %.1f, Insets %s pt (f=%.2f)"
			% [GERAET_SCREEN_SCALE, str(insets_pt), UiScale.for_viewport(root)]
		)
	)
...
func _schreibe_bericht() -> int:
	...
	if _geraete_metriken:
		zeilen.append(
			(
				"- Geräte-Metriken: AN (screen_scale %.1f + Safe-Insets, f=%.2f)"
				% [GERAET_SCREEN_SCALE, UiScale.for_viewport(root)]
			)
		)
	else:
		zeilen.append("- Geräte-Metriken: AUS (PLAYTEST_DEVICE_METRICS=0 — Vergleichsmodus)")
	zeilen.append(
