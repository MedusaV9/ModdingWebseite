...
func test_portrait_reflow_stapelt_einspaltig_und_zurueck() -> void:
	...
	# W18 Befund 3: der Bereich (Scroll + gepinnte Kauf-Zeile) wandert als
	# GANZES nach oben — der Scroll lebt jetzt in ihm drin.
	assert_eq(String(body.get_child(0).name), "DetailBereich", "Hochformat: Vitrine + Details oben")
	assert_true(
		(body.get_child(0) as Node).find_child("DetailScroll", false, false) != null,
		"DetailScroll lebt im DetailBereich"
	)
