...
func test_haupt_01_annehmen_bis_abgeben_mit_belohnung() -> void:
	...
	# W18/B1: der Boot registriert den VOLLEN ranch-Slice (RanchState) —
	# frische Saves starten mit RANCH-2-Lagerbestand (apfel 2). Die
	# Belohnungs-Wache misst deshalb das DELTA statt eines Absolutwerts.
	var start_aepfel := int(gs.get_value("ranch.wirtschaft.lager.apfel", 0))
	...
	assert_eq(
		...
		start_aepfel + 2,
		"Item-Belohnung (+2 Apfel) landet im Ranch-Lager"
	)
...
func test_warte_quest_ueber_zeitsprung_mit_notification() -> void:
	...
	fake.items = [
		{
			# Reale lokalisierte ID nutzen: der Test ersetzt nur die
			# Quest-Definition, nicht den String-Katalog.
			"id": "haupt_01",
			"typ": "neben",
			"geber": "rosi",
	...
	RQuestKatalog.reset_cache()
	assert_true(RQuestState.annehmen(gs, "haupt_01"))
	assert_eq(RQuestState.status(gs, "haupt_01"), RQuestEngine.STATUS_WARTEND)
	var ids: Array = []
	for eintrag: Dictionary in NotifyStub.pending():
		ids.append(str(eintrag["id"]))
	assert_true(ids.has("rquest_haupt_01"), "Fertig-Notification ist geplant")
	...
	assert_eq(RQuestState.tick(gs), ["haupt_01"], "Zeitsprung loest das Warten")
	assert_eq(RQuestState.status(gs, "haupt_01"), RQuestEngine.STATUS_ERFUELLBAR)
