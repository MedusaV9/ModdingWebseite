...
func test_dlc_polster_touchfloors_und_cover_deckel() -> void:
	...
	# W20/P3 (Befund C4): der Kopf ist aus dem Scroll GEPINNT — den Notch-
	# Schutz oben trägt jetzt das Kopf-Polster, das Scroll-Kind hält nur
	# noch einen kleinen Abstand unter dem Kopf.
	assert_eq(
		pad.get_theme_constant("margin_top"),
		roundi(4.0 * float(m["f"])),
		"oberes Scroll-Polster = kleiner Abstand unterm gepinnten Kopf (C4)"
	)
	var kopf_polster := screen.get("_kopf_polster") as MarginContainer
	assert_eq(
		kopf_polster.get_theme_constant("margin_top"),
		_polster_soll(m, "top"),
		"der gepinnte Kopf übernimmt den Notch-Schutz (C4)"
	)
	var floor_px := float(m["floor_px"])
	var back := screen.get("_back") as Button
	assert_true(
