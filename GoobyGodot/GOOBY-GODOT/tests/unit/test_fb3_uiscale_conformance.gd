...
## W20/P4 Mikro-Nachfix: das Befund-Format des Results-Safe-Area-Wächters
## (fb3-Audit quer_2556x1179 — kürzeste Dynamic-Island-Quer-Safe-Area).
const ALT_QUER_2556: Array = [Vector2i(2556, 1179), 3.0, [59.0, 0.0, 59.0, 21.0]]
...
## W20/P4 Mikro-Nachfix: die Results-Knopfzeile (Nochmal/Zur Arcade/Nach
## Hause) hing im 2556×1179-Querformat unter dem Home-Indicator (fb3-Befund
## safe_area @ 09_mg_results) — die zentrierte Karte war höher als die
## Safe-Area (Schrift-Fit deckelt bei der Design-Basis, Knöpfe halten den
## Touch-Floor). Seit dem Fix gibt der Gooby-Sticker Höhe ab („Deko weicht
## zuerst“, Pregame-Regel). Wächter misst den Feier-Worst-Case (Rekord +
## Tagesbonus + Spotlight-Bonus + Ziel geschafft) im Befund-Format.
func test_alt_quer_results_knoepfe_in_der_safe_area() -> void:
	await _pin_format(ALT_QUER_2556)
	...
	results.show_results(feier, {"title_key": "mg.results.title"})
	await wait_frames(5)
	for feld: String in ["_again", "_back", "_home"]:
		var knopf := results.get(feld) as Control
		var rect := knopf.get_global_rect()
		assert_true(
			_safe_rect.grow(2.0).encloses(rect),
			"%s bleibt in der Safe-Area (quer_2556x1179: %s)" % [knopf.name, rect]
		)
	results.free()
	await _unpin_format()
