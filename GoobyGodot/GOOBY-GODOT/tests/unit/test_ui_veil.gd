...
## Eval-2026-08 Befund 6: „Invalid polygon data, triangulation failed“ in
## jedem W1c-/CI-Lauf. Quelle (per Frame-Instrumentierung gefangen): der
## Sweep-Austritt am rechten Kartenrand rief den Pill-Zeichner mit
## Füllbreite < Höhe — die alte Zwei-Kappen-Konstruktion erzeugte doppelte
## Nahtpunkte und (float32-Rundung der getrennten Kappenzentren bei großen
## x-Werten) selbstschneidende Nähte. Die Kombination hier ist der exakt
## gefangene Frame und scheiterte mit der alten Konstruktion deterministisch.
func test_pill_punkte_regression_degeneriertes_polygon() -> void:
	var gepinnt := Rect2(488.2987, 0.0, 7.701266, 17.77778)
	var punkte := LoadingVeilBalken.pill_punkte(gepinnt)
	check(punkte.size() >= 3, "schmale Füllbreite bleibt zeichenbar")
	check(
		not Geometry2D.triangulate_polygon(punkte).is_empty(),
		"gepinnte Sweep-Austritts-Kombination trianguliert"
	)
	for i in punkte.size():
		check(
			punkte[i].distance_to(punkte[(i + 1) % punkte.size()]) > 0.0001,
			"keine doppelten Nachbarpunkte an der Naht (i=%d)" % i
		)
	# Breiten-Raster über die kritische Schwelle (Breite ≈ Höhe) an der
	# gefangenen float32-Position: jede gelieferte Liste trianguliert.
	for schritt in 60:
		var breite := 0.1 + float(schritt) * 0.61
		var raster := LoadingVeilBalken.pill_punkte(Rect2(496.0 - breite, 0.0, breite, 17.77778))
		if raster.size() >= 3:
			check(
				not Geometry2D.triangulate_polygon(raster).is_empty(),
				"Raster-Breite %.2f trianguliert" % breite
			)
	# Unzeichenbare Rects werden VOR draw_polygon abgewiesen (leere Liste).
	check(LoadingVeilBalken.pill_punkte(Rect2(0, 0, 120, 0)).is_empty(), "Höhe 0 → leer")
	check(LoadingVeilBalken.pill_punkte(Rect2(0, 0, 0, 8)).is_empty(), "Breite 0 → leer")
