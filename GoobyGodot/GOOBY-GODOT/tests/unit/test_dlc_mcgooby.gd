...
## W19/Welle B: Goldens/Durchläufe umfassen jetzt AUCH die Belegstation
## (zweite Station) und das Demo-Gate (eine Probeschicht pro Tag vor dem
## Kauf) — Kauf-/Gate-Wächter selbst leben in test_w19_mcgooby_b.gd.
## W19/Welle C: die Szene zieht aus ALLEN Rezepten (Fritteuse/Shake-Bar) —
## das Komplett-Durchspiel nutzt das Stations-Timing (timing_aktuell) und
## neue Goldens; Stations-/Rang-Wächter leben in test_w19_mcgooby_c.gd.
...
## Zutaten-Knopf der Belegen-Leiste per id drücken (Welle-B-Durchspiel).
...
func test_golden_autoplay_abrechnung() -> void:
	# Welle B: der Bot spielt pro Bestellung Grill UND Belegstation —
	# die Goldens umfassen jetzt Zutaten-Punkte und Fehlgriffe.
	var bal := McGoobyKatalog.balance()
	...
	assert_eq(int(gold["fehlgriffe"]), 1, "genau 1 Belegen-Fehlgriff im Bot-Lauf")
	assert_eq(int(gold["punkte"]), 118, "60 Grill/Bonus + 60 Zutaten − 2 Malus")
	assert_eq(int(gold["trinkgeld"]), 2, "nur 1 fehlerfreie Bestellung (×1,1)")
	assert_eq(int(gold["muenzen"]), 31, "floor(118/4) + 2 Trinkgeld")
	var gold2 := McGoobySchichtLogic.simulate_autoplay(1234, _menu(), bal)
	assert_eq(int(gold2["punkte"]), 239)
	assert_eq(int(gold2["roestaroma"]), 0)
...
func test_schicht_szene_erststart_intro_und_komplette_schicht() -> void:
	...
	# Deterministisch durchspielen (Welle C: die Schicht zieht aus ALLEN
	# Stationen — Seed 4711: Garten-Gooby + Pommes Klassik): Timing-Runden
	# im goldenen Fenster der JEWEILIGEN Station (Grill/Fritteuse/Shake),
	# Zutaten in Ticket-Reihenfolge aufs Brötchen wischen.
	var sicherheit := 0
	...
	assert_eq(int(kasse["punkte"]), 85, "50 Burger + 35 Pommes, alles perfekt (Goldwert)")
	assert_eq(int(kasse["muenzen"]), 25, "Kassensturz: 21 Basis + 4 Trinkgeld")
	assert_eq(int(gs.get_value("economy.coins", 0)), 25, "Münzen über Economy.award")
	assert_eq(int(gs.get_value("mcgooby.schichten.gespielt", 0)), 1, "Schicht verbucht")
	assert_eq(int(gs.get_value("mcgooby.schichten.bestwert", 0)), 85)
	assert_eq(int(gs.get_value("mcgooby.schichten.perfekt", 0)), 1, "fehlerfrei = Rang-Futter")
	szene.queue_free()
	await wait_frames(1)
	# Zweitstart mit demselben Spielstand: KEIN Intro mehr — aber die
	# Tages-Demo ist verbraucht (Welle B): Sperre-Karte statt Schicht.
	...
	assert_true(szene2.ist_sperre_offen(), "Tages-Demo verbraucht → Sperre-Karte")
	assert_false(szene2.ist_am_laufen(), "keine zweite Probeschicht am selben Tag")
	szene2.queue_free()
	await wait_frames(1)
	# Nach dem Kauf: keine Sperre — die Schicht startet trotz Demo-Stempel.
	gs.set_value("mcgooby.gekauft", true)
	var szene3: McGoobySchichtScene = (load(SCHICHT_SZENE) as PackedScene).instantiate()
	szene3.gs_override = gs
	szene3.seed_override = GOLD_SEED
	szene3.auto_navigate = false
	tree.root.add_child(szene3)
	await wait_frames(3)
	assert_false(szene3.ist_sperre_offen(), "gekauft = nie gesperrt")
	assert_true(szene3.ist_am_laufen(), "gekauft = Schicht startet sofort")
	szene3.queue_free()
	await wait_frames(1)
...
func test_schicht_szene_pause_und_roestaroma() -> void:
	...
	# geht die Bestellung in die Belegen-Phase weiter (Welle B).
	...
	assert_eq(szene.phase_aktuell(), McGoobySchichtScene.PHASE_BELEGEN, "weiter zum Belegen")
	var punkte: Label = szene.find_child("Punkte", true, false)
	assert_true(punkte.text.contains("5"), "5 Röstaroma-Punkte verbucht")
	szene.queue_free()
	await wait_frames(1)
