...
func baue(wurzel: Node3D) -> Dictionary:
	...
	for schritt: Dictionary in schritte(wurzel):
		gruppen[schritt["zone"]] = (schritt["bau"] as Callable).call()
	return gruppen


## W19-Perf: der Bau als einzelne Schritte {zone, bau} — die Region-Szene
## baut die SPAWN-Zone vor dem Reveal und streamt den Rest über das
## Frame-Budget nach. Jede Zone würfelt mit eigenem RNG-Salz, die
## Reihenfolge ändert also keine Positionen (Determinismus).
func schritte(wurzel: Node3D) -> Array[Dictionary]:
	_bau = RanchBau.new(wurzel)
	return [
		{"zone": "bergmassiv", "bau": _baue_bergmassiv.bind(wurzel)},
		{"zone": "blumenwiese", "bau": _baue_blumenwiese.bind(wurzel)},
		{"zone": "moor", "bau": _baue_moor.bind(wurzel)},
		{"zone": "ruine", "bau": _baue_ruine.bind(wurzel)},
		{"zone": "strand", "bau": _baue_strand.bind(wurzel)},
		{"zone": "obstgarten", "bau": _baue_obstgarten.bind(wurzel)},
		{"zone": "kornfeld", "bau": _baue_kornfeld.bind(wurzel)},
	]
