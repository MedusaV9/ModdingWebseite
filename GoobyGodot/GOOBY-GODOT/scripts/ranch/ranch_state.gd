...
## Ensure-Muster (random_events.gd/GoobyeState.ensure_goobye): den
## ranch-Slice IM update-Block anlegen/heilen, BEVOR er beschrieben wird —
## robust, auch wenn die Registrierung erst NACH GameState.initialize()
## lief (Alt-Prozesse, isolierte Tests). W18/B1: der harte
## `state["ranch"]`-Zugriff crashte sonst mitten im Kauf-/Später-Block.
## Fremde Unterschlüssel (RW-3 quests/npc) erhält normalize_slice VERBATIM.
static func ensure_slice(state: Dictionary) -> Dictionary:
