...
##
## W18/B1-Härtung: die Reihenfolge IM Block ist die Transaktion — ERST den
## Slice sicherstellen (ensure_slice) und den Besitz gegenprüfen (Doppel-
## Klick-/Stale-check-Rennen), DANN abbuchen (Economy.spend ist atomar),
## DANN registrieren. Nach spend() kann nichts mehr scheitern; eine
## Abbuchung ohne Kauf-Registrierung ist damit ausgeschlossen.
static func kaufe(gs: Object) -> String:
	...
	# (W13-Falle „Geld weg ohne Leistung“).
	var ergebnis := [RESULT_BROKE]
	gs.update(
		func(state: Dictionary) -> void:
			var ranch := RanchState.ensure_slice(state)
			if bool(ranch.get("gekauft", false)):
