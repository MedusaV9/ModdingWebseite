...
func _baue_hud() -> void:
	...
	# W19: Entdecker-Karte der Region (Fortschritt ansehen, Ziele planen).
	_karte_knopf = SquishButton.new()
	_karte_knopf.theme_type_variation = "GhostButton"
	_karte_knopf.text = I18nService.t("rkarte.knopf")
	_karte_knopf.pressed.connect(_on_karte)
	_fuss_box.add_child(_karte_knopf)
...
func _wende_hud_metriken_an() -> void:
	...
	for knopf: Control in [_stadt_knopf, _ausritt_knopf, _galopp_knopf, _mp_knopf, _karte_knopf]:
...
## W19: Entdecker-Karte öffnen — „Du bist hier“ ist der Hof.
func _on_karte() -> void:
	var heim := RanchEntdeckerKarte.heimat_punkt()
	if not RanchKarteRouten.oeffne_karte(get_tree(), {"spieler": [heim.x, heim.y]}):
		_zeige_toast("(Route ranch/karte — Router fehlt)")
