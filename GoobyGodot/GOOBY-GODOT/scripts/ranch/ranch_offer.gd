...
static func _inhalt(sheet: PanelSheet, gs: Object) -> Control:
	...
	box.add_child(frage)
	# W18/4 Befund B4: die Knopfzeile sitzt GEPINNT im Blatt-Fuß
	# (PanelSheet.add_footer) statt am Ende des scrollenden Inhalts — quer
	# lag „Jetzt losfahren“ sonst unter der Falz und das Angebot wirkte
	# knopflos (IKEA-Muster W18/F6: Inhalt scrollt, CTA bleibt sichtbar).
