...
## W21: alle Fortschrittsbalken des Screens — _apply_metrics skaliert ihre
## Höhe mit f auf den EINEN Balken-Standard (BAR_H).
var _bars: Array[ProgressBar] = []
## W21 (c): Statistik-Wert-Labels + Zielwerte fürs Count-Up (RM-gated).
var _zaehler_labels: Array = []
...
func _build_ui() -> void:
	...
	# W21 (c): Karten federn gestaffelt ein (MotionKit, RM-gated), danach
	# zählen die Statistik-Werte hoch (count_up) — deferred, weil die
	# Labels erst NACH dem Einhängen im Baum sind.
	MotionKit.stagger_ein(_list_box.get_children())
	call_deferred("_starte_count_up")


## W21 (b): EINE Kopfzeilen-Grammatik — AcnhKit.blatt_kopf (Zurück-Pill
## links, Titel mittig, Level-Kapsel als rechter Info-Chip).
func _build_header() -> Control:
	_header = AcnhKit.blatt_kopf(I18nService.t("profil.titel"), _on_back_pressed)
	_back_btn = _header.back_button
	# Testkontrakt (flow_w18a6): der Zurück-Knopf heißt weiterhin „BackBtn“.
...
func _build_abschluss_card() -> Control:
	...
	# W21-Typo-Hierarchie: der Abschluss-Prozent ist DIE eine Hero-Zahl
	# der Ansicht (SIZE_HEADLINE) — Kartentitel laufen eine Stufe tiefer.
	prozent.theme_type_variation = &"HeadlineLabel"
	...
	var zusatz_titel := Label.new()
	zusatz_titel.name = "AbschlussZusatzTitel"
	zusatz_titel.theme_type_variation = &"SoftLabel"
	zusatz_titel.text = I18nService.t("profil.abschluss_zusatz")
	box.add_child(zusatz_titel)
	for teil in AbschlussLogic.zusatz_komponenten(state):
		var id := str(teil["id"])
		box.add_child(
			_field_row(
				"Abschluss%s" % id.capitalize(),
...
func _stat_cell(key_text: String, value_text: String) -> Control:
	...
	row.add_child(key)
	var value := Label.new()
	# W21-Typo-Hierarchie: Werte laufen auf der Body-Stufe mit 800er-Gewicht
	# (kräftig neben dem Soft-Schlüssel, aber keine Head-Konkurrenz).
	value.add_theme_font_override("font", ThemeService.font(800))
	value.text = value_text
	row.add_child(value)
	# W21 (c): reine Zahlen zählen beim Öffnen hoch (MotionKit.count_up).
	if value_text.is_valid_int():
		_zaehler_labels.append([value, int(value_text)])
	return row
...
func _card(node_name: String, title_text: String, icon := "") -> PanelContainer:
	...
	var title := Label.new()
	# W21-Typo-Hierarchie: Kartentitel = Titel-Stufe; die EINE Headline der
	# Ansicht ist der Abschluss-Prozent (Hero-Zahl).
...
func _field_row(node_name: String, key_text: String, value_text: String) -> Control:
	...
	var value := Label.new()
	value.name = "Wert"
	# W21-Typo-Hierarchie: Wert = Body-800 (wie die Reisepass-Felder).
	value.add_theme_font_override("font", ThemeService.font(800))
	value.text = value_text
	row.add_child(value)
	return row
...
## W21 (c): Lebenszeit-Zähler zählen beim Öffnen hoch statt zu springen
## (MotionKit.count_up ist Reduced-Motion-gated: Endwert sofort).
func _starte_count_up() -> void:
	for paar: Array in _zaehler_labels:
		var label := paar[0] as Label
		if label != null and is_instance_valid(label) and label.is_inside_tree():
			MotionKit.count_up(label, 0, int(paar[1]))
...
func _apply_metrics() -> void:
	...
	# W21 (b): der blatt_kopf hebt Zurück auf den Touch-Floor und hält die
	# Seiten-Slots symmetrisch (Titel bleibt optisch mittig).
	_header.apply_metrics(m)
	for btn in _nav_btns:
		if is_instance_valid(btn):
			ScreenShell.touch_target(btn, m)
	# W21: Balkenhöhen skalieren mit — EIN Balken-Standard (BAR_H).
	for bar in _bars:
		if is_instance_valid(bar):
			bar.custom_minimum_size = Vector2(0.0, AcTokens.px(AcTokens.BAR_H, f))
	# Nutzbare Zeilenbreite (Canvas minus Insets minus Screen-Ränder).
	var canvas: Vector2 = m["canvas"]
	var insets: Dictionary = m["insets"]
	var avail := canvas.x - float(insets["left"]) - float(insets["right"]) - AcTokens.px(48.0, f)
	if _portrait != null:
		...
		# W20 B2: zusätzlich in der HÖHE deckeln — im Querformat mit
		# Geräte-Metriken (f=3,0) trieb der 504-px-Slot die Pass-Karte
		# sonst mit aus dem Canvas.
		side = minf(side, maxf(canvas.y * 0.30, float(m["floor_px"])))
	...
	if _pass_card != null and is_instance_valid(_pass_card):
		...
		_pass_card.setze_schmal(avail < AcTokens.px(560.0, f))
		# W20 B2: im Querformat rücken die hinteren Feldzeilen in eine
		# zweite Spalte — die Karte passt wieder komplett in den Canvas.
		_pass_card.setze_kompakt(canvas.x > canvas.y)
