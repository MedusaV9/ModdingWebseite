...
##
## FIX1 (P0-Runde nach dem ersten iPhone-Test): Skalierung über die
## ZENTRALE Regel `UiScale.for_viewport()` (kurze Kante + Retina-Faktor);
## Stats/Statuszeile BÜNDIG an der Kante (Safe-Area + 8 px Schattenluft);
## Cockpit-Spalte mit deutschen LABELS + einmaligem Coachmark
## (AppSettings `hints.hud_actions_seen`).
##
## UICOZY (Web-Parität, GOOBY/src/ui/hud.js + styles.css): Stat-Icons in
## BEIDEN Layouts in der Pille (farbig, Hochkant flext wie Web flex:1);
## Innenmaße/Schriften über `UiScale` (seit FB3 auch die Dock-Geometrie);
## Münz-Count-Up + Icon-Wackler; Balken gleiten (UiMotion.bar_to);
## Aktions-Icons in Identitätsfarben (Web .g5-hud-btn svg).
##
...
## Halbwellen-Dauer des Alert-Pulses (Quelle: HudStatKapseln; Alias für
## den W19-Wächter test_w19_hud_alert_puls).
const ALERT_PULS_S := HudStatKapseln.ALERT_PULS_S
...
const EDGE_PAD := 8.0
## W21/ACNH P1 — Cockpit-Label aus der Typo-Skala (SIZE_CAPTION statt der
## alten 12er-Sondergröße); Labels erscheinen NUR bei offenem Mehr-Cluster
## (Ruhe-Cockpit ist icon-only, Name lebt im Tooltip + Coachmark).
const LABEL_FONT := AcTokens.SIZE_CAPTION
## Label-Zeile unter der Kachel (Design-px, ×f) — SPACE_L trägt die
## SIZE_CAPTION-Zeile mit Luft.
const LABEL_PAD := float(AcTokens.SPACE_L)
...
## Kachel-Icon + 9px-Label, 0.375rem Lücke, GENAU 5 Kacheln pro Zeile
## (Web-Kommentar V4/FIX-UI: sonst verwaist der 9. Knopf allein in Zeile 2).
## W21/ACNH P1: Kachel-Icons aus dem Icon-Set (ICON_L statt Web-22er).
const DOCK_BTN := 54.0
const DOCK_ICON := float(AcTokens.ICON_L)
...
## W21/ACNH: Stat-Zeilen-Maße (Icon/Balken) liegen zentral in AcTokens +
## HudStatKapseln (Referenz-Umbau „StatKapsel-Gruppe“); hier bleibt nur der
## Level-Ring (Web .g5-ring 40 ≈ 3.25rem-Gesamtkreis im Hochkant-Chip).
const RING_PX := 40.0
...
## W21: false bis zum ersten apply_layout — erst DANACH zählt ein Wechsel
## als Rotation (Stagger-Einblendung der Stats-Kapseln).
var _layout_bekannt := false
var _buttons: Dictionary = {}
## W20 P1 — Slimming (Befund-Top-10): „Mehr“-Umschalter der Cockpit-Wand;
## Zweitrangiges (HudButtonOrder.SECONDARY) lebt eingeklappt dahinter.
var _mehr_button: Button
var _mehr_offen := false
...
func apply_layout(layout: HudLayoutLogic.Layout) -> void:
	...
	# W21/ACNH: ECHTER Wechsel (Rotation) staffelt die Stats-Kapseln neu ein —
	# der Erst-Pass nicht (Wachen messen direkt nach ready, ohne Frames).
	var gewechselt := _layout_bekannt and layout != current_layout
	_layout_bekannt = true
	...
	# W21/ACNH P1 — EINE Knopfgröße für die ganze rechte Möblierung
	# (Kacheln, Mehr, Zahnrad, Auge, Gooby-Knopf): Kompakt-Höhe der
	# 2-Höhen-Skala, nie unter dem physischen Touch-Floor.
	var einheit := maxf(float(AcTokens.px(float(AcTokens.BTN_H_KOMPAKT), f)), floor_px)
	var btn_size := maxf(DOCK_BTN * f, floor_px) if portrait else einheit
	# Hochkant: Label sitzt IN der Kachel (Web .g5-btn-label) — kein Anbau.
	# Quer (W21 P1): Ruhe-Cockpit ist icon-only — die Label-Zeile existiert
	# nur bei offenem Mehr-Cluster.
	var label_h := LABEL_PAD * f if (not portrait and _mehr_offen) else 0.0
	_column_top = EDGE_PAD + float(insets["top"]) + einheit + 12.0
	...
	# W21/ACNH P1: Kachel-Icons in BEIDEN Layouts aus dem Icon-Set (ICON_L).
	var icon_base := DOCK_ICON
	...
	# W20 P1 — Slimming / W21 P1 — Ruhe-Cockpit: in Ruhe zeigen quer nur
	# die RUHE-Aktionen + „Mehr“; alles Weitere lebt hinter dem Cluster.
	_mehr_button.visible = not portrait
	_mehr_button.custom_minimum_size = Vector2(btn_size, btn_size + label_h)
	_apply_button_label(_mehr_button, HudButtonOrder.MEHR, portrait, f)
	_scale_icon_button(_mehr_button, f, icon_base)
	for id: StringName in HudButtonOrder.hinter_mehr():
		(_buttons[id] as Button).visible = portrait or _mehr_offen
	for id: StringName in HudButtonOrder.RUHE:
		(_buttons[id] as Button).visible = true
	# EVAL-2026-08 Lens B Befund 18 („iPhone-HUD belegt zu viele Ränder“):
	# Auge + Gooby-Lupe sind quer NEBENKNÖPFE und leben in Ruhe hinter
	# „Mehr“ — hochkant behält die Bodenzeile beide.
	_nebenknoepfe_anwenden()
	if portrait:
		_column_width = btn_size
		_dock_btn_px = btn_size
		_dock_gap_px = DOCK_GAP * f
		# REST-2: Freihöhe = ECHTE Höhe der Bodenzeile (Auge = einheit) —
		# seit die 10. Kachel (Quests) die zweite Dock-Zeile rechts füllt,
		# würde floor_px allein kollidieren.
		_dock_clearance = einheit + EDGE_PAD
		...
		# W21 P1: nur nötig, wenn Labels existieren (Mehr-Cluster offen).
		if _mehr_offen:
			for id: StringName in order:
				_apply_button_label(_buttons[id], id, false, f)
			_apply_button_label(_mehr_button, HudButtonOrder.MEHR, false, f)
			# W20 P1 (Befund E9): EIN gemeinsames Schriftmaß für die Wand.
			_vereinheitliche_kachel_labels()
	...
	# W21/ACNH Referenz-Umbau: quer werden die Chips EINE kompakte
	# StatKapsel-Gruppe (Segment-Rollen, Mini-Balken), hochkant bleiben es
	# Mini-Pillen — Rollen und Maße zentral in HudStatKapseln.
	var ring_px := HudStatKapseln.stil_anwenden(
		_chip_nodes,
		_stat_chips,
		_stat_bars,
		_stat_icons,
	...
	_level_ring.custom_minimum_size = Vector2.ONE * ring_px
	_coin_icon.custom_minimum_size = Vector2.ONE * AcTokens.px(AcTokens.ICON_M, f)
	_scale_font(_coin_label, AcTokens.SIZE_CAPTION, f)
	_scale_font(_level_label, AcTokens.SIZE_CAPTION, ring_px / RING_PX)
	# W21/ACNH P1: Zahnrad + Auge auf die EINE Kompakt-Größe (BTN_H_KOMPAKT,
	# Abnahme „Zahnrad/Auge auf BTN_H_*-Höhen“) — vorher 56·f bzw. 72·f.
	_settings_button.custom_minimum_size = Vector2.ONE * einheit
	_scale_icon_button(_settings_button, f, icon_base)
	_eye_button.custom_minimum_size = Vector2.ONE * einheit
	_scale_icon_button(_eye_button, f, icon_base)
	HudMehrCluster.gooby_chip_stil(_gooby_chip, portrait, f, einheit, floor_px)
	refresh_safe_area()
	...
	if gewechselt:
		HudStatKapseln.einblenden(_chip_nodes)
...
func _fit_landscape_column(
	...
	# W20 P1 — Slimming: Zeilen zählt nur die SICHTBARE Kachelmenge — die
	# Wand wird auf Geräte-Metriken nie wieder 3-spaltig (Befund-Top-10).
	var ids := HudButtonOrder.cockpit_order(_mehr_offen, 1)
	var count := ids.size()
	...
	ids = HudButtonOrder.cockpit_order(_mehr_offen, columns)
	for i in ids.size():
		_landscape_column.move_child(_kachel(ids[i]), i)
	...
	# W18/E6 — Label-Mindestbreite: die Kachel ist mindestens so breit, dass
	# der schmalste vollständige Titel (Kurzform zählt mit) bei MIN_PX
	# zwischen die Innenränder passt (+2 px Luft). Bei Geräte-Skalen
	# (f ≈ 1,6, Kachel ≥ 78 px) ist das ein No-Op; es greift bei f ≈ 1
	# (Desktop/xvfb), wo die 48er-Kachel nur ~20 px Textfläche ließ und die
	# Playtest-Läufe „Garde“/„IGohb“-Fragmente zeigten.
	# W21 P1 — Ruhe-Cockpit ist icon-only: die Label-Breite zählt NUR bei
	# offenem Mehr-Cluster — in Ruhe bleiben die Kacheln QUADRATISCH
	# (Flächen-Budget ≤ 8 %; die Breite wuchs sonst mit „Garderobe“ mit).
	var btn_breite := btn_size
	if _mehr_offen:
		var mess_font := _kachel(ids[0]).get_theme_font("font")
		var label_bedarf := 0.0
		for id: StringName in ids:
			label_bedarf = maxf(
				label_bedarf,
				HudLabelFit.mindest_breite(
					mess_font, HudMehrCluster.titel(id, _mehr_offen), HudMehrCluster.kurzform(id)
				)
			)
		btn_breite = maxf(
			btn_size,
			ceilf(label_bedarf) + HudMehrCluster.label_seitenraender(_kachel(ids[0])) + 2.0
		)
	for id: StringName in ids:
		var btn := _kachel(id)
		btn.custom_minimum_size = Vector2(btn_breite, btn_size + label_h)
	...
	_column_width = maxf(btn_breite, theme_min.x) * float(columns) + COLUMN_SEP * float(columns - 1)
	return btn_size
	...
	for key: String in stats:
		if _stat_bars.has(key):
			...
			if _last_stats.has(key):
				...
				# W21/ACNH: spürbare Änderung pulst die Kapsel sanft — nie,
				# wenn der Alarm-Puls läuft oder gleich übernimmt.
				HudStatKapseln.wert_puls(
					_stat_chips[key],
					float(_last_stats[key]),
					value,
					_alert_tweens.has(key) or value < STAT_ALERT_THRESHOLD
				)
			else:
				bar.value = value
...
func set_level(level: int, xp_ratio: float = 0.0) -> void:
	# W20 P1 (Befund B8): Pille nur mit Inhalt — ohne Level (≤ 0) weg.
	if _level_chip != null:
		_level_chip.visible = level > 0
	_level_label.text = str(level) if level > 0 else ""
...
## J6 Münzflug — Ziel-Anker der Münz-/Level-Kapsel: globales Zentrum +
## sichtbar? (HUD geduckt/weg → RewardFlug sammelt im Zähler-Toast).
func kapsel_anker(art: StringName) -> Dictionary:
	var chip: Control = _coin_chip if art == &"muenzen" else _level_ring.get_parent()
	var da := chip.is_visible_in_tree() and not _sichtbarkeit.verdeckt()
	return {"pos": chip.get_global_rect().get_center(), "sichtbar": da}


## J6: Kapsel-Puls beim Eintreffen (Squish-Grammatik; RM = EIN Mini-Puls).
func kapsel_puls(art: StringName) -> void:
	var chip: Control = _coin_chip if art == &"muenzen" else _level_ring.get_parent()
	RewardFlug.kapsel_squish(chip, ThemeService.is_reduced_motion(self))
...
func set_eye_active(active: bool) -> void:
	...
	if active:
		...
		_eye_timer.stop()
	_nebenknoepfe_nachziehen()


## EVAL-2026-08 Lens B Befund 18: Sichtbarkeits-Regel der Boden-Nebenknöpfe.
## Quer in Ruhe sind Auge + Lupe hinter „Mehr“ verstaut (Shot 17: „Auge
## unten, Lupe unten“ belegten permanent Randfläche); das GEDRÜCKTE Auge
## bleibt sichtbar, damit sein Zustand nie unsichtbar weiterläuft — das
## 8-s-Auto-Aus räumt es anschließend wieder ein.
func _nebenknoepfe_anwenden() -> void:
	var portrait := current_layout == HudLayoutLogic.Layout.PORTRAIT
	_eye_button.visible = portrait or _mehr_offen or _eye_button.button_pressed
	_gooby_chip.visible = portrait or _mehr_offen


## Nebenknopf-Sichtbarkeit nach einem Auge-Zustandswechsel nachziehen —
## NIE während einer Verdeckung (HudSichtbarkeit stellt ihre gemerkte
## Soll-Sichtbarkeit beim Zurückkommen selbst wieder her).
func _nebenknoepfe_nachziehen() -> void:
	if _sichtbarkeit != null and _sichtbarkeit.verdeckt():
		return
	_nebenknoepfe_anwenden()
...
func bubble_lane() -> Dictionary:
	...
	if portrait and _portrait_dock.visible:
		...
		# Befund 18: die Auge/Lupe-Zeile zählt nur, wenn sie da ist — in
		# Ruhe (hinter „Mehr“) reicht die Blasen-Lane bis zur Safe-Area.
		if _eye_button.visible or _gooby_chip.visible:
...
## W21/ACNH P1: Kacheln sprechen die AcnhKit.icon_button-Sprache (Frost,
## Soft-Outline, Kompakt-Größe) — der Layout-Pass setzt Maße/Icon-Deckel
## pro Orientierung neu, die Identitätsfarben (Web .g5-hud-btn svg) bleiben.
func _build_action_buttons() -> void:
	for action in ACTIONS:
		var id: StringName = action["id"]
		var btn := AcnhKit.icon_button(load("%s%s.svg" % [ICON_DIR, action["icon"]]), 1.0, true)
	...
	# W20 P1 — Slimming: „Mehr“ klappt das Zweitrangige auf/zu (nur quer).
	_mehr_button = HudMehrCluster.baue_knopf(_on_mehr_pressed)
	_landscape_column.add_child(_mehr_button)


func _build_status_chips() -> void:
	var level_chip := _make_chip("LevelChip")
	_level_chip = level_chip
...
## W20 P1 — Verdeckungs-Vertrag (Befund-Top-1): Overlay-Erzeuger melden
## einen Grund an/ab; das HUD duckt sich, solange IRGENDEIN Grund lebt.
func verdecken(grund: StringName) -> void:
	if _sichtbarkeit != null:
		_sichtbarkeit.verdecken(grund)


func freigeben(grund: StringName) -> void:
	if _sichtbarkeit != null:
		_sichtbarkeit.freigeben(grund)
...
func _baue_sichtbarkeit() -> void:
	...
	eingaben.append(_mehr_button)
	kacheln.append(_mehr_button)
...
func _setup_static_buttons() -> void:
	...
	# W20 P1 (Befund E3 „Auge wirkt wie Fremdkörper“): gleiche Icon-
	# Identitätsfarben-Sprache wie die Aktions-Kacheln (Teal statt Ink).
	for state in ["icon_normal_color", "icon_hover_color", "icon_pressed_color"]:
		_eye_button.add_theme_color_override(state, AcTokens.TEAL)
...
## wirken die Icons verloren. W21 P1: `base` kommt aus dem Icon-Set
## (Standard ICON_L — Kacheln, Zahnrad, Auge, Gooby-Knopf), Rundung über
## die zentrale px()-Konvention.
func _scale_icon_button(btn: Button, f: float, base := DOCK_ICON) -> void:
	btn.add_theme_constant_override("icon_max_width", maxi(AcTokens.px(base, f), 16))
...
## Name steht Hochkant unterm Icon (Web .g5-btn-label, 9 px Basis).
## W21/ACNH P1: das RUHE-Cockpit quer ist ICON-ONLY (dezente Labels nur bei
## Bedarf) — der Name lebt im Tooltip + Erstlauf-Coachmark; erst das offene
## Mehr-Cluster zeigt Labels (SIZE_CAPTION) unter ALLEN Kacheln.
...
## W18/E6: Passt selbst MIN_PX nicht (Playtest-Regression: bei f=1 ist die
## Cockpit-Kachel nur ~50 px breit → 22 px Textfläche), springt die
## KURZFORM ein (`hud.<id>_kurz`, ein vollständiges Wort) statt eines
## „IGohb“-Fragments; Ellipsis bleibt der letzte Ausweg, wenn beides
## scheitert. Der Tooltip trägt weiterhin den vollen Namen.
func _apply_button_label(btn: Button, id: StringName, portrait: bool, f: float) -> void:
	btn.alignment = HORIZONTAL_ALIGNMENT_CENTER
	btn.icon_alignment = HORIZONTAL_ALIGNMENT_CENTER
	btn.clip_text = true
	if not portrait and not _mehr_offen:
	...
	var fit := HudLabelFit.einpassen(
		btn.get_theme_font("font"),
		HudMehrCluster.titel(id, _mehr_offen),
		HudMehrCluster.kurzform(id),
		wunsch,
		HudMehrCluster.label_breite(btn)
	)
	btn.text = String(fit["text"])
...
## W20 P1: Kachel-Knoten zu einer Cockpit-Id (inkl. Mehr-Umschalter).
func _kachel(id: StringName) -> Button:
	return _mehr_button if id == HudButtonOrder.MEHR else (_buttons[id] as Button)


## W20 P1 (Befund E9): EIN Schriftmaß für die ganze Wand (HudMehrCluster).
func _vereinheitliche_kachel_labels() -> void:
	var wand: Array[Button] = []
	for id: StringName in HudButtonOrder.cockpit_order(_mehr_offen, 1):
		wand.append(_kachel(id))
	HudMehrCluster.vereinheitliche_labels(wand)


## Erststart-Coachmark „Deine Knöpfe“ (FIX1, Karte in hud_coachmark.gd):
## erklärt die Cockpit-Spalte einmalig (AppSettings-Key HudCoachmark.SEEN_KEY).
func _maybe_show_coachmark() -> void:
	if _coachmark != null:
		return
	var portrait := current_layout == HudLayoutLogic.Layout.PORTRAIT
	_coachmark = HudCoachmark.vielleicht_anzeigen(self, portrait)
	if _coachmark == null:
		return
	add_child(_coachmark)
	(_coachmark as HudCoachmark).dismissed.connect(func() -> void: _coachmark = null)
	_position_coachmark()
...
func _position_coachmark() -> void:
	if _coachmark == null or not is_instance_valid(_coachmark):
		return
	(_coachmark as HudCoachmark).positioniere(_safe_insets(), _column_width)
...
## Alarm-Puls-Buchführung (Tween-Bau in HudStatKapseln — W19-Lehre: der
## Loop-Tween hängt am CHIP und stirbt lautlos mit ihm).
func _update_stat_alerts() -> void:
	for info in STATS:
		...
		if alerting:
			_alert_tweens[id] = HudStatKapseln.alarm_puls_start(
				chip, ThemeService.is_reduced_motion(self)
			)
		else:
			HudStatKapseln.alarm_puls_stop(chip, _alert_tweens[id])
...
## W20 P1 — Mehr-Kachel klappt das Zweitrangige auf/zu: der Layout-Pass
## macht die Geometrie, die frisch enthüllten Kacheln federn gestaffelt
## auf (W21: MotionKit.stagger_ein — Blatt-Pop, Reduced-Motion-gated im
## Kit: dann sofort da, keine Blende).
func _on_mehr_pressed() -> void:
	AudioDirector.try_play(self, "ui_click")
	_mehr_offen = not _mehr_offen
	HudMehrCluster.zustand_anwenden(_mehr_button, _mehr_offen)
	apply_layout(current_layout)
	if not _mehr_offen:
		return
	var enthuellt: Array = []
	for id: StringName in HudButtonOrder.hinter_mehr():
		enthuellt.append(_buttons[id])
	# Befund 18: Auge + Gooby-Lupe wohnen jetzt im Cluster — sie federn
	# beim Aufklappen mit auf.
	enthuellt.append(_eye_button)
	enthuellt.append(_gooby_chip)
	MotionKit.stagger_ein(enthuellt)
