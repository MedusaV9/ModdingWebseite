...
## Kachel-Basis (Design-px) — Spaltenzahl/Seitenverhältnis rechnen hierauf.
const KACHEL_BASIS := Vector2(190.0, 200.0)
...
## W21 (b): EIN Screen-Kopf (AcnhKit.blatt_kopf) statt Eigenbau-HBox.
var _header: AcScreenHeader
## W20/P3 (Befund A12): Fade-Kanten statt nackter dunkler Scrollbalken.
var _rail_fade: ScrollFade
...
func _apply_metrics() -> void:
	...
	# W21 (b): der blatt_kopf hebt Zurück auf den Touch-Floor und skaliert
	# die Kopf-Schriften (Theme-Rollen) — keine Eigenbau-Skalierung mehr.
	_header.apply_metrics(m)
	_scale_font(_count_label, AcTokens.SIZE_CAPTION)
	# W21-Typo-Hierarchie: der Seiten-Name ist DIE eine Headline der Ansicht.
	_scale_font(_page_title, AcTokens.SIZE_HEADLINE)
	# W21 („nichts hat die gleiche Größe“): die Fortschrittszeile unter dem
	# Seitentitel fehlte in dieser Liste — einziges unskaliertes SoftLabel
	# des Screens (20 px fix, auch bei f=2,9).
	if _page_progress != null:
		_scale_font(_page_progress, AcTokens.SIZE_BODY)
	if _page_icon != null:
		_page_icon.custom_minimum_size = (Vector2.ONE * AcTokens.px(AcTokens.ICON_L, maxf(_f, 1.0)))
	...
	# die KACHEL_BASIS). Lücken = SPACE_M × f (Spacing-Grid) — die
	# Separationen der Container ziehen hier synchron mit.
	var luecke := float(AcTokens.px(AcTokens.SPACE_M, _f))
	_wende_abstaende_an(luecke)
	var avail := spalte
	if not hoch:
		avail -= _rail_scroll.custom_minimum_size.x + luecke
	var kachel_w := float(AcTokens.px(KACHEL_BASIS.x, _f))
	var cols := clampi(int(floorf((avail + luecke) / (kachel_w + luecke))), 2, 4)
	_grid.columns = cols
	var tile_w := (avail - luecke * float(cols - 1)) / float(cols)
	_tile = Vector2(tile_w, tile_w * KACHEL_BASIS.y / KACHEL_BASIS.x)
...
## Spacing-Grid-Pass (W21): alle Separationen des Screens aus EINEM Wert
## (SPACE_M × f für Spalten/Grid, SPACE_S × f für die Rail-Chips).
func _wende_abstaende_an(luecke: float) -> void:
	var eng := AcTokens.px(AcTokens.SPACE_S, _f)
	if _rows_box != null:
		_rows_box.add_theme_constant_override("separation", int(luecke))
	if _split != null:
		_split.add_theme_constant_override("separation", int(luecke))
	if _grid != null:
		_grid.add_theme_constant_override("h_separation", int(luecke))
		_grid.add_theme_constant_override("v_separation", int(luecke))
	if _rail_box != null:
		_rail_box.add_theme_constant_override("separation", eng)
...
func _layout_rail(hoch: bool, spalte: float, floor_px: float) -> void:
	...
	if _rail_fade != null:
		_rail_fade.kanten_hoehe(ScrollFade.KANTE * _f)
	if _grid_fade != null:
		_grid_fade.kanten_hoehe(ScrollFade.KANTE * _f)
	if hoch:
		...
		_rail_scroll.custom_minimum_size = Vector2(
			minf(float(AcTokens.px(240.0, _f)), spalte * 0.32), 0.0
		)
		_rail_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
		# W20/P3 (Befund A12): der nackte dunkle V-Balken der Rail stand
		# direkt neben dem Inhalt („doppelter Scrollbalken“) — Fade-Kanten
		# der ScrollFade-Hülle übernehmen die Affordance.
		_rail_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_SHOW_NEVER
	if _rail_fade != null:
		# Die Hülle sitzt im Split — Flags des Scrolls spiegeln, damit die
		# Rail quer weiter die Spalte füllt bzw. hochkant oben schrumpft.
		_rail_fade.size_flags_vertical = _rail_scroll.size_flags_vertical
...
func _layout_chip(chip: Button, hoch: bool, floor_px: float) -> void:
	chip.custom_minimum_size = Vector2(0.0, maxf(float(AcTokens.px(40.0, _f)), floor_px))
	_scale_font(chip, AcTokens.SIZE_CAPTION)
	if hoch:
...
## W21: Basis-px kommen NUR von der Typo-Skala (AcTokens.SIZE_*),
## Rundung über den Token-Helfer (round()-Konvention, Boden 10).
func _scale_font(ctl: Control, base_px: int) -> void:
	if _f > 1.0:
		ctl.add_theme_font_size_override("font_size", AcTokens.font_px(float(base_px), _f))
	else:
...
## W21 (b): EINE Kopfzeilen-Grammatik — AcnhKit.blatt_kopf (Zurück-Pill
## links, Titel mittig, Zähler-Kapsel als rechter Info-Chip).
func _build_header() -> Control:
	_header = AcnhKit.blatt_kopf(I18nService.t("album.titel"), _on_back_pressed)
	_back_btn = _header.back_button
...
func _build_rail() -> Control:
	...
	# W20/P3 (Befund A12): Fade-Kanten ersetzen den Balken; Maus-/Finger-
	# Drags über den Chips pannen die Rail (Chips stehen auf PASS).
	_rail_fade = ScrollFade.um(scroll)
	_rail_fade.name = "RailFade"
	WischPan.an(scroll)
	return _rail_fade
...
func _build_page_panel() -> Control:
	...
	scroll.scroll_deadzone = 24
	# W20/P3: Fade-Kante = sichtbare Scroll-Einladung (Fold-Regel) + Pan
	# per Maus-Drag (Karten stehen auf PASS).
	_grid_fade = ScrollFade.um(scroll)
	_grid_fade.name = "GridFade"
	panel.add_child(_grid_fade)
	WischPan.an(scroll)
	_grid_scroll = scroll
...
func _show_page(page_id: String, animate := false) -> void:
	...
	if page_id == COLLECTIONS_PAGE:
		...
		# Sichtbarkeit AUCH über die Fade-Hülle schalten — der versteckte
		# Scroll alleine ließe den Wrapper die Panel-Höhe blockieren.
		if _grid_fade != null:
	...
	for def: Dictionary in _by_page.get(page_id, []):
		...
		cards.append(card)
	if animate:
		# W21 (c): Listen-Auftritt über die EINE Motion-Grammatik.
		MotionKit.stagger_ein(cards)
...
func _build_sticker_card(def: Dictionary) -> Control:
	...
	# W20/P3 (Befund A12/E-Katalog): `AcCard` ist eine PanelContainer-
	# Variation — auf dem Button griff still der Default-Pill-Stil.
	card.theme_type_variation = &"AcCardButton"
	...
	# W14-Subraster: Kachel-Innenabstand wie die Arcade-Kacheln —
	# W21: skaliert übers Spacing-Grid statt fixer Design-px.
	var rand := float(AcTokens.px(12.0, _f))
	content.offset_left = rand
	content.offset_right = -rand
	content.offset_top = rand
	content.offset_bottom = -rand
	content.mouse_filter = Control.MOUSE_FILTER_IGNORE
	content.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_XS, _f))
...
func _build_new_badge() -> Control:
	...
	style.set_content_margin_all(float(AcTokens.px(AcTokens.SPACE_XS, _f)))
	badge.add_theme_stylebox_override("panel", style)
	var label := Label.new()
	label.text = I18nService.t("album.neu")
	label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	label.add_theme_color_override("font_color", AcTokens.WHITE)
	_scale_font(label, AcTokens.SIZE_CAPTION)
	badge.add_child(label)
	badge.set_anchors_preset(Control.PRESET_TOP_RIGHT)
	var ecke := float(AcTokens.px(AcTokens.SPACE_XS, _f))
	badge.offset_left = -float(AcTokens.px(64.0, _f))
	badge.offset_top = ecke
	badge.offset_right = -ecke
	return badge
...
func _build_card_art(def: Dictionary, unlocked: bool, tint: Color) -> Control:
	...
	# Karten-Kunst, kein Fließtext: das „?“ steht auf 2× SIZE_TITLE (56) —
	# Skalen-Vielfaches statt freier Zahl.
	mark.add_theme_font_size_override(
		"font_size", AcTokens.font_px(AcTokens.SIZE_TITLE * 2.0, maxf(_f, 1.0))
	)
...
func _build_name_band(def: Dictionary, unlocked: bool) -> Control:
	...
	band_style.set_content_margin_all(float(AcTokens.px(AcTokens.SPACE_XS, _f)))
...
func _on_sticker_tapped(def: Dictionary) -> void:
	...
	body.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_S, _f))
	if unlocked:
		var art := _art_rect(str(def.get("image", "")))
		if art != null:
			art.custom_minimum_size = Vector2(0.0, float(AcTokens.px(220.0, _f)))
			body.add_child(art)
	if unlocked:
		...
		_scale_font(rarity_label, AcTokens.SIZE_CAPTION)
		body.add_child(rarity_label)
	var text := Label.new()
	text.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_scale_font(text, AcTokens.SIZE_BODY)
	if unlocked:
	...
	if not unlocked and not bool(def.get("secret", false)):
		...
		_scale_font(hint, AcTokens.SIZE_CAPTION)
		body.add_child(hint)
...
func _on_hub_sticker_unlocked(def: Dictionary) -> void:
	...
	if str(def.get("page", "")) == _current_page:
		_show_page(_current_page)
		_stemple_sticker(str(def.get("id", "")))
...
## W21 (c) Stempel-Moment: der frisch freigeschaltete Sticker landet als
## Stempel auf der Seite (MotionKit.stempel — RM-gated im Kit, §6.3).
func _stemple_sticker(id: String) -> void:
	if _grid == null or id.is_empty():
		return
	var karte := _grid.get_node_or_null("Sticker_%s" % id) as Control
	if karte != null:
		MotionKit.stempel(karte)
