...
func refresh() -> void:
	...
	for child in _cards_box.get_children():
		...
		child.queue_free()
	_cards_box.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_M, _f))
	var c := _slice()
...
func _slot_columns() -> int:
	if _avail_w <= 0.0:
		return SLOT_COLUMNS
	return clampi(int(_avail_w / float(AcTokens.px(68.0, maxf(_f, 1.0)))), 3, 6)
...
func _build_card_header(set_id: String, progress: Dictionary) -> Control:
	...
	header.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_S, _f))
	var icon := _icon_rect(set_id, float(AcTokens.px(AcTokens.ICON_L, maxf(_f, 1.0))))
	if icon != null:
		icon.self_modulate = AcTokens.INK
		header.add_child(icon)
	var title := Label.new()
	# W21-Typo-Hierarchie: Set-Karten-Titel auf SIZE_BUTTON (800er) — die
	# EINE Headline der Ansicht gehoert dem Seiten-Kopf des Albums.
	...
	_scale_font(title, AcTokens.SIZE_BUTTON)
	...
	_scale_font(count, AcTokens.SIZE_CAPTION)
...
func _build_entry_grid(set_id: String, c: Dictionary) -> Control:
	...
	grid.add_theme_constant_override("h_separation", AcTokens.px(AcTokens.SPACE_S, _f))
	grid.add_theme_constant_override("v_separation", AcTokens.px(AcTokens.SPACE_S, _f))
...
func _build_entry_slot(set_id: String, entry_id: String, c: Dictionary) -> Control:
	...
	slot.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_XS, _f))
	...
	style.set_content_margin_all(float(AcTokens.px(AcTokens.SPACE_S, _f)))
	tile.add_theme_stylebox_override("panel", style)
	var frame := Control.new()
	frame.custom_minimum_size = Vector2.ONE * AcTokens.px(52.0, maxf(_f, 1.0))
	...
	if owned:
		...
		mark.add_theme_font_size_override(
			"font_size", AcTokens.font_px(AcTokens.SIZE_TITLE, maxf(_f, 1.0))
		)
	...
	_scale_font(caption, AcTokens.SIZE_CAPTION)
...
func _build_count_badge(count: int) -> Control:
	...
	style.set_content_margin_all(float(AcTokens.px(AcTokens.SPACE_XS, maxf(_f, 1.0))))
	...
	var ecke := float(AcTokens.px(AcTokens.SPACE_XS, maxf(_f, 1.0)))
	badge.offset_left = -float(AcTokens.px(40.0, maxf(_f, 1.0)))
...
func _build_claim_row(set_id: String, c: Dictionary, progress: Dictionary) -> Control:
	...
	_scale_font(status, AcTokens.SIZE_CAPTION)
	...
	button.custom_minimum_size = Vector2(
		0.0, maxf(float(AcTokens.px(AcTokens.BTN_H_KOMPAKT, maxf(_f, 1.0))), _floor_px)
	)
	# Web-Knopf trägt ein KLEINES Icon (13 px) — ohne Deckel füllt das
	# Coin-SVG sonst die ganze Knopfhöhe.
	button.add_theme_constant_override(
		"icon_max_width", AcTokens.px(AcTokens.ICON_M, maxf(_f, 1.0))
	)
	if claimed:
	...
	_scale_font(button, AcTokens.SIZE_BUTTON)
...
## W21: Basis NUR von der Typo-Skala, Rundung über den Token-Helfer.
