...
func _ready() -> void:
	...
	# W21/P3 (d): Einstellungs-Stimmung (warmer Wash statt Cream) auf die
	# Szenen-Wand legen — Mood-Kontexte sind die EINE Quelle (Spec §Moods).
	var wand := get_node_or_null("Wallpaper") as AcWallpaper
	if wand != null:
		wand.apply_context("einstellungen")
...
func _rebuild() -> void:
	...
	var restore_focus := focus_owner != null and is_ancestor_of(focus_owner)
	...
	if not _revealed:
		...
		MotionKit.stagger_ein(_sections.get_children())
...
func _apply_scale() -> void:
	...
	_back.custom_minimum_size = Vector2(0.0, _row_floor())
	_back.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BUTTON, _tf))
	# W14: Kopfzeilen-Konsistenz — Titelgröße wie auf allen anderen Screens.
	_title.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_TITLE, _tf))
	...
	# W20/P3 (Befund E4 Header-Grammatik): Titel mittig wie auf allen
	# anderen Screens (Szene: expand+center); der Spacer spiegelt die
	# Zurück-Breite, damit die Mitte ohne rechte Info-Pille optisch stimmt.
	var spacer := header.get_node_or_null("HeaderSpacer") as Control
	if spacer != null:
		spacer.custom_minimum_size = Vector2(_back.get_combined_minimum_size().x, 0.0)
...
func _build_gruppe_header(gruppe: Dictionary) -> Control:
	...
	wrap.add_theme_constant_override("margin_top", AcTokens.px(AcTokens.SPACE_M, _f))
	wrap.add_theme_constant_override("margin_left", AcTokens.px(AcTokens.SPACE_S, _f))
	var row := HBoxContainer.new()
	row.name = "GruppeHeader"
	row.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_S, _f))
	...
	glyph.custom_minimum_size = Vector2.ONE * AcTokens.px(AcTokens.ICON_L, _tf)
	...
	# W21-Typo-Hierarchie: der EINE Screen-Titel (SIZE_TITLE) gehört der
	# Kopfzeile — Gruppen-Header treten auf SIZE_BUTTON (800er) zurück.
	title.theme_type_variation = "TitleLabel"
	title.text = I18nService.t(str(gruppe["titel_key"]))
	title.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BUTTON, _tf))
	row.add_child(title)
	wrap.add_child(row)
	return wrap
...
func _build_language_row(rows: VBoxContainer) -> void:
	...
	seg.name = "Value"
	seg.add_theme_constant_override("separation", AcTokens.px(AcTokens.SPACE_S, _f))
	...
	for opt: Array in options:
		...
		btn.custom_minimum_size = Vector2(float(AcTokens.px(120.0, _f)), _row_floor())
		btn.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BUTTON, _tf))
...
func _build_about_section() -> void:
	...
	version_label.add_theme_font_size_override(
		"font_size", AcTokens.font_px(AcTokens.SIZE_BODY, _tf)
	)
	...
	news_btn.custom_minimum_size = Vector2(0.0, _row_floor())
	news_btn.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BUTTON, _tf))
	news_btn.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	news_btn.focus_mode = Control.FOCUS_ALL
	...
	credits_title.add_theme_font_size_override(
		"font_size", AcTokens.font_px(AcTokens.SIZE_BODY, _tf)
	)
	...
	for i in lines.size():
		...
		line.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_CAPTION, _tf))
		rows.add_child(line)
