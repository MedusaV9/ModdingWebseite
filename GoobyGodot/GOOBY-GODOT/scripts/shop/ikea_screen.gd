...
## W18 Befund 3: rechte Spalte = Bereich (Scroll + GEPINNTE Kauf-Zeile).
## Der primäre CTA sitzt AUSSERHALB des Scrolls und ist damit in jedem
## Leitformat ohne Scrollen sichtbar (Muster „gepinnter Fertig-Knopf“,
## s. gobnom_level_select).
var _detail_bereich: VBoxContainer
var _detail_scroll: ScrollContainer
var _detail_panel: PanelContainer
var _kauf_leiste: PanelContainer
## W18 Befund 8: Fade-Kanten + Endpolster als Überlauf-Affordance der
## Kategorie-Chips (ScrollFade-Muster aus G7-P54, wie Garderobe/Gestalten).
var _chip_fade: ScrollFade
var _chip_polster: MarginContainer
var _f := 1.0
...
func _apply_metrics() -> void:
	...
	if _chip_fade != null:
		_chip_fade.kanten_hoehe(ScrollFade.KANTE * _f)
		_chip_polster.add_theme_constant_override("margin_right", roundi(ScrollFade.KANTE * _f))
	_zoom_slider.custom_minimum_size = Vector2(AcTokens.px(120.0, _f), _floor)
	_buy_button.custom_minimum_size = Vector2(AcTokens.px(152.0, _f), _floor)
	...
	# Vitrine: nimmt den Platz, der NACH Detail-Panel + gepinnter Kauf-Zeile
	# übrig bleibt (Höhen-Budget statt fester Mindesthöhe) — reicht er
	# nicht, scrollt der Inhalt ÜBER der Kauf-Zeile; der CTA selbst bleibt
	# immer sichtbar (W18 Befund 3).
	var insets: Dictionary = m["insets"]
	var kopf_reserve := AcTokens.px(28.0, _f)
	var body_h := (
		canvas.y - float(insets["top"]) - float(insets["bottom"]) - kopf_reserve - _floor - 10.0
	)
	if portrait:
	...
	var kauf_h := _kauf_leiste.get_combined_minimum_size().y
	var show_min := clampf(body_h - detail_h - kauf_h - 44.0, 140.0, canvas.y * 0.4)
...
func _apply_body_layout(portrait: bool, m: Dictionary) -> void:
	if _body == null or _detail_bereich == null:
		return
	_body.vertical = portrait
	if portrait:
		if _body.get_child(0) != _detail_bereich:
			_body.move_child(_detail_bereich, 0)
		...
		_detail_bereich.size_flags_stretch_ratio = PORTRAIT_DETAIL_SHARE
		return
	...
	_detail_bereich.size_flags_stretch_ratio = 1.0
...
func _build_ui() -> void:
	# W21/P3 (d): Laden-Stimmung (Shop-Blau-Wash + Shop-Kacheln) statt
	# neutralem Cream — dezente Papier-Tönung pro Bereich (Spec §Moods).
	var paper := AcWallpaper.for_context("ikea")
	add_child(paper)
...
func _build_left_column() -> Control:
	...
	_chip_scroll.name = "ChipScroll"
	_chip_scroll.custom_minimum_size = Vector2(0, 52)
	_chip_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	# W18 Befund 8: Fade-Kante statt nackter Scrollbar als Affordance
	# (Garderoben-Tab-Muster) + Touch-Deadzone, damit ein Wisch über den
	# Chips nicht sofort als Chip-Tap zählt.
	_chip_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_SHOW_NEVER
	_chip_scroll.scroll_deadzone = 24
	_chip_fade = ScrollFade.um(_chip_scroll)
	_chip_fade.name = "ChipFade"
	column.add_child(_chip_fade)
	# Endpolster: der letzte Chip steht am Scroll-Ende frei VOR der
	# Fade-Kante statt hart an der Spaltenkante („Deko“ endete randbündig).
	_chip_polster = MarginContainer.new()
	_chip_polster.name = "ChipPolster"
	for seite: String in ["margin_left", "margin_top", "margin_bottom"]:
		_chip_polster.add_theme_constant_override(seite, 0)
	_chip_polster.add_theme_constant_override("margin_right", int(ScrollFade.KANTE))
	_chip_scroll.add_child(_chip_polster)
	_chips = HBoxContainer.new()
	_chips.name = "CategoryChips"
	_chips.add_theme_constant_override("separation", 6)
	_chip_polster.add_child(_chips)
...
func _build_right_column() -> Control:
	# W18 Befund 3: die rechte Spalte ist ein BEREICH — oben der Scroll
	# (Vitrine, FB3-Muster: Minimalhöhe drückt nie den Body aus dem
	# Canvas), unten die GEPINNTE Kauf-Zeile. Der primäre CTA fiel im
	# Leitformat quer sonst unter den Sichtbereich (nur per unauffälligem
	# Scroll erreichbar, toter Leerraum unter der Karte).
	# W20 A6: das Detail-Panel (Name/Meta/Swatches/Zoom) lebt ebenfalls
	# NICHT mehr im Scroll — mit Geräte-Metriken rutschte die komplette
	# Swatch-Zeile + Zoom-Slider unter die Falz und wirkte HINTER der
	# Kauf-Leiste versteckt. Drei disjunkte Zonen: Vitrine (scrollt bei
	# Not), Detail-Zone (immer sichtbar), Kauf-Leiste (gepinnt).
	_detail_bereich = VBoxContainer.new()
	_detail_bereich.name = "DetailBereich"
	_detail_bereich.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_detail_bereich.size_flags_vertical = Control.SIZE_EXPAND_FILL
	_detail_bereich.add_theme_constant_override("separation", 8)
	...
	_detail_bereich.add_child(scroll)
	...
	_detail_bereich.add_child(_build_detail_panel())
	_kauf_leiste = PanelContainer.new()
	_kauf_leiste.name = "KaufLeiste"
	_kauf_leiste.theme_type_variation = &"AcCard"
	_kauf_leiste.add_child(_build_buy_row())
	_detail_bereich.add_child(_kauf_leiste)
	return _detail_bereich
...
func _build_detail_panel() -> Control:
	...
	# W18 Befund 3 / W20 A6: Kauf-Zeile UND dieses Panel leben außerhalb
	# des DetailScrolls — die Zeilen hier (Swatches/Zoom) sind immer
	# sichtbar, gescrollt wird nur die Vitrine darüber.
...
func _refresh_list() -> void:
	...
	# W21/P3: Regalzeilen blenden gestaffelt ein (MotionKit, RM-gated).
	MotionKit.stagger_ein(row_nodes)
...
func _make_row(item: Dictionary) -> Button:
	...
	row.custom_minimum_size = Vector2(0, maxf(AcTokens.px(AcTokens.BTN_H_PRIMAER, _f), _floor))
