...
## 200-ms-Crossfade). Karten-Modi (Web-Basis + W20-Familien):
...
## - home: Rückkehr/Hausräume — „Trautes Heim“ / „Auf dem Heimweg…“.
## W20/VEIL-ZIELE (Playtest-Fund: die DLC-Bibliothek lud unter der
## „Trautes Heim“-Karte): die Ziel-Wahl liegt jetzt in der Registry
## loading_veil_ziele.gd — exakte Ziele vor Präfix-Familien, dazu neue
## Karten dlc/ranch/arcade/reise mit prozeduralen Stimmungs-Covern und
## der ehrliche „Unterwegs…“-Fallback für unbekannte Ziele.
...
## W20: Ziel→Karten-Registry (Familien + Stimmungs-Cover; gleiches
## preload-Muster wie VeilWipe, ebenfalls ohne class_name).
const VeilZiele := preload("res://scripts/core/loading_veil_ziele.gd")
...
## Karten-Modus: "home" | "trip" | "game" (Web) sowie seit W20
## "dlc" | "ranch" | "arcade" | "reise" | "unterwegs" (Registry).
var _modus := "home"
var _laedt_basis := ""
...
## Karten-Modus fürs Ziel — delegiert an die W20-Registry (exakte Ziele
## vor Präfix-Familien, unbekannte Ziele ehrlich „unterwegs“); die
## Web-Regel §2.4 für home/trip bleibt darin erhalten, „game“ setzt
## weiterhin der Minigame-Hint in _apply_variant.
static func modus_fuer_ziel(target: StringName) -> String:
	return VeilZiele.modus_fuer_ziel(target)
...
## Travel-Hint + das Game-Motiv, die W20-Familien ihr Stimmungs-Cover aus
## der Registry. Der Vorhang bleibt in ALLEN Modi das statische
...
func _apply_variant() -> void:
	...
	var cover_tex: Texture2D = _active_hint.get("cover") if minigame else _cover_fuer_modus()
...
## Cover der Nicht-Minigame-Karte (W20): die Registry liefert das
## Stimmungs-Cover der neuen Familien; home/trip antworten dort mit null
## und behalten das Heim-Artwork (Web-Parität, gepinnt in test_ui_veil.gd).
func _cover_fuer_modus() -> Texture2D:
	var registry_cover: Texture2D = VeilZiele.cover_fuer_modus(_modus)
	return registry_cover if registry_cover != null else _lade_textur(COVER_HOME_PFAD)
