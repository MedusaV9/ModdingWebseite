...
## Leergut-Runden (EVAL B §5 „Coaster fährt Runden"): ohne Fahrgast zieht
## der Zug in gemütlichem Tempo seine Bahn — Tests schalten das ab.
const IDLE_TEMPO_M_S := 7.0

var curve: Curve3D
var sim: Dictionary = {}
var faehrt := false
var idle_faehrt := true
var rig: GoobyRig
...
func _physics_process(delta: float) -> void:
	if faehrt:
		...
		_idle_s = fposmod(_idle_s + IDLE_TEMPO_M_S * delta, curve.get_baked_length())
		_setze_zug(_idle_s)
