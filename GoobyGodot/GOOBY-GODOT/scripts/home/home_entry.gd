...
func _ready() -> void:
	...
	_build_hud()
	# W18/J1: EIN Willkommens-Overlay zur Zeit — der Dirigent sequenziert
	# Tagesbonus/Coachmark/Geburtstag über ihre bestehenden Öffnen-Pfade
	# (Playtest-Befund E4 „Overlay-Stau“ nach dem Onboarding).
	OverlayDirigent.attach_to(self)
	...
	if _gs != null:
		...
		# W18/J6: Münz-/XP-Flug zur HUD-Kapsel — hört zentral auf die
		# GameState-Gutschrift (coins_changed/xp_changed), Ebene 50.
		RewardFlug.attach_to(self, _gs)
...
## W18/B4 (Router-Disziplin): das Settings-Overlay lebt in `_ui_layer` und
## überlebte Routenwechsel — ein aus den Settings gestarteter Screen
## (Codes/Spielstand-Übernahme) mountete HINTER dem Overlay und die
## Settings schluckten alle Taps. Jeder Reiseantritt baut das Overlay
## deshalb generisch ab, BEVOR der Zielscreen aufgedeckt wird.
...
func _on_travel_finished(target: Variant = null) -> void:
	...
	# W19/MITBRINGSEL: Wiedersehens-Moment nach der Urlaubs-Abholung —
	# reiht sich beim Overlay-Dirigenten (W18/J1) ein statt ihn zu umgehen.
