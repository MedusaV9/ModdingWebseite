...
## Grid nach einem Preset-Tausch IN PLACE aus dem Save neu füllen — der
## Aufrufer (BuildMode) teilt sich die GridData-Referenz mit Room/Girlanden/
## Overlay und weist sie NIE neu zu. Frisch kollisionsgeprüft; SURFACE-Items
## erst nach ihren Trägern.
static func reload_grid(gs: Object, grid: GridData, room_id: String) -> void:
	for entry: Dictionary in grid.to_items_array():
		grid.remove_item(str(entry["uid"]))
	...
	for entry: Variant in raw if raw is Array else []:
		...
		if int(def["layer"]) == GridData.Layer.SURFACE:
			...
			continue
		_grid_eintrag(grid, def, entry)
	for entry: Dictionary in surface_entries:
		_grid_eintrag(grid, defs[str(entry["item"])], entry)


## Ein Save-Entry (Boden/Decke ODER Wand) ins lebende Grid setzen.
static func _grid_eintrag(grid: GridData, def: Dictionary, entry: Dictionary) -> void:
	var uid := str(entry.get("uid", ""))
	var at_raw: Array = entry.get("at", [0, 0])
	if entry.has("wall"):
		grid.place_wall(def, str(entry["wall"]), int(at_raw[0]), uid)
		return
	grid.place(def, Vector2i(int(at_raw[0]), int(at_raw[1])), int(entry.get("rot", 0)), uid)
