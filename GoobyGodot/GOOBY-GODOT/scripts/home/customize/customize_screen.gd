...
func _apply_metrics() -> void:
	...
	_kauf_button.custom_minimum_size = Vector2(AcTokens.px(152.0, _f), _floor)
...
func _build_ui() -> void:
	var paper := AcWallpaper.for_context("werkstatt")
	add_child(paper)
...
func _make_kachel(
	...
	kachel.text = "%s\n%s" % [titel, status]
	# Kachel-Schrift folgt dem KACHEL-Deckel (_tile_f statt f, s. o.).
	kachel.set_meta(ScreenShell.META_FONT_SKIP, true)
	kachel.add_theme_font_size_override(
		"font_size", AcTokens.font_px(AcTokens.SIZE_CAPTION, _tile_f)
	)
