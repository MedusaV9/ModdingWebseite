...
## Nachbar-Fenster: Tagesglas (Himmelsspiegelung) vs. warme Abendglut.
const FENSTER_GLAS := Color("#A9C4CE")
const FENSTER_GLUT := Color("#FFD98C")
## Fern-Fenster werden schwächer entsättigt als Wände — die Glut soll
## auch am Horizont noch als warmer Punkt lesen.
const FENSTER_NEBEL_DAEMPFUNG := 0.55

## Geteiltes Instanzfarben-Material (Häuser/Zäune/Hügel = 1 Material).
static var _instanz_material: StandardMaterial3D
## Eigenes UNSHADED-Material nur für Nachbar-Fenster — so leuchten sie
## abends warm (EVAL-2026-08 Lens B: „Fenster leuchten abends“).
static var _fenster_material_geteilt: StandardMaterial3D

## Uhrzeit der Kulisse (Fensterlicht abends) — RoomBase setzt sie nach
## attach_to; die Fenster-Instanzfarben ziehen sofort nach.
var stunde := 12.0:
	set(wert):
		stunde = wert
		_fenster_einfaerben()
...
## Fenster-MultiMesh + Nebelfaktor je Fenster (für Tageszeit-Umfärbung).
var _fenster_multi: MultiMesh
var _fenster_nebel: Array[float] = []
...
## Fensterglut 0..1 (EVAL-2026-08 Lens B „Fenster leuchten abends“):
## 0 = Tagesglas, 1 = volle Abendglut. Weiche Rampen morgens (6–8 Uhr aus)
## und abends (17–20 Uhr an) — pure, test_licht_kalibrierung prüft sie.
static func fensterglut(stunde_wert: float) -> float:
	if stunde_wert < 6.0 or stunde_wert >= 20.0:
		return 1.0
	if stunde_wert < 8.0:
		return 1.0 - (stunde_wert - 6.0) * 0.5
	if stunde_wert < 17.0:
		return 0.0
	return (stunde_wert - 17.0) / 3.0


## Fensterfarbe zur Uhrzeit (Basis OHNE Horizont-Entsättigung).
static func fensterfarbe(stunde_wert: float) -> Color:
	return FENSTER_GLAS.lerp(FENSTER_GLUT, fensterglut(stunde_wert))
...
func _haus_hinzu(
	...
	var nebel := 0.0
	if fern:
		nebel = clampf((pos.length() - 20.0) / 55.0, 0.25, 0.72)
		wand = wand.lerp(HORIZONT, nebel)
		dach = dach.lerp(HORIZONT, nebel)
		tuer = tuer.lerp(HORIZONT, nebel)
	var fenster := fensterfarbe(stunde).lerp(HORIZONT, nebel * FENSTER_NEBEL_DAEMPFUNG)
...
func _haeuser_abschliessen() -> void:
	...
	for teil: String in meshes:
		...
		var mat := _fenster_material() if teil == "fenster" else _instanzfarben_material()
		(multi.mesh as PrimitiveMesh).material = mat
...
## Fenster-Instanzfarben an die aktuelle Uhrzeit anpassen (Setter von
## `stunde` ruft das — abends kippt das Glas in warme Glut).
func _fenster_einfaerben() -> void:
	if _fenster_multi == null:
		return
	var basis := fensterfarbe(stunde)
	for i in _fenster_multi.instance_count:
		var nebel := _fenster_nebel[i] if i < _fenster_nebel.size() else 0.0
		var farbe := basis.lerp(HORIZONT, nebel * FENSTER_NEBEL_DAEMPFUNG)
		_fenster_multi.set_instance_color(i, farbe)
...
## UNSHADED + Instanzfarben: Fenster ignorieren die Szenenbeleuchtung und
## „leuchten“ abends aus eigener Kraft (billigster Glüh-Effekt ohne Emission).
static func _fenster_material() -> StandardMaterial3D:
	if _fenster_material_geteilt == null:
		_fenster_material_geteilt = StandardMaterial3D.new()
		_fenster_material_geteilt.vertex_color_use_as_albedo = true
		_fenster_material_geteilt.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	return _fenster_material_geteilt
