...
## Kauf-Gate-Defaults (Doc §6.2: Level 14 + 3000 — greifen ohne Pack).
const DEFAULT_PREIS := 3000
const DEFAULT_FREISCHALT_LEVEL := 14

## Rang-Leiter-Defaults (Welle C, Doc §6.1 auf messbare Größen verdichtet:
## gespielte Schichten, Punkte-Bestwert, fehlerfreie Schichten) — greifen,
## wenn das Pack keinen `raenge`-Block trägt.
const DEFAULT_RAENGE: Array[Dictionary] = [
	{"stern": 1, "schichten": 1, "bestwert": 0, "perfekt": 0},
	{"stern": 2, "schichten": 5, "bestwert": 80, "perfekt": 0},
	{"stern": 3, "schichten": 12, "bestwert": 130, "perfekt": 0},
	{"stern": 4, "schichten": 25, "bestwert": 170, "perfekt": 1},
	{"stern": 5, "schichten": 40, "bestwert": 200, "perfekt": 3},
]
...
## Kaufpreis des DLC in ᴳ (Balance-Block des Menü-Packs, Doc §6.2: 3000 —
## „sofort eine EINKOMMENS-Maschine“, deshalb teurer als Goo und Bye).
static func preis() -> int:
	return maxi(0, int(balance().get("preis", DEFAULT_PREIS)))


## Freischalt-Level des Kaufs (Doc §6.2: 14 — EIN Level vor der Ranch,
## entzerrt die beiden Kauf-Momente; die Probeschicht bleibt für alle frei).
static func freischalt_level() -> int:
	return maxi(1, int(balance().get("freischalt_level", DEFAULT_FREISCHALT_LEVEL)))


## Rang-Leiter des Laden-Sterne-Fortschritts (Welle C): aufsteigende
## Stufen {stern, schichten, bestwert, perfekt} aus dem Pack — fehlt der
## Block, greifen die Defaults (Fail-Safe wie preis/freischalt_level).
static func raenge() -> Array:
	var raw: Variant = daten().get("raenge", [])
	if raw is Array and not (raw as Array).is_empty():
		return raw
	return DEFAULT_RAENGE.duplicate(true)


## Stations-Eintrag per id ({} = unbekannt) — Namen/Gesten für die Tab-Leiste.
