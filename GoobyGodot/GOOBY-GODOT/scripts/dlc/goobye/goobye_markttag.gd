...
##
## W19 Welle B (§4.4 Tagesangebot): Sortiments-Zeilen dürfen zusätzlich
## `angebot: true` tragen (GoobyeAngebot.sortiment_markieren) — das wirkt
## NUR auf Vergleiche (+40 % Griff-Chance) und den Stückpreis (−15 %),
## nie auf die Zufallsfolge. Ohne Flag bleibt jeder Plan bit-identisch
## zu Welle A (Golden-Tests unverändert).
##
			...
			. append(
				{
					...
					"angebot": bool((zeile as Dictionary).get("angebot", false)),
				}
			)
		)
	return out
...
static func _listen_einkauf(
	...
	for p in anzahl:
		...
		# Tagesangebot (§4.4): +40 % Griff-Lust — nur der VERGLEICH ändert
		# sich, die Los-Folge bleibt (Monotonie: Angebot ⇒ nie weniger).
		if bool(zeile.get("angebot", false)):
			chance += GoobyeAngebot.GRIFF_BONUS
...
static func _quengelware(
	...
	for zeile: Dictionary in zeilen:
		if str((zeile["ware"] as Dictionary).get("gruppe", "")) != "suesses":
			continue
		var preis := GoobyeAngebot.preis_fuer_zeile(
			zeile["ware"], float(zeile["faktor"]), bool(zeile.get("angebot", false))
		)
		...
		. append(
			{
				"ware": ware_id,
				"preis":
				GoobyeAngebot.preis_fuer_zeile(
					zeile["ware"], float(zeile["faktor"]), bool(zeile.get("angebot", false))
				),
			}
		)
