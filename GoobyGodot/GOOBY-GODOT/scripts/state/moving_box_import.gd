...
static func _base64url_decode(text: String) -> PackedByteArray:
	# Marshalls.base64_to_raw() schreibt bei kaputten Zeichen selbst dann
	# einen Engine-ERROR, wenn der Aufrufer den leeren Rückgabewert korrekt
	# behandelt. Vorvalidieren hält hostile Importdaten aus dem Engine-Log.
	if text.is_empty() or text.length() % 4 == 1:
