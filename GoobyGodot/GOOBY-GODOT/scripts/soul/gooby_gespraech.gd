...
## W19-Playtest: die Gesprächs-Chips (z. B. der Abend-Prompt „Kuschelabend?“)
## erschienen als graue Karte HINTER der offenen Heimkehr-Übergabe — sie
## reihen sich jetzt beim Overlay-Dirigenten (W18/J1) ein statt ihn zu
## umgehen. Sanfter Hinweis = NACH Heimkehr (5), Tagesbonus (10),
## Coachmark (20) und Geburtstag (30).
const PRIO_GESPRAECH := 40
...
## Ebene-1-Chips — mit Dirigent im Baum erst an ihrer Reihe (W19: nie
## unter/hinter ein offenes Willkommens-Overlay drängeln). true = Gespräch
## läuft oder ist eingereiht.
func starte(anlass_id: String) -> bool:
	...
	if dirigent == null:
		# Ohne Dirigenten (nackte Tests) öffnen die Chips direkt — wie bisher.
		_oeffne_chips(gespraech)
		return true
	dirigent.anfordern("gespraech", PRIO_GESPRAECH, _oeffne_chips.bind(gespraech))
	return true


## Öffnen-Pfad (ruft der Dirigent an seiner Reihe; null = inzwischen
## hinfällig — Raum weg oder schon andere Chips offen, dann kommt das
## nächste Ticket). Ebene-2-Chips laufen bewusst weiter direkt: sie sind
## die Fortsetzung DESSELBEN Gesprächs, kein neues Willkommens-Overlay.
func _oeffne_chips(gespraech: Dictionary) -> Control:
	if aktiv() or seele == null or _ui_layer() == null:
		return null
	_ebene = 1
