...
static func default_slice() -> Dictionary:
	return {
		...
		"besuche": {},
		"souvenirTag": 0,
		# W19/MITBRINGSEL (additiv): Heimkehr-Latches — ReiseLogic.abholen
		# stempelt Ziel/Abflug/Abhol-Zeitpunkt, HeimkehrLogik feiert den
		# Reunion-Moment genau einmal (heimkehrGefeiert) und leitet daraus
		# Mitbringsel-Seed + 24-h-GOOBY-FREE-Fenster ab.
		"heimkehrZiel": "",
		"heimkehrAbflug": 0,
		"heimkehrAt": 0,
		"heimkehrGefeiert": false,
	}
...
static func slice_of(state: Dictionary) -> Dictionary:
	...
	return {
		...
		# W19/MITBRINGSEL: Heimkehr-Latches (additiv) — gleiche Regel wie die
		# W13B-/W15-Latches (sonst würfe slice_of sie weg).
		"heimkehrZiel": raw.get("heimkehrZiel") if raw.get("heimkehrZiel") is String else "",
		"heimkehrAbflug": maxi(0, int(_num(raw.get("heimkehrAbflug")))),
		"heimkehrAt": maxi(0, int(_num(raw.get("heimkehrAt")))),
		"heimkehrGefeiert": raw.get("heimkehrGefeiert") is bool and raw.get("heimkehrGefeiert"),
