...
const INTRO_S := 1.5
## Warm-weiße Kontur des Hinweises (M7): hebt die Zeile von Wiese/Himmel ab.
...
func _build_labels() -> void:
	# W21/P5: Zeit/Serie sitzen als Tinte auf der Kit-Frost-Pille — die
	# warm-weiße Kontur bleibt nur dem Hinweis (auf Milchglas).
...
## Canvas-Einheiten ≠ Fensterpixel. W21/P5: Zeit/Serie als Kit-Chip (EIN
## Raster über alle Spiele), Hinweis mit Typo-MINIMUM (MgHudKit.font_px).
func _layout_hud() -> void:
	if _time_label == null:
		return
	var vp := get_viewport_rect().size
	MgHudKit.style_chip(_time_label, _combo_label, _ui)
	MgHudKit.layout_chip(_time_label, _combo_label, _ui)
...
func _catch(item: Dictionary, vp: Vector2) -> void:
	...
	# W21/P5 Feier-Beat: die Goldmöhre ist DER Beat des Laufs — Gold-Pille
	# + Sparkle (MotionKit/RM-gated) statt kleinem Zahlen-float_text.
	if kind == "golden":
		MgHudKit.feier_beat(
			self, view_size, _ui, I18nService.t("mg.hudkit.beat_gold", {"n": int(item["value"])})
		)
	if ctx.juice != null:
		if kind == "golden":
...
## Kit-Chip hinter Zeit/Serie, Milchglas hinter dem Hinweis: fallende Ware
## und Wiese zogen sonst direkt durch die Ziffern (Lesbarkeit auf dem Handy).
func _draw() -> void:
	if _time_label == null:
		return
	MgHudKit.draw_chip(self, _hud_plate, _time_label, _combo_label, _ui)
...
func _draw_banner() -> void:
	MgHudKit.draw_banner(self, _banner_plate, get_viewport_rect().size, _ui, _banner, _banner_t)
