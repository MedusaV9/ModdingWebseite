...
## W21: Milchglas-Plate hinter dem Steuer-Hinweis (HintFeel, s. dort).
...
func _process(delta: float) -> void:
	...
	_step_lauf(delta)
	_step_optik(delta)
	HintFeel.fade_hint(_hud, _hint_label, elapsed)
...
func _sprung_geschafft(h: Dictionary, qualitaet: String) -> void:
	...
	# W21/P5 Feier-Beat: der Perfekt-Sprung ist DER Kern-Moment — Gold-Pille
	# + Sparkle (MotionKit/RM-gated); der Gut-Sprung bleibt beim float_text.
	if qualitaet == "perfekt":
		var beat_text := I18nService.t("mg.hudkit.beat_perfekt", {"n": plus})
		MgHudKit.feier_beat(self, view_size, _ui(), beat_text)
	elif ctx.juice != null:
		var at := _screen_pos(Vector3(float(h["at"]), 1.6, 0.0))
		var text := "%s +%d" % [I18nService.t("mg.ranchParcours.gut"), plus]
		ctx.juice.float_text(at, text, AcTokens.GOLD)
...
func _build_hud() -> void:
	...
	HintFeel.install(_hud, _hint_label, _hint_plate, func() -> float: return elapsed)
	# W21/P5: Zeit/Punkte als Kit-Chip (Frost-Pille statt nackter Labels).
	MgHudKit.install_chip(_hud, _zeit_label, _punkte_label, _ui)
...
## DER _ui-Faktor (Kit-Kanon) — vorher klebte das HUD in Fix-Pixeln.
func _ui() -> float:
	return MgHudKit.ui_scale(view_size)
...
func _layout_hud() -> void:
	if _zeit_label == null:
		return
	var ui := _ui()
	MgHudKit.style_chip(_zeit_label, _punkte_label, ui)
	MgHudKit.layout_chip(_zeit_label, _punkte_label, ui)
	_hint_label.add_theme_font_size_override("font_size", MgHudKit.font_px(15.0, ui))
	var s := HintFeel.clamp_size(_hint_label, minf(view_size.x - 24.0 * ui, 340.0 * ui))
	_hint_label.position = Vector2((view_size.x - s.x) * 0.5, 12.0 * ui)
	# W21/P5: die Reit-Buttons skalieren mit (_ui), Touch-Floor 48 px bleibt
	# auch unterm 0,75er-Boden erhalten (memoryMatch-Spick-Knopf-Muster).
	for btn: Button in [_galopp_btn, _sprung_btn]:
		btn.add_theme_font_size_override("font_size", MgHudKit.font_px(16.0, ui))
		btn.custom_minimum_size = Vector2(150.0 * ui, maxf(48.0, 64.0 * ui))
		btn.size = btn.custom_minimum_size
	_galopp_btn.position = Vector2(18.0 * ui, view_size.y - _galopp_btn.size.y - 18.0 * ui)
	_sprung_btn.position = Vector2(
		view_size.x - _sprung_btn.size.x - 18.0 * ui, view_size.y - _sprung_btn.size.y - 18.0 * ui
	)
