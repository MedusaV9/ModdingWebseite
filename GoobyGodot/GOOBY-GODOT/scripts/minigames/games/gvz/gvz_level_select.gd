...
func _ready() -> void:
	...
	# W20/P4 Inhaltsspalte: die zentrale VBox (Titel/Gitter/Balken/Footer)
	# wird in _apply_metrics auf ScreenShell.content_width gedeckelt und
	# über symmetrische Margins zentriert — der Gold-Balken zog sich sonst
	# als 97-%-Web-Leiste über die volle Viewport-Breite (Stretch-Wache).
	column.set_meta(ScreenShell.META_CONTENT_COLUMN, true)
...
## W20/P4: die Seiten-Margins deckeln die Spalte zusätzlich auf
## ScreenShell.content_width und zentrieren sie (Inhaltsspalte statt
## Vollbreite-Streckung — Kacheln/Balken behalten ihr Design-Maß).
func _apply_metrics() -> void:
	if _margin == null:
		return
	var m := ScreenShell.metrics(get_viewport())
	var f: float = m["f"]
	var rand_x := maxf(14.0 * f, (size.x - ScreenShell.content_width(m)) / 2.0)
	for side in ["left", "right"]:
		_margin.add_theme_constant_override("margin_%s" % side, int(rand_x))
