...
## W18/4 B2: gepinnte Korb-Zeile + Bestell-CTA (leben im PhoneShell-Fuß,
## Standalone-Fallback inline) — Ziel des Korb-Feedback-Pulses.
var _korb_zeile: Label
...
func _render() -> void:
	for kind in _box.get_children():
		kind.queue_free()
	# W18/4 B2: gepinnten Fuß der Vor-Ansicht räumen (nur das Menü füllt ihn).
	PhoneShell.leere_app_fuss(self)
	_korb_zeile = null
	_kaufen_knopf = null
	if gs == null:
		return
...
func _restaurant_karte(restaurant: Dictionary) -> Button:
	...
	# W18/4 B2: PASS — Wisch über der Karte scrollt (s. _knopf).
...
## Menü + kleiner Warenkorb des gewählten Restaurants. W18/4 B2 (IKEA-
## Muster W18/F6): Korb-Zeile + „Bestellen“ leben GEPINNT im PhoneShell-
## Fuß statt am Scroll-Ende (~2 Viewports unter der Falz im Querformat);
## Standalone ohne PhoneShell (Sheet-Weg) bauen sie wie bisher inline.
func _render_menue() -> void:
	...
	var fuss := PhoneShell.app_fuss_bereich(self)
	var ziel: Control = fuss if fuss != null else _box
	var korb_text := I18nService.t("phone.gooberando.warenkorb_leer")
	if not _warenkorb.is_empty():
		korb_text = I18nService.t("phone.gooberando.warenkorb").format(
			{"n": _warenkorb.size(), "summe": summe, "gebuehr": GooberandoLogic.LIEFERGEBUEHR}
		)
	_korb_zeile = PhoneShell.app_label(
		ziel, korb_text, "CaptionLabel" if _warenkorb.is_empty() else ""
	)
	var coins := int(gs.get_value("economy.coins", 0))
	_kaufen_knopf = _knopf(
		I18nService.t("phone.gooberando.bestellen").format({"summe": summe}), "PrimaryButton"
	)
	_kaufen_knopf.disabled = _warenkorb.is_empty() or coins < summe
	_kaufen_knopf.pressed.connect(_on_bestellen)
	ziel.add_child(_kaufen_knopf)
	if fuss != null:
		fuss.visible = true
	if not _warenkorb.is_empty():
...
func _on_in_den_korb(gericht: Dictionary) -> void:
	...
	_render()
	# W18/4 B2 „Korb-Add ohne Feedback“: die (jetzt gepinnte, sichtbare)
	# Korb-Zeile + der CTA pulsen fühlbar. Deferred: die frisch gebauten
	# Controls brauchen erst ihren Layout-Pass (Pivot aus der Größe).
	_korb_puls.call_deferred()


func _korb_puls() -> void:
	for ctl: Control in [_korb_zeile, _kaufen_knopf]:
		if ctl != null and is_instance_valid(ctl) and ctl.is_inside_tree():
			UiMotion.bounce(ctl)
...
## W18/4 B2: PASS statt Button-Default STOP — Drags über den Knöpfen
## erreichen so den PhoneShell-Scroll (Wisch scrollt, F6-Arcade-Muster);
## der Tap bleibt beim Knopf (unter der Deadzone feuert pressed normal).
...
## W18/4 B2: der gepinnte Fuß liegt AUSSERHALB des App-Teilbaums und
## bekommt seine Skalierung deshalb explizit mit.
func _skaliere_schriften() -> void:
	PhoneShell.app_fonts_skalieren(self)
	var fuss := PhoneShell.app_fuss_bereich(self)
	if fuss != null:
		PhoneShell.app_fonts_skalieren(fuss)
