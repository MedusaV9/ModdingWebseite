...
## AKTIVE Wagen/Goobys (Gameplay-Checks laufen nur über diese Listen);
## die *_pool-Listen halten die EINMAL gebaute Maximal-Flotte — der
## Rhythmus-Tick (W18/J4) aktiviert/deaktiviert daraus per Skalen-Fade.
var _verkehr: Array[Dictionary] = []
var _verkehr_pool: Array[Dictionary] = []
var _prompt_ort := ""
var _toast: Node
var _licht_profil: Dictionary = {}
var _sfx_timer := 6.0
var _fussgaenger: Array[Dictionary] = []
var _fuss_pool: Array[Dictionary] = []
var _fuss_zeit := 0.0
var _near_miss_sperre := 0.0
var _ausparken: CityAusparken
## Rhythmus-Tick (W18/J4): Uhr nur alle TICK_S lesen, Stunde cachen.
var _rhythmus_rest := 0.0
var _stunde_cache := 12.0
...
func _ready() -> void:
	...
	_stunde_cache = _stunde()
	_licht_profil = CityAmbiente.licht_profil(_stunde_cache)
	_bau = CityBau.new(self, karte, _licht_profil, _stunde_cache)
	_bau.baue_licht()
	_bau.baue_boden()
	_bau.baue_strassen()
	_bau.baue_laternen()
...
## sonst kommt die Zeit aus der INJIZIERTEN GameState-Clock (AGENTS-Regel:
## der DEV-Zeit-Override pinnt genau diese Uhr, damit springt die ganze
## Stadt deterministisch mit). Ohne GameState: Systemzeit als Fallback.
func _stunde() -> float:
	if stunde_override >= 0.0:
		return stunde_override
	var gs := game_state()
	if gs != null and "clock" in gs and gs.clock != null:
		var bias := int(Time.get_time_zone_from_system().get("bias", 0))
		return CityRhythmus.stunde_von_ms(int(gs.clock.now_ms()), bias)
	var jetzt := Time.get_time_dict_from_system()
	return float(jetzt["hour"]) + float(jetzt["minute"]) / 60.0


## W18/4-B1: Öffnungsregel eines Orts JETZT — dieselbe INJIZIERTE Uhr wie
## _stunde() (GameState-Clock + Zeitzonen-Bias; der DEV-Zeit-Override pinnt
## genau diese Uhr, Flows/Tests springen deterministisch mit).
func ort_offen_jetzt(ort_id: String) -> bool:
	var bias := int(Time.get_time_zone_from_system().get("bias", 0))
	var lokal_s := int(floor(now_ms() / 1000.0)) + bias * 60
	return OrtKatalog.ist_offen(ort_id, lokal_s, karte)


## ------------------------------------------------- Tagesrhythmus (W18/J4)


## Alle TICK_S: Uhr lesen, Dichte-Ziele setzen (Autos/Goobys faden sanft
## rein/raus), Licht-Schicht umschalten (Sonne/Himmel/Laternen/Fenster/
## Schilder/Vögel) und Auto-Scheinwerfer nachführen. Zwischen den Ticks
## kostet der Rhythmus NICHTS (Low-End-Budget).
func _rhythmus_tick(delta: float) -> void:
	_rhythmus_rest -= delta
	if _rhythmus_rest > 0.0:
		return
	_rhythmus_rest = CityRhythmus.TICK_S
	_stunde_cache = _stunde()
	_setze_dichte_ziele(_stunde_cache)
	_bau.setze_licht_stunde(_stunde_cache)
	_licht_profil = _bau.licht_profil()
	var an: bool = _licht_profil["lichter_an"]
	if auto != null:
		auto.setze_licht(an)
	for eintrag in _verkehr_pool:
		var lichter: Node3D = eintrag.get("lichter")
		if lichter != null:
			lichter.visible = an


func _setze_dichte_ziele(stunde: float) -> void:
	var v_ziel := CityVerkehr.anzahl(stunde)
	var f_ziel := CityFussgaenger.anzahl(stunde)
	if leben_reduziert():
		v_ziel = maxi(1, v_ziel / 2)
		f_ziel = maxi(1, f_ziel / 2)
	_verkehr = _aktiviere(_verkehr_pool, v_ziel)
	_fussgaenger = _aktiviere(_fuss_pool, f_ziel)


## Die ersten `ziel` Pool-Einträge sind aktiv (deterministisch — dieselben
## Nachtschwärmer bleiben nachts übrig); Rückgabe: die aktive Teilliste.
func _aktiviere(pool: Array[Dictionary], ziel: int) -> Array[Dictionary]:
	var aktive: Array[Dictionary] = []
	for i in pool.size():
		pool[i]["aktiv"] = i < ziel
		if i < ziel:
			aktive.append(pool[i])
	return aktive


## Skalen-Fade eines Pool-Eintrags Richtung aktiv/inaktiv (UEBERGANG_S);
## Reduced Motion schaltet sofort. Rückgabe: aktueller Fade (0..1).
func _fade_schritt(eintrag: Dictionary, delta: float) -> float:
	var ziel := 1.0 if bool(eintrag.get("aktiv", true)) else 0.0
	var fade := float(eintrag.get("fade", 1.0))
	if fade == ziel:
		return fade
	if _reduced_motion():
		fade = ziel
	else:
		fade = move_toward(fade, ziel, delta / CityRhythmus.UEBERGANG_S)
	eintrag["fade"] = fade
	var node: Node3D = eintrag["node"]
	node.visible = fade > 0.001
	node.scale = Vector3.ONE * float(eintrag.get("basis_scale", 1.0)) * maxf(fade, 0.001)
	return fade
...
## W18/J4: gebaut wird EINMAL die Maximal-Flotte (Rhythmus-Kurvengipfel);
## aktiv ist nur das Tageszeit-Soll, der Rest wartet unsichtbar im Pool.
func _baue_verkehr() -> void:
	...
	var flotte := CityRhythmus.verkehr_max()
	var ziel := CityVerkehr.anzahl(_stunde_cache)
	if leben_reduziert():
		flotte = maxi(1, flotte / 2)
		ziel = maxi(1, ziel / 2)
	var licht_an: bool = _licht_profil["lichter_an"]
	for i in flotte:
			...
			. append(
				{
					...
					"aktiv": aktiv,
					"fade": 1.0 if aktiv else 0.0,
					"basis_scale": CityCarFeel.CAR_SCALE,
					"lichter": _bau.haenge_autolichter(wagen, licht_an),
				}
			)
		)
		if aktiv:
			_verkehr.append(_verkehr_pool[_verkehr_pool.size() - 1])
...
## an der Tageszeit (und halbiert sich im Spar-Modus). W18/J4: gebaut wird
## die Maximal-Belegschaft, aktiv ist das Tageszeit-Soll; der ERSTE Gooby
## ist der Zeitungs-Leser des Morgens (Prop nur in der Morgen-Pause).
func _baue_fussgaenger() -> void:
	var wurzel := Node3D.new()
	wurzel.name = "Fussgaenger"
	add_child(wurzel)
	var flotte := CityRhythmus.fussgaenger_max()
	var ziel := CityFussgaenger.anzahl(_stunde_cache)
	if leben_reduziert():
		flotte = maxi(1, flotte / 2)
		ziel = maxi(1, ziel / 2)
	var routen := CityFussgaenger.routen(karte, flotte, karte.deko_seed())
	var szene: PackedScene = load(GOOBY_GLB) if ResourceLoader.exists(GOOBY_GLB) else null
	if szene == null:
		return
	for i in routen.size():
		var route := routen[i]
		...
		route["anim"] = ""
		route["aktiv"] = i < ziel
		route["fade"] = 1.0 if i < ziel else 0.0
		route["basis_scale"] = FUSSGAENGER_SCALE
		node.visible = i < ziel
		if i == 0:
			route["zeitung"] = true
			route["zeitung_node"] = _baue_zeitung(node)
		_fuss_pool.append(route)
		if bool(route["aktiv"]):
			_fussgaenger.append(route)
	_update_fussgaenger(0.0)


## Aufgeschlagene Zeitung (zwei helle Quads im Knick, unshaded) — lokale
## Einheiten, der Gooby-Root ist bereits skaliert. Ein Quad-Paar, kein
## neues Material pro Frame (Low-End-Budget).
func _baue_zeitung(gooby: Node3D) -> Node3D:
	var zeitung := Node3D.new()
	zeitung.name = "Zeitung"
	zeitung.visible = false
	var mat := StandardMaterial3D.new()
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	mat.albedo_color = Color(0.96, 0.95, 0.88)
	mat.cull_mode = BaseMaterial3D.CULL_DISABLED
	for seite: float in [-1.0, 1.0]:
		var blatt := MeshInstance3D.new()
		var quad := QuadMesh.new()
		quad.size = Vector2(0.2, 0.16)
		quad.material = mat
		blatt.mesh = quad
		blatt.position = Vector3(seite * 0.09, 0.0, 0.0)
		blatt.rotation_degrees = Vector3(-18.0, seite * 24.0, 0.0)
		zeitung.add_child(blatt)
	zeitung.position = Vector3(0.0, 0.42, 0.24)
	gooby.add_child(zeitung)
	return zeitung
...
func _update_ambient_sfx(delta: float) -> void:
	...
	_spiele_ambient_sfx(CityAmbiente.sfx_wahl(_stunde_cache, randf()))
...
func _update_verkehr(delta: float) -> void:
	_ampel_zeit += delta
	var blinkt := CityVerkehr.ampel_blinkt(_stunde_cache)
	var ampeln: Dictionary = {} if blinkt else _bau.ampel_lookup
	for eintrag in _verkehr_pool:
		var fade := _fade_schritt(eintrag, delta)
		if fade <= 0.0 and not bool(eintrag["aktiv"]):
			continue
...
func _update_fussgaenger(delta: float) -> void:
	if _fuss_pool.is_empty():
		return
	_fuss_zeit += delta
	var zeitung_zeit := CityRhythmus.zeitungs_gooby_aktiv(_stunde_cache)
	for route in _fuss_pool:
		var fade := _fade_schritt(route, delta)
		if fade <= 0.0 and not bool(route["aktiv"]):
			_setze_zeitung(route, false)
			continue
		...
		var liest := zeitung_zeit and bool(route.get("zeitung", false)) and bool(zustand["steht"])
		_setze_zeitung(route, liest)
		var soll := "walk"
		if bool(zustand["steht"]):
			# Der Zeitungs-Leser steht ruhig ins Blatt vertieft (kein Winken).
			soll = "idle" if liest else ("wave" if bool(zustand["winkt"]) else "idle")
		_spiele_gooby_clip(route, soll)


func _setze_zeitung(route: Dictionary, sichtbar: bool) -> void:
	var zeitung: Node3D = route.get("zeitung_node")
	if zeitung != null and zeitung.visible != sichtbar:
		zeitung.visible = sichtbar
...
func _on_betreten(ort_id: String) -> void:
	...
	if str(eintrag.get("typ", "")) == "stub" or str(eintrag.get("szene", "")).is_empty():
		...
		return
	# W18/4-B1: Öffnungszeiten waren tote Daten — OrtKatalog.ist_offen hatte
	# null Aufrufer, der Wochenmarkt (Sa 8–14) war immer betretbar. Das Gate
	# prüft VOR Energie-Abzug/Route; der „geschlossen“-Text existiert bereits.
	if not ort_offen_jetzt(ort_id):
...
func _spawn_bei(spawn: String) -> void:
	...
	# W18/4-B12: Park-Pads ans Auto — im Prompt-Radius darf die gehaltene
	# Bremse GANZ anhalten (kein 1,2-m/s-Kriechen mehr aus dem Parkfeld).
	var pads: Array[Vector3] = [karte.parkplatz_welt("zuhause")]
	for eintrag: Dictionary in karte.orte():
		pads.append(karte.parkplatz_welt(str(eintrag.get("id", ""))))
	auto.park_pads = pads
