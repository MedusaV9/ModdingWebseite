...
## W18/4-B12: `stillstand_ok` (Auto steht auf einem Park-Pad) hebt das
## Kriech-Minimum auf — nur so kann ein gebremstes Auto WIRKLICH stehen
## (vorher kroch es mit 1,2 m/s in den Fassaden-Kollider und wurde vom
## Depenetrations-Punch vom Parkfeld geschleudert). Fahren/Bremsen abseits
## der Pads bleibt VERBATIM beim Web-Kontrakt (§C7.2).
static func step_speed(
	speed: float, target: float, braking: bool, dt: float, stillstand_ok := false
) -> float:
	if braking:
		var minimum := 0.0 if stillstand_ok else BRAKE_MIN_SPEED
		return maxf(minimum, speed - BRAKE_DECEL * dt)
