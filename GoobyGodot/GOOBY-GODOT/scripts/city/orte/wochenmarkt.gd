...
## Markt-Stände dezent Richtung Sand-Pastell tinten (Kenney-Farben bleiben
## erkennbar — Tint-Stärke bewusst niedrig).
const TINT_STAND := Color("#EFD9A8")
...
func _baue_innenraum() -> void:
	...
	# J3/CC0 (Integrations-Plan „Wochenmarkt“): zwei echte Marktstände
	# flankieren die Tafel, davor Kürbis/Melone als Boden-Auslage
	# (Ursprünge ragen unter y = 0 — Ablage kompensiert min-Y), dazu
	# Karren + Esel als Hingucker am Rand.
	OrtRequisiten.tinte(
		_prop("%s/stall_green.glb" % CC0_STADT, Vector3(-2.0, 0.0, -3.0), 15.0, 1.4),
		TINT_STAND,
		0.25
	)
	OrtRequisiten.tinte(
		_prop("%s/stall_red.glb" % CC0_STADT, Vector3(2.0, 0.0, -3.1), -15.0, 1.4), TINT_STAND, 0.25
	)
	OrtRequisiten.pastellisiere(
		_prop("%s/pumpkin_harvested.glb" % CC0_CROPS, Vector3(-3.1, 0.023, -3.0), 25.0, 0.55)
	)
	OrtRequisiten.pastellisiere(
		_prop("%s/watermelon_harvested.glb" % CC0_CROPS, Vector3(3.05, 0.103, -3.05), -30.0, 0.5)
	)
	_prop("%s/cart.glb" % CC0_STADT, Vector3(5.8, 0.0, -1.6), 195.0, 1.1)
	var esel := _prop("%s/donkey.glb" % CC0_TIERE, Vector3(5.0, 0.0, -2.6), 160.0, 0.26)
	OrtRequisiten.pastellisiere(esel)
	OrtRequisiten.spiele_idle(esel)
	_baue_auslage()
	_baue_eigenstand()
...
## J3: Ambient-Leben — 4 Marktbummler ziehen über den Platz (das
## Marktgemurmel passt hier wörtlich); die Stand-Kunden am Eigenstand
## bleiben davon unberührt (eigene Choreo, s. _richte_kunden_ein).
