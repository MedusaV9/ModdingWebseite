...
## Roher Footprint (Breite, Tiefe) der Ecke-Ursprung-Sitzbank (GLB-AABB).
const GRUND_BANK := Vector2(0.4, 0.2)
## Wartehallen-Bänke in Terminal-Blau (passend zum Schalter-Gooby).
const TINT_BANK := Color("#9BB7E8")
...
## Mitbringsel: heimkehr-Domain mit lokalisiertem Zielnamen — W19).
static func gfree_name(ware: Dictionary) -> String:
	var id := str(ware.get("id", ""))
	var typ := str(ware.get("typ", ""))
	if typ == "moebel":
	...
	if typ == "mitbringsel":
		return HeimkehrLogik.mitbringsel_name(
			str(ware.get("mitbringsel_typ", "")), str(ware.get("ziel_id", ""))
		)
...
func _baue_innenraum() -> void:
	...
	# J3/CC0: Wartehallen-Bankreihe (drei Sitzbänke, Blick auf die
	# Abflug-Tafeln) — die Rollkoffer bringen die Besucher selbst mit.
	for x: float in [-3.9, -3.05, -2.2]:
		var bank := _cc0("%s/bench.glb" % CC0_MOEBEL, Vector3(x, 0.0, 0.9), 0.0, 2.0, GRUND_BANK)
		OrtRequisiten.tinte(bank, TINT_BANK, 0.4)


## J3: Ambient-Leben — 4 Reisende mit Rollkoffern kreuzen die Wartehalle
## (Hallen-Gemurmel statt Tür-Glöckchen), der Schalter-Gooby tippt an der
## Kasse; Frau Fernweh (Stammkundin, 11–16 Uhr) schaut nur nach den Tafeln.
...
func _baue_gfree_hinweis() -> void:
	...
	# Helle Kontur (Muster minigame_host-Countdown): die Caption schwebt
	# über dem 3D-Bild — Gooby/heller Boden laufen sonst durch den Text.
	_gfree_hinweis.add_theme_color_override("font_outline_color", Color(1.0, 0.98, 0.92, 0.9))
...
func _gfree_status_aktualisieren() -> void:
	...
	# W19/MITBRINGSEL: das 24-h-Heimkehr-Fenster öffnet den Stand AUCH ohne
	# Abflug-Buchung — der dezente Hinweis zeigt Ziel + Rest-Stunden.
	var fenster := gs != null and HeimkehrLogik.fenster_offen(gs.state(), _now_ms())
	_gfree_btn.disabled = not (offen or fenster)
	_gfree_btn.tooltip_text = "" if offen or fenster else I18nService.t("gfree.zu")
	if _gfree_hinweis == null or not is_instance_valid(_gfree_hinweis):
		return
	_gfree_hinweis.visible = not offen
	if fenster and not offen:
		_gfree_hinweis.text = _fenster_hinweis_text(gs)
	elif not offen:
		_gfree_hinweis.text = I18nService.t("gfree.zu")


## Text des Mitbringsel-Fenster-Hinweises (heimkehr-Domain, W19).
func _fenster_hinweis_text(gs: Object) -> String:
	var v := Vacation.slice_of(gs.state())
	return (
		I18nService
		. t(
			"heimkehr.gfree.hinweis",
			{
				"ziel": I18nService.t("travel.ziel.%s" % str(v["heimkehrZiel"])),
				"stunden": HeimkehrLogik.fenster_rest_h(gs.state(), _now_ms()),
			}
		)
	)
...
func _positioniere_gfree_hinweis() -> void:
	...
	_gfree_hinweis.add_theme_constant_override("outline_size", int(roundf(6.0 * f)))
...
func _on_gooby_free() -> void:
	...
	if gs == null:
		return
	# W19: das Heimkehr-Fenster öffnet den Stand auch ohne Abflug-Buchung.
	if not gooby_free_offen(gs.state()) and not HeimkehrLogik.fenster_offen(gs.state(), _now_ms()):
...
func _gfree_aktualisieren() -> void:
	...
	# W19: exklusive Heimkehr-Angebote ZUERST (nur im 24-h-Fenster da);
	# das Standard-Sortiment bleibt an die Abflug-Buchung gebunden.
	if gs != null:
		for ware: Dictionary in HeimkehrLogik.fenster_sortiment(gs.state(), _now_ms()):
			_gfree_liste.add_child(_gfree_zeile(ware, coins))
	if gs != null and gooby_free_offen(gs.state()):
...
func _gfree_zeile(ware: Dictionary, coins: int) -> Control:
	...
	elif str(ware.get("typ", "")) == "mitbringsel":
		# W19: Fenster-Ware trägt den „Nur 24 h“-Chip statt „Exklusiv“.
		var fenster_chip := Label.new()
		fenster_chip.theme_type_variation = "CaptionLabel"
		fenster_chip.text = I18nService.t("heimkehr.gfree.exklusiv")
		zeile.add_child(fenster_chip)
	var btn := Button.new()
...
func _on_gfree_kauf(ware: Dictionary) -> void:
	# W19: Fenster-Waren kauft die pure HeimkehrLogik (inventory.items,
	# zeitinjiziert); Möbel/Snacks laufen unverändert über kaufe_gfree.
	var ergebnis := (
		HeimkehrLogik.kaufe(game_state(), ware, _now_ms())
		if str(ware.get("typ", "")) == "mitbringsel"
		else kaufe_gfree(game_state(), ware)
	)
	match ergebnis:
		KAUF_OK:
			AudioDirector.try_play(self, "ui_click")
			# J3: GOOBY-FREE-Kauf piept am Schalter (Kassen-Verhalten).
			_on_leben_kunde_zahlt(str(ware.get("id", "")))
