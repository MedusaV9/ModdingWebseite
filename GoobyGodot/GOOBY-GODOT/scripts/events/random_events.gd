...
## W18/E3b: Aktives Event an den Raum pinnen, in dem seine Szene zuerst
## aufgebaut wurde — der Raumwechsel räumt die Inszenierung ab (der Runner
## stirbt mit dem Raum), und NUR der Rückbesuch dieses Raums baut sie
## wieder auf; fremde Räume lassen das Event aktiv liegen (Timeout/Fail
## übernimmt wie bisher). Ohne aktives Event ist der Aufruf ein No-Op.
static func pin_room(gs: Object, room_id: String) -> void:
	gs.update(
		func(state: Dictionary) -> void:
			var slice: Dictionary = _ensure_slice(state)
			var active: Dictionary = slice.get("active", {})
...
## Gepinnter Raum des aktiven Events ("" = ungepinnt oder kein Event).
static func pinned_room_of(gs: Object) -> String:
	return str(active_of(gs).get("room", ""))
