...
struct QuizGameView: View {
        ...
        if currentRound > 0, currentRound <= totalRounds {
            ...
            if let result = verdict(round: previous), let guesser = guesserId(round: previous) {
                PillTag(text: result == "right"
                        ...
                        tint: result == "right" ? Licht.lampengold : Licht.glut)
            }
        }
    }
    ...
    /// Answer line ON the paper card: an inner writing well (matte fill +
    /// kante hairline — DreamyFieldStyle is night-tuned) and a send stamp
    /// in the couple ink (letter-paper glyph on it reads ≥4.5:1 by the
    /// inkOnPaper construction).
    private func answerInput(round: Int) -> some View {
        HStack(spacing: LayoutMetrics.s(10)) {
            ...
            // VoiceOver would otherwise read the paperplane glyph name
            // (re-eval 2, Befund 6).
            .accessibilityLabel(L10n.t("games.a11y.sendAnswer"))
        }
    }
    ...
    private func revealSection(round: Int, subject: String,
        ...
        VStack(spacing: LayoutMetrics.s(12)) {
            // Truth wears the couple ink, the guess plain secondary ink —
            // both readable washes on the paper card.
