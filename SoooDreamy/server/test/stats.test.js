...
test('widget snapshot: query token auth, and nulls/zeros for a fresh single-member couple', async (t) => {
  const { baseUrl } = await makeApp(t);
  ...
  const res = await anon.get(`/api/widget-snapshot?token=${encodeURIComponent(created.body.token)}`);
  assert.equal(res.status, 200);
  const snap = res.body;
