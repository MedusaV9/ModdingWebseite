...
test('inbox aggregates everything created strictly after since', async (t) => {
  ...
  const lastTouch = (await b.api.post('/api/touches', { json: { type: 'kiss' } })).body.touch;
  const photo = (
  ...
  assert.equal(inbox.touches.count, 2);
  assert.deepEqual(inbox.touches.last, lastTouch);
  ...
  // From a's perspective: partner (b) has NOT answered; the coupon a created is not "for me".
  ...
  assert.equal(aInbox.messages.count, 2); // messages are couple-wide, senderId disambiguates
