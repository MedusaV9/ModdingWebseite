...
const GRID_A = '🟩🟩🟨⬛⬛\n🟩🟩🟩🟩🟩';
const GRID_B = '⬛⬛⬛⬛⬛\n🟨🟨⬛⬛⬛\n🟩🟩🟩🟩🟩';
const LEGACY_TOKEN_A = 'legacy_wordle_token_a_123456';
const LEGACY_TOKEN_B = 'legacy_wordle_token_b_123456';

/** Boots an app on a crafted store.json with pre-v4 plaintext bearer records. */
async function makeSeededApp(t, coupleExtras) {
  ...
  const store = {
    ...
    tokens: {
      [LEGACY_TOKEN_A]: { coupleId: 'c_seed', memberId: 'm_a' },
      [LEGACY_TOKEN_B]: { coupleId: 'c_seed', memberId: 'm_b' },
    },
  };
  ...
  return {
    baseUrl,
    a: client(baseUrl, LEGACY_TOKEN_A),
    b: client(baseUrl, LEGACY_TOKEN_B),
  };
}
