...
test('GET /api/daily lists answered days newest-first with per-member reveal semantics', async (t) => {
  ...
  // Two days ago: both answered. Yesterday: only A. Today: only B.
  await a.api.post(`/api/daily/${twoDaysAgo}`, { json: { questionId: 1, text: 'A day1' } });
  await b.api.post(`/api/daily/${twoDaysAgo}`, { json: { questionId: 1, text: 'B day1' } });
  ...
  assert.deepEqual(
    aView[0],
    { dateKey: today, questionId: 3, myAnswer: null, partnerAnswer: null, bothAnswered: false, streak: 0 },
  );
...
test('GET /api/daily?limit trims to the newest days; bad limit → 400', async (t) => {
  ...
  for (let days = 0; days < 4; days++) {
    await a.api.post(`/api/daily/${dateKeyDaysAgo(days)}`, { json: { questionId: days, text: `entry ${days}` } });
  }
