...
var _hint_plate := StyleBoxFlat.new()  ## W21: Milchglas-Plate (HintFeel, s. dort).
var _ende_timer := 0.0
...
func _process(delta: float) -> void:
	...
	_step_sim(delta)
	_step_optik(delta)
	HintFeel.fade_hint(_hud, _hint_label, t_abs)
...
func _schaf_drin(drin: int) -> void:
	...
	# W21/P5 Feier-Beat: „Schaf im Pferch!" ist DER Kern-Moment — Gold-Pille
	# + Sparkle (MotionKit/RM-gated) statt kleinem float_text am Tor.
	var text := I18nService.t("mg.hudkit.beat_schaf", {"n": drin, "max": schafe.size()})
	MgHudKit.feier_beat(self, view_size, _ui(), text)
	if ctx.juice != null:
...
func _build_hud() -> void:
	...
	HintFeel.install(_hud, _hint_label, _hint_plate, func() -> float: return t_abs)
	# W21/P5: Zeit/Drin als Kit-Chip — Frost-Pille statt nackter Labels auf
	# Gras/Himmel; der Maler hängt am HUD-Draw (Ranch-Muster wie HintFeel).
	MgHudKit.install_chip(_hud, _zeit_label, _drin_label, _ui)
	_layout_hud()
	_update_labels()
...
func _layout_hud() -> void:
	if _zeit_label == null:
		return
	var ui := _ui()
	MgHudKit.style_chip(_zeit_label, _drin_label, ui)
	MgHudKit.layout_chip(_zeit_label, _drin_label, ui)
	_hint_label.add_theme_font_size_override("font_size", MgHudKit.font_px(15.0, ui))
	var s := HintFeel.clamp_size(_hint_label, minf(view_size.x - 24.0 * ui, 360.0 * ui))
	_hint_label.position = Vector2((view_size.x - s.x) * 0.5, view_size.y - s.y - 10.0 * ui)
