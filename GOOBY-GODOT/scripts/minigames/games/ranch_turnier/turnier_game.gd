...
func _baue_menu() -> Control:
	...
	# W18/4: Fuß AUSSERHALB der Scroll-Spalte (comp_level_select-Muster) —
	# „Fertig“ bleibt unten gepinnt sichtbar, statt hochkant mit Liga-Panel
	# + Disziplinen-Grid unter die Falz zu rutschen; nur der Inhalt scrollt.
	var aussen := VBoxContainer.new()
	aussen.add_theme_constant_override("separation", 10)
	margin.add_child(aussen)
	var scroll := ScrollContainer.new()
	scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
	scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
	aussen.add_child(scroll)
