...
## W21/P4 (d) „Pregame behält sein Gesicht quer“: im Quer-Canvas wird die
## Karte zur ZWEI-Spalten-Karte (Cover + Gooby-Badge links, Zeilen rechts)
## und nutzt den Wallpaper-Leerraum — Cover/Badge bleiben SICHTBAR statt
## wie früher zuerst wegzuschrumpfen.
const CARD_QUER_WIDTH := 780.0
## Cover-Spaltenbreite quer (Design-px, ×f).
const COVER_QUER_W := 240.0
...
## W21/P4 (d): Spalten-Container der Karte — hochkant untereinander (Cover
## oben), quer nebeneinander (Cover links, Zeilen rechts).
var _spalten: BoxContainer
...
## Spalten-Basis der Pregame-Karte je Orientierung (Vertrag für den
## FB3-Audit-Breiten-Deckel, Muster PhoneShell.basis_groesse): quer die
## breite Zwei-Spalten-Karte, hochkant die klassische Karte.
static func karten_basis(m: Dictionary) -> float:
	var canvas: Vector2 = m["canvas"]
	return CARD_QUER_WIDTH if canvas.x > canvas.y else CARD_BASE_WIDTH
...
func _build_ui() -> void:
	...
	# W20/P4 Inhaltsspalte: die Karte IST die Spalte dieses Screens —
	# gedeckelt über card_width (_apply_touch_floor) und im Safe-Rechteck
	# zentriert (CenterContainer). Das Meta-Flag macht den Vertrag für die
	# FB3-Wachen sichtbar (content_mitte misst Deckel+Zentrierung live;
	# die Stretch-Wache wertet das randnahe Hochkant-Cover sonst als
	# Vollbreite-Balken ohne Spalte).
	_card.set_meta(ScreenShell.META_CONTENT_COLUMN, true)
	_center.add_child(_card)
	# W21/P4 (d): BoxContainer statt fixer VBox — _apply_touch_floor kippt
	# ihn quer auf horizontal (Cover-Spalte links, Zeilen rechts), damit
	# Cover + Gooby-Badge das Gesicht des Screens BLEIBEN statt als erste
	# Deko der Höhe zu weichen.
	_spalten = BoxContainer.new()
	_spalten.vertical = true
	_spalten.add_theme_constant_override("separation", 12)
	_card.add_child(_spalten)
	...
	# Inhaltsspalte (Titel bis Knöpfe) — quer die RECHTE Spalte neben dem
	# Cover, hochkant wie gehabt unter dem Cover.
	var rows := VBoxContainer.new()
	rows.add_theme_constant_override("separation", 12)
	rows.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_spalten.add_child(rows)
	...
	# W19/SPOTLIGHT: ist DIESES Spiel heute das „Spiel des Tages“, steht
	# der +50-%-Bonus sichtbar VOR dem Start — nach dem Einlösen ehrlich
	# die Eingelöst-Zeile statt eines falschen Versprechens.
	_add_spotlight_line(rows)
...
func _apply_touch_floor() -> void:
	...
	# W21/P4 (d): quer wird die Karte zur breiten ZWEI-Spalten-Karte
	# (karten_basis) und nutzt den Wallpaper-Leerraum — Cover + Gooby
	# behalten ihren Platz, statt als erste Deko der Höhe zu weichen.
	var canvas: Vector2 = m["canvas"]
	var quer := canvas.x > canvas.y
	_card.custom_minimum_size = Vector2(ScreenShell.card_width(m, karten_basis(m)), 0)
	_spalten.vertical = not quer
	...
	if quer:
		# Cover-SPALTE: feste Breite, volle Kartenhöhe — kostet keine Höhe
		# und bleibt darum IMMER sichtbar (das „Gesicht“ des Screens).
		_cover.custom_minimum_size = Vector2(COVER_QUER_W * float(m["f"]), 0.0)
		_cover.size_flags_vertical = Control.SIZE_EXPAND_FILL
		_cover.visible = true
	else:
		# Hochkant wie gehabt: Cover ist Deko, skaliert mit und weicht
		# ZUERST, wenn die Karte höher würde als der sichere Bereich —
		# Chips/Knöpfe behalten ihre Tippfläche, nichts läuft aus dem Canvas.
		_cover.size_flags_vertical = Control.SIZE_FILL
	...
	# Quer sitzt er VOLL im Kartenrand (die Spalte endet an der Kartenkante).
	if _gooby != null:
		...
		_gooby.offset_top = -d if quer else -d * 0.62
		_gooby.offset_bottom = 0.0 if quer else d * 0.38
	# Überlauf-Fit (G3): reicht der Platz trotzdem nicht (Endlos-Chip +
...
## W19/SPOTLIGHT: Spiel-des-Tages-Zeile — Tag kommt aus der INJIZIERTEN
## GameState-Uhr (Clock-Muster wie _add_modifier_banner). Ohne GameState
## (Tests ohne State) erscheint keine Zeile.
func _add_spotlight_line(rows: VBoxContainer) -> void:
	var gs := _resolve_state()
	if gs == null or gs.get("clock") == null:
		return
	var today: String = gs.clock.local_day()
	if not ArcadeSpotlight.ist_spotlight(game_id, today):
		return
	var line := Label.new()
	line.name = "SpotlightLine"
	line.theme_type_variation = &"CaptionLabel"
	line.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	line.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	line.add_theme_color_override("font_color", Color(0.93, 0.61, 0.15))
	if ArcadeSpotlight.bonus_verfuegbar(gs.state(), today):
		line.text = I18nService.t(
			"mg.spotlight.pregame.aktiv", {"prozent": ArcadeSpotlight.BONUS_PROZENT}
		)
	else:
		line.text = I18nService.t("mg.spotlight.pregame.eingeloest")
	rows.add_child(line)
