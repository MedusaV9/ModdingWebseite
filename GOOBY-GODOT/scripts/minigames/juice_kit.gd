class_name JuiceKit  # gdlint: ignore=max-public-methods
extends Node
## (Lint-Ausnahme W18: clear_overlays ist die 21. öffentliche Methode — ein
## Klassen-Split der gewachsenen Juice-API wäre ein eigener Umbau, kein Fix.)
...
## W18 B2: laufender Combo-Ausblende-Tween — clear_overlays() killt ihn,
## damit sein queue_free-Callback nie auf ein schon freigegebenes Label trifft.
var _combo_out_tween: Tween
...
func count_to(label: Label, from: int, to: int, dur := 0.8, prefix := "", suffix := "") -> void:
	...
	tween.tween_method(_count_step.bind(label_ref, from, to, prefix, suffix, state), 0.0, 1.0, dur)
	tween.tween_callback(_count_done.bind(label_ref, to, prefix, suffix))
...
## W18 B2 (Playtest Welle H): räumt ALLE Overlay-Reste deterministisch ab —
## Combo-„×N“-Label, Edge-Glow, Float-Texte, Flashes, Konfetti-/Münz-Partikel,
## Ringe. Der Host ruft das bei Rundenende/Neustart/Beenden, damit kein
## Overlay-Zombie Results, Pause-Modal oder die Folgerunde überlagert.
## Synchron (remove_child + deferred queue_free), damit Wächter direkt nach
## dem Aufruf einen leeren Layer sehen; der Combo-Ausblende-Tween wird
## mitgekillt (kein Callback auf ein schon freigegebenes Label).
func clear_overlays() -> void:
	if _combo_out_tween != null and _combo_out_tween.is_valid():
		_combo_out_tween.kill()
	_combo_out_tween = null
	_combo_label = null
	_edge_glow_rect = null
	_edge_glow_strength = 0.0
	var parent := _overlay_parent()
	if parent == null:
		return
	for child in parent.get_children():
		# Getrackte Pop-Tweens des Kindes mitkillen — sonst warnt die Engine
		# („Target object freed before starting“), wenn ein frisch gestarteter
		# scale_pop sein Ziel verliert.
		var id := child.get_instance_id()
		var pop: Variant = _pop_tweens.get(id)
		if pop is Tween and (pop as Tween).is_valid():
			(pop as Tween).kill()
...
func _count_step(
	...
	var label := label_ref.get_ref() as Label
	if label == null or not is_instance_valid(label):
		return
...
func _count_done(label_ref: WeakRef, to: int, prefix: String, suffix: String) -> void:
	var label := label_ref.get_ref() as Label
