...
static func award(
	...
	# W19/SPOTLIGHT: „Spiel des Tages“ — +50 % auf die TATSÄCHLICH gezahlte
	# Auszahlung, genau einmal pro Lokaltag (Auswahl + Anspruch prüft/
	# verankert ArcadeSpotlight, Marker minigames.spotlightBonusDay).
	# Ungedeckelter Reason: durch Einmal-pro-Tag + paid-Basis (selbst schon
	# 150-c-/Endlos-gedeckelt) hart begrenzt. paid == 0 verbraucht nichts.
	var spotlight_bonus := ArcadeSpotlight.beanspruche_bonus(state, id, paid, today)
	if spotlight_bonus > 0:
		spotlight_bonus = Economy.award(econ, spotlight_bonus, "spotlight", today)
	...
	return {
		...
		# W19/SPOTLIGHT: Spiel-des-Tages-Bonus für die Results-Zeile.
		"spotlightBonusCoins": spotlight_bonus,
	}
