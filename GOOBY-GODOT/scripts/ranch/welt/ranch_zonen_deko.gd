...
func baue(wurzel: Node3D) -> Dictionary:
	var gruppen := {}
	for schritt: Dictionary in schritte(wurzel):
		var gruppe: Variant = (schritt["bau"] as Callable).call()
		if not str(schritt["zone"]).is_empty():
			gruppen[schritt["zone"]] = gruppe
	return gruppen


## W19-Perf: der Zonen-Bau als einzelne Schritte {zone, bau} — die
## Region-Szene baut die SPAWN-Zone vor dem Reveal und streamt den Rest
## über das Frame-Budget nach. Jede Zone würfelt mit eigenem RNG-Salz,
## die Reihenfolge ändert also keine Positionen (Determinismus).
## zone = "" markiert globale Schritte (Bäume/Gras) ohne Gruppen-Eintrag.
func schritte(wurzel: Node3D) -> Array[Dictionary]:
	_bau = RanchBau.new(wurzel)
	return [
		{"zone": "hof", "bau": _baue_hof.bind(wurzel)},
		{"zone": "waeldchen", "bau": _baue_waeldchen.bind(wurzel)},
		{"zone": "see", "bau": _baue_see.bind(wurzel)},
		{"zone": "huegelkamm", "bau": _baue_huegelkamm.bind(wurzel)},
		{"zone": "bachlauf", "bau": _baue_bachlauf.bind(wurzel)},
		{"zone": "scheune_alt", "bau": _baue_scheune_alt.bind(wurzel)},
		{"zone": "turnierplatz", "bau": _baue_turnierplatz.bind(wurzel)},
		{"zone": "hufingen", "bau": _baue_hufingen.bind(wurzel)},
		{"zone": "weidetal", "bau": _baue_weidetal.bind(wurzel)},
		{"zone": "", "bau": _baue_baeume.bind(wurzel)},
		{"zone": "", "bau": _baue_gras.bind(wurzel)},
	]
...
## W19-Perf: das Gras-Wind-Material früh erzeugen — der Wetter-Controller
## braucht die Referenz VOR dem gestreamten Gras-Bau; die später gebauten
## Instanzen teilen sich dasselbe Material-Objekt.
func erzeuge_gras_material() -> ShaderMaterial:
	if gras_material != null:
		return gras_material
