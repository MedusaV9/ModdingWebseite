...
func _babble(text: String, emotion: String, melodie: String, token: int) -> void:
	if _streams.is_empty():
		# Leer-Abbruch ersetzt auch ein ggf. UNTERBROCHENES Gebrabbel (sagt
		# bricht den alten Lauf ab): _talking und die Duck-Meldung müssen
		# mit, sonst bleibt das Klangbett auf einer stummen Quelle geduckt.
		_talking = false
		Klangbett.melde_gebrabbel(self, false)
		fertig.emit()
		return
	...
	if plan.is_empty():
		...
		return
	_talking = true
	# W18/J5 (Audio-Hook): das Orts-Klangbett duckt, solange Gooby brabbelt.
	Klangbett.melde_gebrabbel(self, true)
