...
## W18/J1 Morgen-Ritual: das Blatt GLEITET federnd rein (UiMotion-Grammatik,
## reduced-motion-gated dort). Headless bleibt animationsfrei — Unit-Tests
## messen sofort stabile Geometrie. Nach dem Tween einmal _relayout, falls
## eine Autowrap-Nachmessung die Karte währenddessen neu zentriert hat.
func _gleite_rein() -> void:
	if DisplayServer.get_name() == "headless":
		return
	var tween := UiMotion.slide_up_in(_card)
	if tween != null:
		tween.finished.connect(_relayout)
