...
func open_panel() -> void:
	...
	# W18/B1: _panel UNABHÄNGIG vom Sheet auf Gültigkeit prüfen — vorher
	# hing nach einem extern zerstörten Panel für immer eine tote Referenz
	# fest (das Blatt blieb ab dann bei jedem Öffnen leer).
...
func _refresh_panel() -> void:
	...
	# W20 B3 („Neu würfeln unter der Falz“): Reroll-Zeile als gepinnter
	# Blatt-Fuß — in jedem Leitformat ohne Scrollen erreichbar.
	if _sheet != null and is_instance_valid(_sheet):
		_sheet.add_footer(_panel.footer_control())
...
func _on_panel_claim(id: String) -> void:
	# W18/J6: Quelle des Münzflugs optional mitmelden — der Flug startet am
	# getappten Abholen-Knopf statt in der Bildmitte (RewardFlug hört die
	# Gutschrift selbst über GameState.coins_changed/xp_changed).
	if _panel != null and is_instance_valid(_panel):
		RewardFlug.melde_quelle(_panel.claim_anker(id))
	var result := claim(id)
	if not bool(result["ok"]):
		_refresh_panel()
		return
	...
	var panel_da := _panel != null and is_instance_valid(_panel)
	if panel_da:
		_panel.mark_claimed(id, _bonus_info())
	var bonus: Dictionary = result.get("bonus", {})
	if not bonus.is_empty() and panel_da:
