...
func _build_viewport() -> void:
	...
	# W20/P3 (Befund B10 „Vorschau überbelichtet"): Sonne 1.1 + Füll 0.45 +
	# Ambient 0.9 OHNE Tonemapper brannten helle Wände zu reinem Weiß aus —
	# Tapeten-Muster unsichtbar. Der echte Raum (room_base/home_licht) lebt
	# bei ~0.5/0.35/0.52 MIT Filmic-Rolloff; die Vorschau zieht nach (etwas
	# heller als der Wohnraum, damit Muster/Farben freundlich lesbar sind).
	var sonne := DirectionalLight3D.new()
	sonne.light_energy = 0.7
