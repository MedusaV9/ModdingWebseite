...
## W18/4-B9: ohne Home-Gooby (Urlaub) KEIN Runner — sonst grüßte die Seele
## als Geister-Stimme aus dem leeren Raum (room.say-Blase ohne Sprecher).
...
## W18/J1: mit Overlay-Dirigent im Baum reiht sich die Karte dort ein
## (nie gleichzeitig mit Tagesbonus/Coachmark); ohne Dirigent wie bisher.
func _open_birthday_panel() -> void:
	if not room.has_method("ui_layer"):
		return
	var dirigent := OverlayDirigent.find(self)
	if dirigent == null:
		_zeige_geburtstags_panel()
		return
	dirigent.anfordern("geburtstag", OverlayDirigent.PRIO_GEBURTSTAG, _zeige_geburtstags_panel)


## Öffnen-Pfad der Geburtstags-Karte (ruft der Dirigent an ihrer Reihe;
## null, wenn der Raum inzwischen gewechselt wurde — Ticket verfällt).
func _zeige_geburtstags_panel() -> Control:
	if room == null or not is_instance_valid(room) or not room.has_method("ui_layer"):
		return null
...
func _busy() -> bool:
	...
	if room.has_method("is_build_mode_active") and room.is_build_mode_active():
		return true
	# W21 (Blocker #2 „Event-Kaperung“): während einer Event-Inszenierung
	# darf das Idle-Leben Gooby nicht bewegen/umposen — walk_to läuft an
	# set_wander_enabled(false) vorbei (_scripted), der Playtest fand ihn
	# deshalb mitten im Nutella-Event hinters Sofa getragen. Streichel-
	# Taps (handle_tap) bleiben bewusst ungebremst.
	var events := room.get_node_or_null("EventRunner")
	if events != null and events.has_method("is_running") and events.is_running():
		return true
	return false
