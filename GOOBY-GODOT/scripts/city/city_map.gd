...
## Parkplatz-Trigger-Position eines Orts: STRASSENSEITIGE Ort-Tile-Mitte, um
## trigger_offset_m Richtung Straßen-Tile geschoben (Auto parkt am Bordstein
## vorm Laden). W18/4-B7: nicht mehr blind tiles[0] — beim Flughafen grenzt
## tiles[0]=[0,2] NICHT an strasse=[0,4], das Pad landete im Gebäude-Collider
## und die Kollisionsauflösung schob das Auto vom Trigger. `trigger_offset_m`
## liegt mit 9,5 m außerhalb der 7,5-m-Ort-Collider (city_bau) + Auto-Radius.
func parkplatz_welt(ort_id: String) -> Vector3:
	...
	if not eintrag.is_empty():
		...
		tile = park_tile(eintrag.get("tiles", [[0, 0]]), strasse)
...
## Ort-Tile, vor dem das Parkplatz-Pad liegt: das Tile mit dem KLEINSTEN
## Manhattan-Abstand zum Straßen-Tile (der echte Straßen-Nachbar hat 1;
## Fallback bei Datenfehlern bleibt deterministisch das erste Tile).
static func park_tile(tiles_raw: Array, strasse: Vector2i) -> Vector2i:
	var best := Vector2i.ZERO
	var best_d := 2147483647
	for raw: Variant in tiles_raw:
		var tile := _tile_von(raw)
		var d := absi(tile.x - strasse.x) + absi(tile.y - strasse.y)
		if d < best_d:
			best_d = d
			best = tile
	return best
