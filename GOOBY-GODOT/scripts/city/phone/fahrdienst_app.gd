...
func aktualisiere() -> void:
	for kind in get_children():
		kind.queue_free()
	# W20/P3 (Befund B7 „Bestell-CTA unter der Falz“): gepinnte CTAs der
	# letzten Ansicht räumen — die neuen landen wieder im Shell-Fuß.
...
## W20/P3 (Befund B7): Haupt-CTA in den gepinnten Shell-Fuß (W18/4-B2-
## Muster) — ohne Shell (Standalone/Tests) bleibt er im eigenen Fluss.
func _pinne_cta(btn: Control) -> void:
	if PhoneShell.pinne_in_fuss(self, btn):
		PhoneShell.app_fonts_skalieren(btn)
	else:
		add_child(btn)
