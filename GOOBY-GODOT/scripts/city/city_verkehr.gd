...
## Eckwerte der CityRhythmus-Kurve: mittags voll, nachts Nachtschwärmer.
const TAG_AUTOS := 9
const NACHT_AUTOS := 3
...
## Verkehrsmenge zur Stunde — W18/J4: folgt der CityRhythmus-Dichtekurve
## (morgens ruhig, mittags voll, abends golden, nachts Nachtschwärmer).
static func anzahl(stunde: float) -> int:
	return CityRhythmus.verkehr_anzahl(stunde)
