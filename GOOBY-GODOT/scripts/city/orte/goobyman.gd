...
## CC0-Drogerie-Regale hinter dem Verkäufer und eine Nachschub-Ecke.
...
const GRUND_KARTON := Vector2(0.372, 0.212)

## Drogerie-Palette: Regale in Heldenrosa (passend zum Schriftzug).
const TINT_REGAL := Color("#F2C4D9")
...
func _baue_innenraum() -> void:
	...
	# J3/CC0: Drogerie-Regal-Paar hinter dem Verkäufer + offener
	# Nachschub-Karton bei den Kisten (Warenannahme).
	for x: float in [1.0, 1.75]:
		var regal := _cc0(
			"%s/bookcase_closed.glb" % CC0_MOEBEL, Vector3(x, 0.0, -3.6), 0.0, 1.7, GRUND_REGAL
		)
		OrtRequisiten.tinte(regal, TINT_REGAL, 0.45)
	_cc0(
		"%s/cardboard_box_open.glb" % CC0_MOEBEL, Vector3(4.5, 0.0, -0.7), -16.0, 1.9, GRUND_KARTON
	)
	_baue_schriftzug()


## J3: Ambient-Leben — 3 Kunden browsen die Regale, der Verkäufer bekommt
## das Kassen-Verhalten (GoobymanSheet-Käufe piepen via _on_gekauft).
...
func _on_gekauft(ware_id: String) -> void:
	# J3: Kassen-Piep + Winken über das Kassen-Verhalten; während des
	# Umhang-Gags bleibt die Figur ungestört in ihrer Helden-Pose.
	if _umhang_laeuft:
		return
	if kassen_npc != null:
