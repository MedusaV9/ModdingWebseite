...
## Ehrlicher Offline-Fallback: Releases erhalten ihren öffentlichen WSS-Host
## über configure_release_endpoint.py; ein Loopback-Default wäre auf Geräten
## stets der Spieler selbst und damit eine falsche „Online“-Completion.
const DEFAULT_NET := {"host": "", "port": 443, "tls": true}
...
func _ready() -> void:
	...
	if build_services:
		...
		# W18/B5: Mail-Service EAGER anhängen (Muster Outbox/Redeem) — ein
		# MAIL_NEW-Push VOR dem ersten lazy UI-attach (Post-Schalter/Telefon)
		# verpuffte sonst ungehört und die Ungelesen-Kapsel blieb auf 0.
		# attach() ist idempotent: die UI bekommt später DIESELBE Instanz.
		NetMail.attach(self)
	if auto_connect:
		connect_now()
...
func connect_now() -> void:
	...
	var host := str(net_config.get("host", "")).strip_edges()
	# Kein Release-/User-Endpunkt konfiguriert: ehrlich offline bleiben, ohne
	# eine ungültige URL oder eine endlose Reconnect-Schleife zu erzeugen.
	if host.is_empty():
