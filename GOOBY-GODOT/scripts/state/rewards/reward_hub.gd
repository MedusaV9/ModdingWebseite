...
##
## W21/ACNH P1 (Erstlauf-Serialisierung): Feiern warten zusätzlich, bis der
## Overlay-Dirigent seine Willkommens-Sequenz beendet hat — Sticker-Toast +
## Jubel-Blase kommen NACH Tagesbonus/Coachmark/Geburtstag statt darüber
## (max. 1 Overlay + 1 Toast gleichzeitig; Wächter test_w21_erstlauf_serie).
...
## W18/J1 Morgen-Ritual: so lange gehört die Bühne zuerst dem Gooby-Gruß
## (Betreten-Moment der Seele), DANN gleitet das Tagesbonus-Blatt rein.
const MORGEN_GRUSS_S := 2.4
...
## Feier-Abstand injizierbar (AGENTS.md „Zeit injizieren“): Tests takten
## Salven, ohne FEIER_ABSTAND_S × n Echtzeit auszusitzen.
var feier_abstand_s := FEIER_ABSTAND_S
...
## W18/J1: ein noch WARTENDES Dirigenten-Ticket wird ebenfalls zurück-
## gezogen — der Bonus bleibt bis Mitternacht abholbar (Resume prüft neu).
...
## W21/ACNH P1 (Erstlauf-Serialisierung, Befund „Overlay-Chaos aus 4
## Schichten“): Feiern (Toast + Konfetti + Gooby-Jubel-Blase) warten, bis
## der Overlay-Dirigent seine Willkommens-Sequenz (Tagesbonus, Coachmark,
## Geburtstag …) beendet hat — Sticker-Toast und Jubel-Blase stapelten
## sich sonst AUF dem Blatt/Coachmark. Polling statt Signal-await: die
## Sequenz kann auch über zurueckziehen() leer werden (kein sequenz_leer).
func _warte_auf_freie_buehne() -> void:
	while true:
		var dirigent := OverlayDirigent.find(self)
		if dirigent == null or not dirigent.belegt():
			return
		var tree := get_tree()
		if tree == null:
			return
...
## W18/J1 Morgen-Ritual: mit Dirigent im Baum reiht sich das Blatt dort ein
## (Vorlauf = Gooby begrüßt zuerst, danach ggf. der Quests-Hinweis) — ohne
## Dirigent (nackte Tests) öffnet es wie bisher direkt.
func _maybe_offer_daily_bonus() -> void:
	...
	if dirigent == null:
		_oeffne_tagesbonus()
		return
	if not dirigent.overlay_fertig.is_connected(_on_overlay_fertig):
		dirigent.overlay_fertig.connect(_on_overlay_fertig)
	dirigent.anfordern(
		"tagesbonus", OverlayDirigent.PRIO_TAGESBONUS, _oeffne_tagesbonus, MORGEN_GRUSS_S
	)


## Öffnen-Pfad des Tagesbonus-Blatts (ruft der Dirigent an seiner Reihe;
## should_offer wird HIER erneut geprüft — Mitternachts-/Claim-Rennen).
func _oeffne_tagesbonus() -> Control:
	if _daily_popup != null and is_instance_valid(_daily_popup):
		return _daily_popup
	if _gs == null or _layer == null or not DailyBonusPopup.should_offer(_gs, _local_day()):
		return null
...
## Morgen-Ritual-Schlussakkord: nach dem Tagesbonus-Blatt darf der
## „Was nun?“-Hinweis übernehmen (DANN ggf. Quests-Hinweis — der Advisor
## entscheidet selbst, ob er etwas zu sagen hat).
func _on_overlay_fertig(id: String) -> void:
	if id != "tagesbonus":
		return
