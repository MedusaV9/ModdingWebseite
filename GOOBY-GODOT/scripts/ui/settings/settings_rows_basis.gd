...
func _add_section(node_name: String, title: String, show_title := true) -> VBoxContainer:
	...
	rows.add_theme_constant_override("separation", AcTokens.px(12.0, _f))
	if show_title:
		var title_label := Label.new()
		title_label.name = "SectionTitle"
		# W21-Typo-Hierarchie: GENAU EIN Titel (SIZE_TITLE) pro Screen —
		# der gehört dem Screen-Kopf. Gruppen-Header stehen auf SIZE_BUTTON,
		# Sektions-Titel eine Stufe darunter (SIZE_BODY, 800er-Gewicht der
		# TitleLabel-Variation).
...
func _section_button(rows: VBoxContainer, node_name: String, text_key: String) -> SquishButton:
	...
	# 2-Höhen-System: Sektions-Knöpfe sind KOMPAKT (= Touch-Floor).
	btn.custom_minimum_size = Vector2(0.0, _row_floor())
...
func _add_help(rows: VBoxContainer, node_name: String, text: String) -> void:
	...
	label.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_CAPTION, _tf))
...
func _add_pick_row(
	...
	picker.custom_minimum_size = Vector2(float(AcTokens.px(208.0, _f)), _row_floor())
	picker.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BODY, _tf))
...
func _add_range_row(
	...
	slider.custom_minimum_size = Vector2(float(AcTokens.px(240.0, _f)), _row_floor())
...
func _make_row(rows: VBoxContainer, key: String, label_text: String) -> HBoxContainer:
	...
	row.add_theme_constant_override("separation", AcTokens.px(12.0, _f))
	var label := Label.new()
	label.name = "RowLabel"
	...
	label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	label.add_theme_font_size_override("font_size", AcTokens.font_px(AcTokens.SIZE_BODY, _tf))
	row.add_child(label)
	rows.add_child(row)
	return row
