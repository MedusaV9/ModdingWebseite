...
##
## W20 P1 — ZENTRALER VERDECKUNGS-VERTRAG (Befund-Top-1 „volles HUD hinter
## Tagesbonus/Telefon"): nicht mehr nur BuildMode+PanelSheet, sondern JEDES
## modale Overlay duckt das HUD. Drei zusätzliche Kanäle, alle ohne fremde
## Layout-Hunks:
## 1. `verdecken(grund)`/`freigeben(grund)` — manuelle Anmeldung mit
##    Grund-Zähler (mehrere Gründe/Verschachtelung bleiben robust).
## 2. PanelStack-Watcher: FREMDE Stack-Mitglieder ohne PanelSheet-Signale
##    (DailyBonusPopup, HeimkehrMoment, Geburtstags-Panel …) werden pro
##    Frame gezählt; HUD-eigene Mitglieder (Status-Sheet) zählen NICHT.
## 3. Gruppe `hud_verdecker`: Vollbild-Modals ohne PanelStack (PhoneShell)
##    melden sich per add_to_group an — sichtbar = HUD weicht.
## Die Guide-/Tour-Karte meldet sich BEWUSST NICHT an: sie ist eine
## Dauer-Karte, deren Schritte HUD-Knöpfe brauchen („öffne die Arcade").
##
## BOOT-LAGE (W20 P1c, Video-Review „Stats fehlten ~15 s"): es gibt KEINE
## Boot-Choreographie, die die Status-Spalte anfangs versteckt — das HUD
## startet mit leerem Grund-Zähler SICHTBAR (Wache: test_w20_overlay_choreo
## „Startlage: HUD sichtbar"); im Review-Video war die Spalte durchgehend
## gerendert und nur vom Cursor-folgenden Aufnahme-Ausschnitt abgeschnitten.
...
## W20 P1: Gruppe für Vollbild-Modals ohne PanelStack (z. B. PhoneShell) —
## sichtbares Mitglied ⇒ HUD weicht (Vertrag im Datei-Kopf).
const VERDECKER_GROUP := &"hud_verdecker"
...
## W20 P1: manuelle Verdeckungs-Gründe (StringName → Zähler, s. verdecken).
var _gruende: Dictionary = {}
## W20 P1: Frame-Watcher-Zustand (fremde PanelStack-Panels / Gruppe aktiv).
var _extern_verdeckt := false
var _verdeckt := false
...
## Frame-Watcher (Muster whats_next_hint._process): fremde PanelStack-
## Panels und `hud_verdecker`-Mitglieder haben keine Signale an uns —
## Übergänge werden hier erkannt und wie ein Blatt behandelt (BLENDE).
func _process(_delta: float) -> void:
	var extern := _fremde_panels() > 0 or _verdecker_aktiv()
	if extern == _extern_verdeckt:
		return
	_extern_verdeckt = extern
	_apply(Stil.BLENDE)
...
## W20 P1 — manuelle Anmeldung eines Verdeckungs-Grunds (Kanal 1 des
## Vertrags im Datei-Kopf). Zähler pro Grund: zweimal verdecken braucht
## zweimal freigeben; mehrere Gründe bleiben unabhängig (ODER).
func verdecken(grund: StringName) -> void:
	_gruende[grund] = int(_gruende.get(grund, 0)) + 1
	_apply(Stil.BLENDE)


func freigeben(grund: StringName) -> void:
	if not _gruende.has(grund):
		return
	var rest := int(_gruende[grund]) - 1
	if rest <= 0:
		_gruende.erase(grund)
	else:
		_gruende[grund] = rest
	_apply(Stil.BLENDE)


## Anzahl aktiver manueller Gründe (Tests/Debug).
func grund_zaehler() -> int:
	return _gruende.size()


## FREMDE PanelStack-Mitglieder (Kanal 2): Panels, die kein PanelSheet
## sind (die laufen über opened/closed) und nicht UNTER dem HUD hängen
## (Status-Sheet-Ausschluss). Sichtbarkeit zählt — ein unsichtbar
## geparktes Panel verdeckt nichts.
func _fremde_panels() -> int:
	var zahl := 0
	for panel: Control in PanelStack.members():
		if panel is PanelSheet:
			continue
		if _hud != null and _hud.is_ancestor_of(panel):
			continue
		if panel.is_inside_tree() and panel.is_visible_in_tree():
			zahl += 1
	return zahl


## Kanal 3: mindestens ein sichtbares Mitglied der Gruppe `hud_verdecker`
## (Vollbild-Modals ohne PanelStack, z. B. PhoneShell).
func _verdecker_aktiv() -> bool:
	var tree := get_tree()
	if tree == null:
		return false
	for node: Node in tree.get_nodes_in_group(VERDECKER_GROUP):
...
## Effektiver Zustand = Baumodus ODER ≥1 offenes Blatt ODER ein manueller
## Grund ODER ein externer Verdecker (Watcher). Nur ÜBERGÄNGE animieren —
## der Stil des Auslösers bestimmt die Animation.
func _apply(stil: int) -> void:
	var ziel := bau_aktiv() or blatt_zaehler() > 0 or not _gruende.is_empty() or _extern_verdeckt
	if ziel == _verdeckt:
		return
	_verdeckt = ziel
	if ziel:
