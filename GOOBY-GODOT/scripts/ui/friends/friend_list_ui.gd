...
extends RefCounted
## Gemeinsame Optik-Helfer für SocialScreen + PhoneFriendsApp (W4-P4 POLISH):
...
static func build_empty_state(_art_key: String, text_key: String, ui_scale := 1.0) -> Control:
	...
	# `ScreenShell.scale_fonts` fahren, dürfen hier nicht noch einmal
	# multiplizieren (Runde-2-Befund: Mini-Sticker/Doppel-Skala).
