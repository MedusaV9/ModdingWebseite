...
func _show_step(step: OnboardingLogic.Step) -> void:
	...
	_animate_step_in()
	_speak_step(step)
	# Sichtbare Karte ist vollständig per Tastatur/Controller erreichbar;
	# nach jedem Schritt beginnt der Fokus am ersten sinnvollen Eingabefeld.
	FocusNavigation.grab_first_deferred(_steps[step])
