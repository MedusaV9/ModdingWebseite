...
## Aktueller UiScale-Faktor (setzt _apply_metrics) — Zeilen-Bauer skalieren
## Balkenhöhen damit; 1.0 = Standalone/Tests ohne Metrics-Lauf.
var _f := 1.0
...
func _build_ui() -> void:
	# W21 (d): Trophäen-Stimmung (Gold-Akzent) statt Standard-Blätter.
	add_child(AcWallpaper.for_context("erfolge"))
...
func _build_header() -> Control:
	_header = AcnhKit.blatt_kopf(I18nService.t("achievements.titel"), _on_back_pressed)
...
func _show_category(cat: String) -> void:
	...
	# W21 („nichts hat die gleiche Größe“): Zeilen entstehen NACH
	# _apply_metrics (und bei jedem Kategorie-Wechsel) — ohne Nachskalieren
	# blieben Name/Beschreibung dauerhaft in Basis-px (20 statt 33).
	ScreenShell.scale_fonts(_list_box, _f)
	# W21 (c): Zeilen federn beim Kategorie-Wechsel gestaffelt ein
	# (MotionKit.stagger_ein ist Reduced-Motion-gated).
...
func _build_row(def: Dictionary) -> Control:
	...
	# W21-Typo-Hierarchie: Zeilen-Titel = Titel-Stufe (wie Profil-Karten) —
	# Headline bleibt der EINEN Hero-Zeile pro Ansicht vorbehalten.
...
func _build_progress(def: Dictionary, unlocked: bool) -> Control:
	...
	# W21: EIN Balken-Standard (BAR_H) für alle Fortschrittsbalken.
	bar.custom_minimum_size = Vector2(0.0, AcTokens.px(AcTokens.BAR_H, _f))
