...
## W20 B2 („Reisepass läuft unten aus dem Canvas“): Kompakt-Modus fürs
## Querformat — die hinteren Feldzeilen (Spielzeit/Serie/Merkmale) wandern
## in eine ZWEITE Spalte neben die erste. Mit Geräte-Metriken (f=3,0)
## stapelten sonst 7 Zeilen à ~145 px und die halbe Karte fehlte.
var _kompakt := false
var _felder_links: VBoxContainer
var _felder_rechts: VBoxContainer
var _kompakt_reihen: Array[BoxContainer] = []
## Kopf-Spalte (Republik über Dokument) — kompakt kippt sie EINZEILIG.
var _kopf_spalte: BoxContainer
...
func _ready() -> void:
	# W21/P3 (Profil-Kartenraster): AcCard wie ALLE Profil-Karten — gleiche
	# Breite/Radius/Polster statt Sonder-Radius (Wächter profil_kartenraster).
...
func _baue_vorderseite() -> VBoxContainer:
	...
	# Plain-BoxContainer statt VBox: nur der kann die Achse wechseln —
	# der Kompakt-Kopf kippt die Spalte einzeilig (s. _wende_kompakt_an).
	var kopf_spalte := BoxContainer.new()
	kopf_spalte.vertical = true
	kopf_spalte.add_theme_constant_override("separation", 0)
	kopf.add_child(kopf_spalte)
	_kopf_spalte = kopf_spalte
	...
	wort.uppercase = true
	# Einzeiliger Kompakt-Kopf: das Wort hängt dann NEBEN der Republik und
	# sitzt vertikal mittig statt an der Zeilen-Oberkante.
	wort.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	...
	_felder_links = felder
	# W20 B2: zweite Felder-Spalte für den Kompakt-Modus (quer) — leer und
	# unsichtbar, bis setze_kompakt die hinteren Zeilen herüberzieht.
	_felder_rechts = VBoxContainer.new()
	_felder_rechts.name = "FelderRechts"
	_felder_rechts.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_felder_rechts.alignment = BoxContainer.ALIGNMENT_CENTER
	_felder_rechts.add_theme_constant_override("separation", 6)
	_felder_rechts.visible = false
	reihe.add_child(_felder_rechts)
	...
	_kompakt_reihen.clear()
	var spielzeit := _field_row("Spielzeit", I18nService.t("profil.spielzeit"), _spielzeit_text())
	felder.add_child(spielzeit)
	var serie := _field_row(
		"Serie",
		I18nService.t("profil.serie"),
		I18nService.t("profil.serie_wert", {"n": int(_wert("daily.streak", 0))})
	)
	felder.add_child(serie)
	var merkmale := _field_row("Merkmale", I18nService.t("reisepass.merkmale"), _merkmal_text())
	felder.add_child(merkmale)
	# Diese drei Zeilen wandern im Kompakt-Modus in die rechte Spalte
	# (Reihenfolge bleibt beim Zurückwandern erhalten — s. setze_kompakt).
	_kompakt_reihen.assign([spielzeit, serie, merkmale])
	...
	# G3: Grundmaß bleibt der Kompakt-Knopf (48 Design-px) — den physischen
	# Touch-Floor legt _wende_floors_an nach dem Einhängen obendrauf.
	foto_btn.custom_minimum_size = Vector2(0.0, float(AcTokens.BTN_H_KOMPAKT))
...
func _baue_level_row() -> Control:
	...
	bar.custom_minimum_size = Vector2(0.0, float(AcTokens.BAR_H))
...
func _baue_mrz() -> Control:
	...
	for i in mrz.size():
		...
		label.add_theme_font_size_override("font_size", AcTokens.SIZE_CAPTION)
...
## W21: dito XpBar-Höhe ×f auf dem EINEN Balken-Standard (BAR_H).
func _wende_floors_an() -> void:
	if not is_inside_tree():
		return
	var m := ScreenShell.metrics(get_viewport())
	if _foto_btn != null and is_instance_valid(_foto_btn):
		ScreenShell.touch_target(_foto_btn, m)
	var xp_bar := find_child("XpBar", true, false) as ProgressBar
	if xp_bar != null:
		xp_bar.custom_minimum_size = Vector2(0.0, AcTokens.px(AcTokens.BAR_H, float(m["f"])))
...
func _on_foto_aendern() -> void:
	...
	if fotos.is_empty():
		...
		leer.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		leer.custom_minimum_size = Vector2(AcTokens.px(320.0, f), 0.0)
		box.add_child(leer)
	else:
	...
	if not passfoto_von(_state()).is_empty():
		...
		standard.custom_minimum_size = Vector2(0.0, AcTokens.px(AcTokens.BTN_H_KOMPAKT, f))
	...
	abbrechen.custom_minimum_size = Vector2(0.0, AcTokens.px(AcTokens.BTN_H_KOMPAKT, f))
...
func _baue_picker_raster(fotos: Array, m: Dictionary) -> Control:
	...
	var breite := minf(
		AcTokens.px(420.0, f), ScreenShell.card_width(m, 520.0) - AcTokens.px(40.0, f)
	)
	var hoehe := minf(
		AcTokens.px(260.0, f),
		maxf(ScreenShell.card_max_height(m) - 3.2 * float(m["floor_px"]), 160.0)
	)
	scroll.custom_minimum_size = Vector2(breite, hoehe)
	var raster := GridContainer.new()
	raster.name = "PickerRaster"
	raster.columns = clampi(int(breite / AcTokens.px(140.0, f)), 2, 4)
...
func _field_row(zeilen_name: String, key_text: String, value_text: String) -> Control:
	...
	var value := Label.new()
	value.name = "Wert"
	# W21/P3 („Weltme“-Fix, Teil 1): Werte laufen auf der Body-Stufe mit
	# 800er-Gewicht statt als Headline — die Hierarchie bleibt (Schlüssel
	# soft, Wert kräftig), aber lange Werte brechen lesbar um, statt die
	# Kompakt-Spalte zu sprengen (Befunde §Reisepass).
	value.add_theme_font_override("font", ThemeService.font(800))
...
func setze_schmal(schmal: bool) -> void:
	if _schmal == schmal:
		return
	_schmal = schmal
	_foto_flags_anwenden()
	for row in _feld_reihen:
		if is_instance_valid(row):
			_wende_schmal_an(row)


## W20 B2: Kompakt-Modus (Querformat) — die hinteren Feldzeilen wandern in
## die rechte Spalte, die Karte halbiert grob ihre Feldhöhe und passt mit
## Geräte-Metriken wieder KOMPLETT in den Canvas (Probe pass_im_canvas).
## Zurückwandern hängt die Zeilen in Originalreihenfolge ans Spalten-Ende.
func setze_kompakt(kompakt: bool) -> void:
	_kompakt = kompakt
	_wende_kompakt_an()


## Kompakt-Zustand auf die (fertig gebaute) Vorderseite anwenden — auch
## von _ready gerufen, falls setze_kompakt VOR dem Baum-Aufbau kam.
func _wende_kompakt_an() -> void:
	if _felder_rechts == null:
		return
	_felder_rechts.visible = _kompakt
	var ziel := _felder_rechts if _kompakt else _felder_links
	for row in _kompakt_reihen:
		if is_instance_valid(row) and row.get_parent() != ziel:
			row.get_parent().remove_child(row)
			ziel.add_child(row)
	# Kompakt-Kopf: „GOOBY-REPUBLIK · Reisepass“ EINZEILIG nebeneinander
	# statt gestapelt — spart eine volle Textzeile Kartenhöhe.
	if _kopf_spalte != null:
		_kopf_spalte.vertical = not _kompakt
		_kopf_spalte.add_theme_constant_override("separation", 10 if _kompakt else 0)
	# W21/P3 („Weltme“-Fix, Teil 2): Werte umbrechen ÜBERALL — der frühere
	# Ellipsen-Deckel (AUTOWRAP_OFF + TRIM_ELLIPSIS quer) entfällt, weil die
	# Werte jetzt auf der flachen Body-800-Stufe laufen statt als Headline;
	# lange Werte (Merkmale-Gag) brechen lesbar um, ohne die Karte aus dem
	# Canvas zu drücken. Kompakt bekommt der Wert mehr Platz als der kurze
	# Schlüssel; 50/50 ließ "Merkmale" auf vier Zeilen wachsen und schob die
	# Pass-Karte im Leit-Querformat unter den Canvas.
	for row in _feld_reihen:
		...
		if wert == null:
			continue
		wert.size_flags_stretch_ratio = 2.4 if _kompakt else 1.6
		# In der schmalen rechten Querformat-Spalte sind die Feldschlüssel
		# Meta-Labels: Caption-Größe verhindert Vierzeiler wie
		# "Besondere Merkmale", während der eigentliche Wert Body bleibt.
		var key := row.get_child(0) as Label
		if key != null and is_inside_tree():
			var f := float(ScreenShell.metrics(get_viewport())["f"])
			var key_size := AcTokens.SIZE_CAPTION if _kompakt else AcTokens.SIZE_BODY
			key.add_theme_font_size_override("font_size", AcTokens.font_px(key_size, f))
	_foto_flags_anwenden()


## Foto-Slot-Vertikalverhalten: schmal = oben (quadratisch über den
## gestapelten Feldern), kompakt = mittig neben den zwei Spalten,
## sonst zeilenhoch mitwachsend (Bestand).
func _foto_flags_anwenden() -> void:
	if _schmal:
		foto_slot.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
	elif _kompakt:
		foto_slot.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	else:
		foto_slot.size_flags_vertical = Control.SIZE_FILL
