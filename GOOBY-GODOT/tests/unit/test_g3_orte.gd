...
func test_flughafen_leiste_verlassen_und_gfree_sichtbar() -> void:
	...
	# W19-Playtest-Fund: die Caption schwebt frei über dem 3D-Bild —
	# ohne helle Kontur läuft Gooby/heller Boden durch den Text.
	assert_true(
		hinweis.has_theme_color_override("font_outline_color"),
		"Hinweis-Zeile trägt eine Kontur-Farbe (Lesbarkeit über 3D)"
	)
	assert_true(hinweis.get_theme_constant("outline_size") > 0, "Hinweis-Kontur hat Breite > 0")
