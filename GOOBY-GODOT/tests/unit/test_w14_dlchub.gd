...
## spielbar (goo_und_bye → Angebot; mcgooby seit W19/Welle B mit Kauf-Gate
## Level 14/3000 + täglich freier Probeschicht als Demo-Zweitknopf) —
...
## testen test_dlc_goobye.gd / test_dlc_mcgooby.gd / test_w19_mcgooby_b.gd.
...
func test_dlc_copy_verspricht_nur_implementierte_funktionen() -> void:
	var text := FileAccess.get_file_as_string(PACK_DATEI).to_lower()
	for unbelegt: String in [
		"vier knuffige mitarbeiter",
		"four adorable staff",
		"freunde kaufen echt",
		"friends really shop",
		"koop-schichten",
		...
		assert_false(text.contains(unbelegt), "unbelegte Store-Behauptung entfernt: %s" % unbelegt)
	for belegt: String in [
		"kühlmodule",
		"cooling modules",
		"lieferwagen",
		"delivery van",
		...
		assert_true(
			text.contains(belegt), "implementierter DLC-Loop wird konkret benannt: %s" % belegt
		)
...
func test_status_ableitung_ranch() -> void:
	...
	# W19/McGooby-B: Kauf-Gate nach dem Goobye-Muster (Level 14 / gekauft).
	var mcgooby := DlcKatalog.eintrag("mcgooby")
	assert_eq(
		DlcKatalog.status_fuer(mcgooby, _gs(1, false)),
		DlcKatalog.STATUS_GESPERRT,
		"mcgooby: Level < 14 → gesperrt"
	)
	assert_eq(
		DlcKatalog.status_fuer(mcgooby, _gs(14, false)),
		DlcKatalog.STATUS_VERFUEGBAR,
		"mcgooby: Level 14, nicht gekauft → verfügbar"
	)
	var mc_gekauft := _gs(1, false)
	mc_gekauft.set_value("mcgooby.gekauft", true)
	assert_eq(
		DlcKatalog.status_fuer(mcgooby, mc_gekauft),
		DlcKatalog.STATUS_INSTALLIERT,
...
func test_aktions_und_routen_mapping() -> void:
	...
	# W19/McGooby-B: Kauf-Gate — Angebot ab Level 14, gekauft → Schicht.
	var mcgooby := DlcKatalog.eintrag("mcgooby")
	assert_eq(str(mcgooby.get("route", "")), "mcgooby_schicht", "mcgooby: Schicht-Route")
	assert_eq(DlcKatalog.aktion_fuer(mcgooby, _gs(1, false)), DlcKatalog.AKTION_GESPERRT)
	assert_eq(DlcKatalog.aktion_fuer(mcgooby, _gs(14, false)), DlcKatalog.AKTION_ANGEBOT)
	var mc_gekauft := _gs(1, false)
	mc_gekauft.set_value("mcgooby.gekauft", true)
...
func test_screen_smoke_drei_karten_und_details() -> void:
	...
	detail.queue_free()
	# W19/McGooby-B: Level 20, nicht gekauft → Angebots-Knopf PLUS der
	# Demo-Zweitknopf (die Probeschicht bleibt täglich frei spielbar).
	var schicht := screen.oeffne_detail("mcgooby")
	var schicht_knopf: Button = schicht.get_meta(DlcScreen.META_AKTION, null)
	assert_true(schicht_knopf != null and not schicht_knopf.disabled, "Angebots-Knopf aktiv")
	assert_eq(schicht_knopf.text, I18nService.t("dlc_mcgooby.knopf.angebot"))
	var demo_knopf: Button = schicht.get_meta(DlcScreen.META_DEMO, null)
	assert_true(demo_knopf != null and not demo_knopf.disabled, "Demo-Knopf aktiv")
	assert_eq(demo_knopf.text, I18nService.t("dlc_mcgooby.knopf.probeschicht"))
	schicht.queue_free()
	screen.queue_free()
	await wait_frames(1)
