...
## EVAL-2026-08 Lens B Befund 18 (Shot 17 „Kannst du kurz be“ — Standbild
## riss mitten im Wort ab): die Blase poppt mit KOMPLETTER erster Zeile
## (Einzeiler sofort ganz) und enthüllt danach WORT-weise — jeder
## Zwischenstand endet an einer Wortgrenze, nie in einem Wort-Fragment.
func test_acbubble_typewriter_erste_zeile_dann_wortweise() -> void:
	AcBubble.warteschlange = AcBubble.Warteschlange.new()
	var layer := Control.new()
	layer.size = Vector2(1280, 720)
	tree.root.add_child(layer)
	# Einzeiler: die ERSTE Zeile ist der ganze Text — steht sofort komplett.
	var kurz := AcBubble.show_bubble(layer, "Zwölf Zeichen", {"dauer_s": 9.0})
	kurz.auto_zeit = false
	var kurz_label := kurz.get_node("Kapsel/BubbleText") as Label
	assert_eq(kurz_label.visible_characters, -1, "Einzeiler steht beim Pop komplett")
	# Mehrzeiler (wickelt sicher über MAX_WIDTH): Zeile 1 komplett beim
	# Pop, der Rest tippt Wort für Wort nach (Zeit injiziert).
	var text := (
		"Weißt du was? Ich wollte schon immer mal in den Funkelpark"
		+ " und dann noch eine ganze Woche lang Waffeln mit dir essen!"
	)
	var lang := AcBubble.show_bubble(layer, text, {"dauer_s": 9.0})
	lang.auto_zeit = false
	var label := lang.get_node("Kapsel/BubbleText") as Label
	var start := label.visible_characters
	assert_true(start > 0 and start < text.length(), "Pop: Zeile 1 steht, Rest tippt noch")
	assert_eq(start, AcBubble.wort_grenze(text, start), "Pop endet an einer Wortgrenze")
	var frueher := start
	for _i in 60:
		lang.advance_time(1.0 / GoobyVoice.RATE)
		var jetzt := label.visible_characters
		if jetzt < 0:
			break
		assert_true(jetzt >= frueher, "Reveal läuft nur vorwärts")
		assert_eq(jetzt, AcBubble.wort_grenze(text, jetzt), "Zwischenstand an Wortgrenze")
		frueher = jetzt
	lang.advance_time(60.0)
...
func test_acbubble_ersetze_text_statt_stapeln() -> void:
	...
	# Befund 18: Einzeiler stehen beim Pop KOMPLETT (keine Tipp-Zeit mehr) —
	# advance_time zählt ab sofort reine Standzeit.
