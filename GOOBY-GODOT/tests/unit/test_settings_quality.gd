...
func test_mesh_lod_folgt_sichtweite_nicht_schatten() -> void:
	var prev_fps := Engine.max_fps
	var settings: Node = AppSettingsScript.new(_fresh_path("settings.json"))
	settings.set_setting("graphics.preset", "benutzerdefiniert")
	settings.set_setting("graphics.draw_distance", 0.7)
	settings.set_setting("graphics.shadows", "hoch")
	var pair := _service_mit(settings)
	var svc: QualityService = pair[0]
	var viewport: SubViewport = pair[1]
	assert_almost(
		viewport.mesh_lod_threshold,
		QualityService.LOD_THRESHOLD_FAR,
		0.001,
		"kurze Sichtweite wechselt früh auf grobe Meshes"
	)
	settings.set_setting("graphics.shadows", "aus")
	assert_eq(viewport.positional_shadow_atlas_size, 0, "Schatten-Regler wirkt aufs Atlas")
	assert_almost(
		viewport.mesh_lod_threshold,
		QualityService.LOD_THRESHOLD_FAR,
		0.001,
		"Schatten ändern die LOD-Schwelle nicht"
	)
	settings.set_setting("graphics.draw_distance", 1.0)
	assert_almost(
		viewport.mesh_lod_threshold,
		QualityService.LOD_THRESHOLD_NEAR,
		0.001,
		"Sichtweiten-Regler wirkt direkt auf mesh_lod_threshold"
	)
	svc.free()
	viewport.free()
	settings.free()
...
func test_lod_sichtweiten_mapping_klemmt_und_ist_monoton() -> void:
	assert_almost(QualityService.lod_threshold_for(0.0), 4.0, 0.001)
	assert_almost(QualityService.lod_threshold_for(0.7), 4.0, 0.001)
	assert_almost(QualityService.lod_threshold_for(0.85), 2.5, 0.001)
	assert_almost(QualityService.lod_threshold_for(1.0), 1.0, 0.001)
	assert_almost(QualityService.lod_threshold_for(9.0), 1.0, 0.001)
