...
func test_fuzz_save_manager_bootet_immer() -> void:
	...
	# binaer_muell als BYTES statt \u0000-String-Literal: ein NUL im Literal
	# laesst den Export-Tokenizer (Binaertokens, script_export_mode=2) bei
	# JEDEM Export "Unicode parsing error ... NUL character" warnen (Lens C
	# Befund 10). Gleiche Bytes wie das alte Literal "\u0000\u0001GOOBY\u00ff
	# kaputt" in UTF-8 — via store_buffer landet der Muell unveraendert auf
	# der Platte, der Testinhalt ist identisch.
	var binaer_muell := PackedByteArray([0x00, 0x01])
	binaer_muell.append_array("GOOBY\u00ff kaputt".to_utf8_buffer())
	...
	for mut_name: String in text_level_payloads.keys():
		...
		var payload: Variant = text_level_payloads[mut_name]
