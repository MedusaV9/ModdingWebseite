extends TestCase
## VERPFLICHTENDER Integrationstest gegen den ECHTEN GOOBY-SERVER (W3c VISIT,
...
## Fehlender Server, fehlendes Node oder ein nicht startbarer Prozess sind
## harte Fehler: Release-CI darf diese Systemgrenze niemals per Skip passieren.
...
func test_social_stack_gegen_echten_server() -> void:
	...
	if OS.execute(ENV_BIN, ["node", "--version"]) != 0:
		fail_test("node ist nicht über PATH auffindbar")
		return
