...
## (Der frühere Friends-Anfrage-Check lief gegen den gelöschten
## Vollbild-Freunde-Screen — die Handy-Variante deckt
## test_g5_cutscene_friends ab.)
...
func test_social_screen_hat_stabile_fokusnavigation() -> void:
	var services := SocialServices.new()
	tree.root.add_child(services)
	var screen := SocialScreen.new()
	screen.services_override = services
	...
	screen._add_incoming_card("Anfrage", "Ja", "Nein", func() -> void: pass, func() -> void: pass)
	await wait_frames(2)
	assert_eq(screen._back_btn.focus_mode, Control.FOCUS_ALL, "Zurück fokussierbar")
	assert_eq(screen._practice_btn.focus_mode, Control.FOCUS_ALL, "Offline-Übung fokussierbar")
	assert_eq(tree.root.gui_get_focus_owner(), screen._back_btn, "Social startet mit Zurück-Fokus")
	var controls := FocusNavigation.focusable(screen)
	assert_true(controls.size() >= 4, "Zurück, Übung und Anfrage-Aktionen erreichbar")
	for control in controls:
		assert_false(control.focus_next.is_empty(), "%s hat Tab-Nachfolger" % control.name)
		assert_false(control.focus_previous.is_empty(), "%s hat Tab-Vorgänger" % control.name)
	screen.queue_free()
	services.queue_free()
	await wait_frames(1)
