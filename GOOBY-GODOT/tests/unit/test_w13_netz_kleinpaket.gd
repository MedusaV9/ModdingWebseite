...
func test_produkt_default_ist_ehrlich_offline_statt_loopback() -> void:
	assert_eq(NetClient.DEFAULT_NET, {"host": "", "port": 443, "tls": true})
	assert_eq(ContentRegistryService.NET_DEFAULTS, NetClient.DEFAULT_NET)
	var client := NetClient.new()
	client.auto_connect = false
	client.build_services = false
	var identity_path := "user://w13_unconfigured_identity.json"
	client.identity_path = identity_path
	var links: Array = []
	...
	await wait_frames(1)
	client.connect_now()
	assert_eq(links.size(), 0, "ohne Release-/User-Endpunkt kein ungültiger Verbindungsversuch")
	assert_eq(client.status, NetClient.Status.OFFLINE)
	assert_false(
		client.is_processing(), "unkonfigurierter Offline-Client erzeugt keine Retry-Schleife"
	)
	client.queue_free()
	await wait_frames(1)
