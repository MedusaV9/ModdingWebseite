...
func _mount(game_id: String) -> Node:
	...
	await wait_frames(1)
	game.call("start")
	# Level-Select-Spiele bis in den Gefechts-Pfad öffnen. Explizite Calls
	# bewahren dabei die unterschiedlichen Signaturen (gvz: int,
	# gobnom: String + int); die frühere untypisierte callv-Tabelle konnte
	# den gvz-Integer als erstes gobnom-Argument weiterreichen.
	if game_id == "gvz":
