extends TestCase
## VERPFLICHTENDER Integrationstest gegen den ECHTEN GOOBY-SERVER (W2c):
...
## FRIENDS_STATE) über ECHTE WebSocketPeer-Verbindungen. Fehlender Server,
## fehlendes Node oder ein nicht startbarer Prozess sind harte Testfehler:
## Release-CI darf diese Systemgrenze niemals per Skip grün färben.

const ENV_BIN := "/usr/bin/env"


func test_hello_und_freunde_flow_gegen_echten_server() -> void:
	var server_js := _server_js()
	if not FileAccess.file_exists(server_js):
		fail_test("GOOBY-SERVER/server.js fehlt am Repo-relativen Pfad: %s" % server_js)
		return
	var probe := OS.execute(ENV_BIN, ["node", "--version"])
	if probe != 0:
		fail_test("node ist nicht über PATH auffindbar (Exit %d)" % probe)
		return
	...
	if pid <= 0:
		fail_test("echter GOOBY-Server-Prozess startet nicht (PID %d)" % pid)
		return
...
func _server_js() -> String:
	return ProjectSettings.globalize_path("res://../GOOBY-SERVER/server.js").simplify_path()
