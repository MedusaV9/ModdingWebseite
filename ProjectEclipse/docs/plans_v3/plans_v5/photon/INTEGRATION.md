...
- **`reducedFx` integration (already law, keep it):** `PhotonBridge.available()`
  ...
  bypass `FxBudget`, `reducedFx` is the ONLY throttle. Registry rule: never register a
  high-frequency cue (ore procs, glide trails, ambient loops) with a Photon leg;
  Photon rows are reserved for SEQUENCE-grade one-shots (altar ceremony, rift opens,
  future finale beats). If a future cue needs rate control, gate its send server-side
  (like the existing beam-range send) — do not add a client-side Photon budget until a
  real problem is measured.
