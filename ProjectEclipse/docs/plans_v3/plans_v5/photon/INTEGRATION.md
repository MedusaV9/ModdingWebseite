...
|---|---|---|---|
| Altar milestone level-up | `ritual/AltarBlockEntity.completeMilestone` sends `S2CQuasarPayload(ALTAR_LEVELUP_RING)` to players in beam range (world-wide at L5) | `EclipsePayloads.handleQuasar` → `PhotonBridge.enhanceQuasarCue(emitterId, pos)` → `QuasarSpawner.spawnOrFallback` | `enhanceQuasarCue` matches `ALTAR_LEVELUP_RING` → `spawn(eclipse:altar_levelup)` |
...
> record became `Row` and grew a `loop` flag (WINDOWED-only law, §4). Duplicate logical
> ids are refused with a WARN (first registration wins). `PhotonBridge` also gained the
> §3.5 entity executor (`spawnOnEntity`), `SpawnOptions`
> (offset/rotation/scale/delay/allowMulti on both executor kinds),
> `spawnLoop`/`stopLoop` (handle + `getRuntime().destroy(force)`), a per-tick sweep of
> every live executor (dead runtime → forget; dead entity / level change → destroy) and
> a hard budget of `MAX_LIVE_EXECUTORS = 24` live Photon executors (spawns beyond it are
> refused). Dev smoke tests: `/dev photon status` and `/dev photon test <fxId> [pos]`
> (client-targeted over the `FxDevPayloads` lane).
...
- **`reducedFx` integration (already law, keep it):** `PhotonBridge.available()`
  ...
  high-frequency cue (ore procs, glide trails) with a Photon leg;
  Photon rows are reserved for SEQUENCE-grade one-shots (altar ceremony, rift opens,
  future finale beats) and WINDOWED loops (next rule). If a future cue needs rate
...
## 6. Authoring workflow — editor `.fx` → repo

Photon effects are DATA: compressed-NBT `.fx` files authored in Photon's in-game
...
1. **Author:** launch the dev client (`run/mods-client` already carries photon+ldlib2
   ...
   flow, build the effect. Iterate in-world against the real cue (trigger with the
   existing dev commands, e.g. `FxDevCommands` for rifts).
2. **Export:** save/export the effect from the editor as a `.fx` file (compressed NBT —
   never hand-edit; re-export for every change).
3. **Place — two sanctioned destinations:**
...
4. **Id contract:** the file name must match the logical id path the bridge/registry
...
5. **Verify:** join a world with Photon installed and trigger the cue; success is the
   absence of the one-time INFO `Photon is loaded but assets/eclipse/fx/<id>.fx is
   absent` and the effect visibly layering over the Quasar cue. A WARN
   `Photon effect <id> failed` means a bad export — re-export.
6. **Do NOT commit third-party `.fx` files** ripped from other packs/mods (license), and
