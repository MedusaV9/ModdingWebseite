...
## STORM 2.0 W-C additions (2026-07-25)

Photon suite assets generated by `tools/photon/build_storm_fx.py` (committed fxlib
source of truth), consumed by `stormfx/StormPhotonFx`. `.fxproj` files are the
editor-openable uncompressed-NBT siblings (PHOTON-ADVANCED-1 §7) of their `.fx` blobs.

| Path | Size | Status |
|---|---:|---|
| `assets/eclipse/fx/storm_debris_belt.fx` (+`.fxproj` 17.1 KB) | 2.3 KB | USED — `StormPhotonFx` windowed loop |
| `assets/eclipse/fx/storm_cloud_belt.fx` (+`.fxproj` 14.5 KB) | 1.9 KB | USED — `StormPhotonFx` windowed loop |
| `assets/eclipse/fx/storm_vein_bolt.fx` (+`.fxproj` 11.4 KB) | 2.1 KB | USED — `StormPhotonFx` flash-serial one-shot |
| `assets/eclipse/fx/storm_skirt_dust.fx` (+`.fxproj` 9.1 KB) | 1.9 KB | USED — `StormPhotonFx` windowed loop |
| `assets/eclipse/fx/storm_dust_puff.fx` (+`.fxproj` 7.2 KB) | 1.7 KB | USED — `storm_skirt_dust` FirstCollision sub-emitter child |
| `assets/eclipse/textures/particle/storm_puff_atlas.png` | 296 KB | USED — `storm_cloud_belt` 4x4 SingleRow flipbook (1024², IDEAS-STORM-2 #3) |