...
`assets/eclipse/geo/entity/fog_tyrant.geo.json` (GeckoLib, 25 bones / 35 cubes,
4.2-block regal storm wraith — layered tattered storm-robe, caged chest core, twin
lance arms, fog-plume shoulder wisps, hooded head under a floating shard-crown;
hovers, never quite touches the ground). As with all GeckoLib mobs, the geo file **is** the UV source of truth — the
