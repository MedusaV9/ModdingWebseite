...
// near-black and the only light left is a SONAR read. A thin depth-banded contour shell
// pulses OUTWARD from the player (linearized depth ≈ distance from the camera, so the
// expanding equidistance shell is a true 3D ping crawling over the terrain), leaving a
// fading afterglow ring behind it; faint static depth contours every few blocks keep a
// minimal sense of space between pings, and a whisper of silhouette edge keeps nearby
// mass readable. Sky is beyond the ping range: pure void. Fed by client.GlitchZoneFx:
// Strength (0..1 ramp — no-op at 0), Time (wall-clock seconds; drives the ping clock),
// Detail (0 under reduced FX: the sweep parks at a fixed radius — banded contours stay,
// nothing moves).
...
void main() {
    ...
    float ring = exp(-abs(dist - sweep) * 2.2);
    float behind = step(dist, sweep) * exp(-(sweep - dist) * 0.12);
    // The ping loses energy as it expands (and the parked reduced-FX shell holds ~60%).
    float energy = mix(0.6, 1.0 - 0.65 * phase, detail);
    float ping = (ring * 1.0 + behind * 0.22) * energy * (1.0 - sky) * step(dist, PING_RANGE + 8.0);

    // --- static depth contours ---------------------------------------------------------------
    // Faint equidistance lines every 6 blocks (triangle-wave band around the contour),
    // fading with distance — the minimal map that keeps the void navigable between pings.
    float bandPos = abs(fract(dist / 6.0) - 0.5) * 2.0;
