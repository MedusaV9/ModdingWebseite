...
/**
 * Timed server buff engine (R16/R17): persistence, tick-down, announcements, and
 * {@link S2CBuffStatePayload} sync. Registers as the live {@link TimedBuffApi} delegate.
 *
 * <p>PLAN-A A7: the rotating buff bossbar is gone — buff timers render ONLY in the
 * TAB-expanded sidebar, fed by the existing buff/sidebar payloads (no
 * {@code ServerBossEvent} is ever created here).</p>
 */
...
public final class TimedBuffService implements TimedBuffApi {
    ...
    // PLAN-A A7: refreshBossbar/ensureBossbar/removeBossbar (and the rotating bar name)
    // are deleted — buff countdowns reach the client via S2CBuffStatePayload and the
    // sidebar aggregate; the TAB-expanded sidebar is the only timer surface.
}
