...
public final class FxCues {
    ...
    /**
     * MB2 §7 / POLISH1-FXCOUPLING: star_call Photon partner ({@code eclipse:
     * wizard_star_call} + {@code _fast}) — staff-tip mote column during the rooted
     * raise, gather indraw, release ring + beam pulse on the telegraph beat, then a
     * conducting star-dust drizzle exactly as long as the bolts fall. The child fx
     * sells the DIRIGIEREN; the server's END_ROD/Firework impacts keep owning the
     * {@code starZone} (MB2 budget/readability law — the zone is TABU for this asset).
     * ENTITY lane on Orin ({@code veilfx/WizardFxRows} row, {@code AutoRotate.XROT}
     * staff-tip anchor); {@code a} = seconds to the release beat (1.30 base / 0.95
     * unveiled — the row's leg swaps in the {@code _fast} asset for {@code a <= 1.1},
     * closing MB2 §9.2's "Anim-Akzent 0.35 s zu spät" honesty gap), {@code b} = shower
     * seconds (bolts × 0.25, informational — the two variants bake the matching
     * 50t/70t drizzle windows). Sent by {@code WizardOrinEntity.tickCombat} on the
     * telegraph-START tick, the same tick as the ANIM_STAR_CALL trigger.
     */
